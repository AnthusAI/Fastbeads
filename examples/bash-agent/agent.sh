...
if ! command -v fbd &> /dev/null; then
    log_error "fbd is not installed"
    echo "Install with: go install github.com/steveyegge/fastbeads/cmd/fbd@latest"
    exit 1
fi

# Check if we're in a beads-initialized directory
if ! fbd list &> /dev/null; then
...
# Find ready work
find_ready_work() {
    fbd ready --json --limit 1 2>/dev/null | jq -r '.[0] // empty'
}
...
claim_task() {
    ...
    fbd update "$issue_id" --status in_progress --json > /dev/null
...
do_work() {
    ...
    if [[ $((RANDOM % 2)) -eq 0 ]]; then
        ...
        # Create new issue
        local new_issue=$(fbd create "Follow-up: $title" \
        ...
        fbd dep add "$new_id" "$issue_id" --type discovered-from
...
complete_task() {
    ...
    fbd close "$issue_id" --reason "$reason" --json > /dev/null
