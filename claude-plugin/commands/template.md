...
```bash
fbd template list
fbd template show <template-name>
fbd template create <template-name>
```

## Description

Templates can be:
- **Built-in**: Provided by fbd (epic, bug, feature)
...
```bash
fbd template show <template-name>
fbd template show <template-name> --json
```

**Examples:**

```bash
$ fbd template show bug
Template: bug
Type: bug
Priority: P1
Labels: bug
...
```bash
$ fbd template create performance
...
## Using Templates with `fbd create`

Use the `--from-template` flag to create issues from templates:

```bash
fbd create --from-template <template-name> "Issue title"
```
...
fbd create --from-template bug "Login crashes on special chars" -p 0

# Use epic template but add extra labels
fbd create --from-template epic "Q4 Infrastructure" -l infrastructure,ops
```
...
$ fbd create --from-template epic "Phase 3 Features"
...
$ fbd create --from-template bug "Auth token validation fails"
...
# Use custom template
$ fbd template create security-audit
$ fbd create --from-template security-audit "Review authentication flow"
```
...
# Now 'fbd create --from-template bug' uses your custom template
```
...
- [fbd create](create.md) - Create issues
- [fbd list](list.md) - List issues
