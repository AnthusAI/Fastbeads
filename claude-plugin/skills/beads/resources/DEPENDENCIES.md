...
Deep dive into fbd's four dependency types: blocks, related, parent-child, and discovered-from.

## Contents

- [Overview](#overview) - Four types at a glance, which affect fbd ready?
...
fbd supports four dependency types that serve different purposes in organizing and tracking work:

| Type | Purpose | Affects `fbd ready`? | Common Use |
|------|---------|---------------------|------------|
...
**Effect**: Issue B disappears from `fbd ready` until issue A is closed.
...
Effect: api-endpoint-2 won't show in fbd ready until db-schema-1 closed
```
...
Effect: fbd ready shows only migrate-1; closing it reveals migrate-2, etc.
```
...
```bash
fbd dep add prerequisite-issue blocked-issue
# or explicitly:
fbd dep add prerequisite-issue blocked-issue --type blocks
```
...
fbd ready shows only current step.
```
...
```
1. Close db-schema-1
2. fbd automatically updates: api-endpoint-2 is now ready
3. fbd ready shows api-endpoint-2
4. No manual unblocking needed
```

This is why `blocks` is powerful - fbd maintains ready state automatically.
...
**Effect**: No impact on `fbd ready`. Pure informational link.
...
Effect: Both show in fbd ready; choosing one doesn't block the other
```

### Creating related Dependencies

```bash
fbd dep add issue-1 issue-2 --type related
```
...
**Effect**: No impact on `fbd ready`. Creates hierarchical structure.
...
Effect: Hierarchical structure; all show in fbd ready (unless blocked)
```
...
```bash
fbd dep add child-task-id parent-epic-id --type parent-child
```
...
All children show in fbd ready immediately.
...
fbd ready shows only first child.
...
**Effect**: No impact on `fbd ready`. Tracks origin and provides context.
...
```bash
fbd dep add original-work-id discovered-issue-id --type discovered-from
```
...
**Problem**: No parallel work possible; `fbd ready` shows only one issue.
...
**Wrong**:
```bash
fbd dep add api-endpoint database-schema
...
**Right**:
```bash
fbd dep add database-schema api-endpoint
...
When you run `fbd show issue-id` on an issue, you see:
