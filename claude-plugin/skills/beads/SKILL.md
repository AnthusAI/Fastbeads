...
| fbd (persistent) | TodoWrite (ephemeral) |
|-----------------|----------------------|
...
**Decision test**: "Will I need this context in 2 weeks?" → YES = fbd
...
```bash
fbd --version  # Requires v0.47.0+
```

- **fbd CLI** installed and in PATH
- **Git repository** (fbd requires git for sync)
- **Initialization**: `fbd init` run once (humans do this, not agents)

## CLI Reference

**Run `fbd prime`** for AI-optimized workflow context (auto-loaded by hooks).
**Run `fbd <command> --help`** for specific command usage.

Essential commands: `fbd ready`, `fbd create`, `fbd show`, `fbd update`, `fbd close`, `fbd sync`

## Session Protocol

1. `fbd ready` — Find unblocked work
2. `fbd show <id>` — Get full context
3. `fbd update <id> --status in_progress` — Start work
4. Add notes as you work (critical for compaction survival)
5. `fbd close <id> --reason "..."` — Complete task
6. `fbd sync` — Persist to git (always run at session end)

## Advanced Features

| Feature | CLI | Resource |
|---------|-----|----------|
| Molecules (templates) | `fbd mol --help` | [MOLECULES.md](resources/MOLECULES.md) |
| Chemistry (pour/wisp) | `fbd pour`, `fbd wisp` | [CHEMISTRY_PATTERNS.md](resources/CHEMISTRY_PATTERNS.md) |
| Agent beads | `fbd agent --help` | [AGENTS.md](resources/AGENTS.md) |
| Async gates | `fbd gate --help` | [ASYNC_GATES.md](resources/ASYNC_GATES.md) |
| Worktrees | `fbd worktree --help` | [WORKTREES.md](resources/WORKTREES.md) |

## Resources

| Resource | Content |
|----------|---------|
| [BOUNDARIES.md](resources/BOUNDARIES.md) | fbd vs TodoWrite detailed comparison |
...
- **fbd prime**: AI-optimized workflow context
- **GitHub**: [github.com/steveyegge/fastbeads](https://github.com/steveyegge/fastbeads)
