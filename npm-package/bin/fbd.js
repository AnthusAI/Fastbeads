...
let binaryName = 'fbd';
  ...
  if (!fs.existsSync(binaryPath)) {
    console.error(`Error: fbd binary not found at ${binaryPath}`);
...
function main() {
  ...
  // Spawn the native fbd binary with all command-line arguments
  ...
  child.on('error', (err) => {
    console.error(`Error executing fbd binary: ${err.message}`);
    process.exit(1);
  });
