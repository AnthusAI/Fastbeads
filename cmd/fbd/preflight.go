...
Examples:
  fbd preflight              # Show checklist
  fbd preflight --check      # Run checks automatically
  fbd preflight --check --json  # JSON output for programmatic use
`,
	Run: runPreflight,
}
...
func runPreflight(cmd *cobra.Command, args []string) {
	...
	fmt.Println()
	fmt.Println("Run 'fbd preflight --check' to validate automatically.")
}
...
func runVersionSyncCheck() CheckResult {
	command := "Compare cmd/fbd/version.go and default.nix"

	// Read version.go
	versionGoContent, err := os.ReadFile("cmd/fbd/version.go")
	if err != nil {
		return CheckResult{
			Name:    "Version sync",
			Passed:  false,
			Skipped: true,
			Output:  fmt.Sprintf("Cannot read cmd/fbd/version.go: %v", err),
			Command: command,
		}
	}
