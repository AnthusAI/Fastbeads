...
func TestCheckDatabaseVersionJSONLMode(t *testing.T) {
	...
	// Without this, doctor treats it as a fresh clone needing 'fbd init' (bd-4ew)
...
func TestCheckDatabaseVersionFreshClone(t *testing.T) {
	...
	// This simulates a fresh clone that needs 'fbd init'
	tmpDir := t.TempDir()
	beadsDir := filepath.Join(tmpDir, ".beads")
	if err := os.Mkdir(beadsDir, 0750); err != nil {
		t.Fatal(err)
	...
	if check.Fix == "" {
		t.Error("Expected fix field to recommend 'fbd init'")
	}
}
...
func TestCheckSyncBranchHookCompatibility(t *testing.T) {
	tests := []struct {
		...
		hasGitDir      bool
		hookVersion    string // Empty means no hook, "custom" means non-fbd hook
		expectedStatus string
	}{
		{
