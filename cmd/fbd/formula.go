...
import (
	...
	"github.com/steveyegge/fastbeads/internal/formula"
...
The Rig → Cook → Run lifecycle:
  ...
  - Cook: Transform to proto (fbd cook expands macros, applies aspects)
...
Examples:
  ...
  fbd formula list --type workflow
  fbd formula list --type aspect`,
	Run: runFormulaList,
}
...
Examples:
  fbd formula show shiny
  fbd formula show rule-of-five
  fbd formula show security-audit --json`,
...
Examples:
  fbd formula convert shiny              # Convert shiny.formula.json to .toml
  fbd formula convert ./my.formula.json  # Convert specific file
  fbd formula convert --all              # Convert all JSON formulas
  fbd formula convert shiny --delete     # Convert and remove JSON file
  fbd formula convert shiny --stdout     # Print TOML to stdout`,
...
func runFormulaConvert(cmd *cobra.Command, args []string) {
	...
	if len(args) == 0 {
		...
		fmt.Fprintf(os.Stderr, "Usage: fbd formula convert <name|path> [--all]\n")
		os.Exit(1)
	}
