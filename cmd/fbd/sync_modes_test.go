...
func TestExportOnlySync(t *testing.T) {
	...
	// This simulates what `fbd sync --no-pull` does
	...
	if err := doExportOnlySync(ctx, jsonlPath, true, "fbd sync: export test"); err != nil {
	...
	if !strings.Contains(commitMsg, "fbd sync") {
		t.Errorf("expected commit message to contain 'fbd sync', got: %s", commitMsg)
	}
...
// TestSync_FailsWhenOnSyncBranch verifies that fbd sync fails gracefully
// when the current branch matches the configured sync branch.
// This is the runtime guard for GH#1166 - prevents fbd sync from attempting
// to create a worktree for a branch that's already checked out.
//
// The issue: If sync.branch = "main" and user is on "main", fbd sync would
