...
Performance Mode (--perf):
  ...
  - Times key operations (fbd ready, fbd list, fbd show, etc.)
...
Examples:
  fbd doctor              # Check current directory
  fbd doctor /path/to/repo # Check specific repository
  fbd doctor --json       # Machine-readable output
  fbd doctor --fix        # Automatically fix issues (with confirmation)
  fbd doctor --fix --yes  # Automatically fix issues (no confirmation)
  fbd doctor --fix -i     # Confirm each fix individually
  fbd doctor --fix --fix-child-parent  # Also fix child→parent deps (opt-in)
  fbd doctor --fix --force # Force repair even when database can't be opened
  fbd doctor --fix --source=jsonl # Rebuild database from JSONL (source of truth)
  fbd doctor --dry-run    # Preview what --fix would do without making changes
  fbd doctor --perf       # Performance diagnostics
  fbd doctor --output diagnostics.json  # Export diagnostics to file
  fbd doctor --check=pollution          # Show potential test issues
  fbd doctor --check=pollution --clean  # Delete test issues (with confirmation)
  fbd doctor --check=validate         # Data-integrity checks only
  fbd doctor --check=validate --fix   # Auto-fix data-integrity issues
  fbd doctor --deep             # Full graph integrity validation
  fbd doctor --server           # Dolt server mode health checks
  fbd doctor --migration=pre    # Validate readiness for Dolt migration
  fbd doctor --migration=post   # Validate Dolt migration completed
  fbd doctor --migration=pre --json  # Machine-parseable migration validation`,
...
func runDiagnostics(path string) doctorResult {
	...
	// Check 11a: fbd in PATH (needed for Claude hooks to work)
	...
	// Don't fail overall check for missing fbd in PATH, just warn

	// Check 11b: Documentation fbd prime references match installed version
	...
	// Check 21: Merge artifacts (from fbd clean)
	...
	// Check 22: Orphaned dependencies (from fbd repair-deps, fbd validate)
	...
	// Check 23: Duplicate issues (from fbd validate)
	...
	// Check 24: Test pollution (from fbd validate)
	...
	// Check 25: Git conflicts in JSONL (from fbd validate)
