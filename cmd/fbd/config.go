...
Custom Status States:
  ...
  Example:
    fbd config set status.custom "awaiting_review,awaiting_testing,awaiting_docs"
...
var configSetCmd = &cobra.Command{
	...
	Run: func(_ *cobra.Command, args []string) {
		...
		// fbd doctor reads it from git config, so we write there for consistency.
		if key == "beads.role" {
...
// This addresses the confusion when `fbd config list` shows one value but the effective
...
func showConfigYAMLOverrides(dbConfig map[string]string) {
	...
	// Check sync.branch - can be overridden by FBD_SYNC_BRANCH/BEADS_SYNC_BRANCH env var or config.yaml sync-branch
...
Examples:
  ...
  fbd config validate --json`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("\nRun 'fbd config set <key> <value>' to fix configuration issues.")
		os.Exit(1)
	},
}
