...
func importFromJSONL(ctx context.Context, jsonlPath string, renameOnImport bool, opts ...bool) error {
	// Get current executable path to avoid "./fbd" path issues
	...
	// Guardrail: single-process backends (e.g., Dolt) must not spawn a helper `fbd import`
	...
	// Run import command
	cmd := exec.CommandContext(ctx, exe, args...) // #nosec G204 - fbd import command from trusted binary
...
func importFromJSONLInline(ctx context.Context, jsonlPath string, renameOnImport bool, _ /* noGitHistory */ bool, protectLeftSnapshot bool) error {
	...
	// Match `fbd import --protect-left-snapshot` behavior.
	if protectLeftSnapshot {
...
// See: https://github.com/steveyegge/fastbeads/issues/417
