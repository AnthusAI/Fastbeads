...
Examples:
  fbd federation sync                      # Sync with all peers
  fbd federation sync --peer town-beta     # Sync with specific peer
  fbd federation sync --strategy theirs    # Auto-resolve using remote values`,
...
Examples:
  fbd federation status                    # Status for all peers
  fbd federation status --peer town-beta   # Status for specific peer`,
...
Examples:
  fbd federation add-peer town-beta dolthub://acme/town-beta-beads
  fbd federation add-peer town-gamma 192.168.1.100:3306/beads --user sync-bot
  fbd federation add-peer partner https://partner.example.com/beads --user admin --password secret`,
...
func runFederationSync(cmd *cobra.Command, args []string) {
	...
	if len(peers) == 0 {
		FatalErrorRespectJSON("no federation peers configured (use 'fbd federation add-peer' to add peers)")
	}
