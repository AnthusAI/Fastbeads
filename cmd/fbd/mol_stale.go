...
Examples:
  fbd mol stale              # List all stale molecules
  fbd mol stale --json       # Machine-readable output
  fbd mol stale --blocking   # Only show those blocking other work
  fbd mol stale --unassigned # Only show unassigned molecules
  fbd mol stale --all        # Include molecules with 0 children`,
	Run: runMolStale,
}
...
func runMolStale(cmd *cobra.Command, args []string) {
	...
	for _, mol := range result.StaleMolecules {
		...
		if mol.BlockingCount > 0 {
			...
			fmt.Printf("       → Close with: fbd close %s\n", mol.ID)
