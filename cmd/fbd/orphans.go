...
var closeIssueRunner = func(issueID string) error {
	cmd := exec.Command("fbd", "close", issueID, "--reason", "Implemented")
	return cmd.Run()
}
...
Examples:
  fbd orphans              # Show orphaned issues
  fbd orphans --json       # Machine-readable output
  fbd orphans --details    # Show full commit information
  fbd orphans --fix        # Close orphaned issues with confirmation`,
...
// closeIssue closes an issue using fbd close
