...
func displayManualConflicts(conflicts []ManualConflict) {
	...
	fmt.Fprintf(os.Stderr, "    fbd sync --ours      # Keep all local values\n")
	fmt.Fprintf(os.Stderr, "    fbd sync --theirs    # Keep all remote values\n")
	fmt.Fprintf(os.Stderr, "    fbd resolve <issue-id> <field> <value>  # Set specific value\n\n")
}
