...
func TestDoltDoctor_NoSQLiteWarningsAfterInitAndCreate(t *testing.T) {
	...
	socketPath := filepath.Join(tmpDir, ".beads", "fbd.sock")
	env := []string{
		"BEADS_TEST_MODE=1",
	...
	for _, forbidden := range []string{
		...
		if strings.Contains(doctorOut, forbidden) {
			t.Fatalf("fbd doctor printed sqlite-specific warning %q in dolt mode; output:\n%s", forbidden, doctorOut)
		}
	}
	...
	if !strings.Contains(strings.ToLower(doctorOut), "dolt") {
		...
		t.Fatalf("fbd doctor output did not mention dolt; output:\n%s", doctorOut)
	}
