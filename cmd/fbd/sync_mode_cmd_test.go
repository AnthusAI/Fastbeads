...
// TestSyncModeListText tests `fbd sync mode list` text output.
func TestSyncModeListText(t *testing.T) {
	...
	// Verify usage hint
	if !strings.Contains(output, "fbd sync mode set") {
		t.Error("output should contain usage hint")
	}
}

// TestSyncModeListJSON tests `fbd sync mode list --json` output.
...
// TestSyncModeCurrentWithStore tests `fbd sync mode current` with a database.
...
// TestSyncModeCurrentWithoutStore tests `fbd sync mode current` without a database.
...
// TestSyncModeSetValidModes tests `fbd sync mode set` with valid modes.
...
// TestSyncModeSetInvalidMode tests `fbd sync mode set` with an invalid mode.
...
// TestSyncModeSetReadonly tests that `fbd sync mode set` fails in readonly mode.
