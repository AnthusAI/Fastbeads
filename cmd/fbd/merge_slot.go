...
Examples:
  fbd merge-slot create              # Create merge slot for current rig
  fbd merge-slot check               # Check if slot is available
  fbd merge-slot acquire             # Try to acquire the slot
  fbd merge-slot release             # Release the slot
  fbd merge-slot wait                # Wait for slot to become available`,
}
...
Use --holder to specify who is acquiring (default: FBD_ACTOR/BD_ACTOR/BEADS_ACTOR env var).`,
	Args: cobra.NoArgs,
...
func init() {
	mergeSlotAcquireCmd.Flags().StringVar(&mergeSlotHolder, "holder", "", "Who is acquiring the slot (default: FBD_ACTOR/BD_ACTOR/BEADS_ACTOR)")
...
func getMergeSlotID() string {
	// Use the prefix from beads config (default "fbd")
	prefix := "fbd"
...
func runMergeSlotCheck(cmd *cobra.Command, args []string) error {
	...
	if err != nil || slot == nil {
		...
		fmt.Printf("Run 'fbd merge-slot create' to create one.\n")
		return nil
	}
...
func runMergeSlotAcquire(cmd *cobra.Command, args []string) error {
	...
	if holder == "" {
		return fmt.Errorf("no holder specified; use --holder or set FBD_ACTOR/BD_ACTOR/BEADS_ACTOR env var")
	}
	...
	if err != nil {
		return fmt.Errorf("merge slot not found: %s (run 'fbd merge-slot create' first)", slotID)
	}
