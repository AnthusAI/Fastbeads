...
Examples:
  fbd sql 'SELECT COUNT(*) FROM issues'
  fbd sql 'SELECT id, title FROM issues WHERE status = "open" LIMIT 5'
  fbd sql 'DELETE FROM dirty_issues WHERE issue_id = "bd-abc123"'
  fbd sql --csv 'SELECT id, title, status FROM issues'
