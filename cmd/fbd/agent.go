...
Examples:
  fbd agent state gt-emma running     # Set emma's state to running
  fbd agent heartbeat gt-emma         # Update emma's last_activity timestamp
  fbd agent show gt-emma              # Show emma's agent details`,
}
...
Examples:
  fbd agent state gt-emma running   # Set state to running
  fbd agent state gt-mayor idle     # Set state to idle`,
...
Examples:
  fbd agent heartbeat gt-emma   # Update emma's last_activity
  fbd agent heartbeat gt-mayor  # Update mayor's last_activity`,
...
Examples:
  fbd agent show gt-emma   # Show emma's agent details
  fbd agent show gt-mayor  # Show mayor's agent details`,
	Args: cobra.ExactArgs(1),
	RunE: runAgentShow,
}
...
This enables queries like:
  fbd list --type=agent --label=role_type:witness
  fbd list --type=agent --label=rig:gastown

Use --dry-run to see what would be changed without making changes.

Examples:
  fbd agent backfill-labels           # Backfill all agent beads
  fbd agent backfill-labels --dry-run # Preview changes without applying`,
