...
EXAMPLES:
  fbd rename-prefix kw-                # Rename from 'knowledge-work-' to 'kw-'
  fbd rename-prefix mtg- --repair      # Consolidate multiple prefixes into 'mtg-'
  fbd rename-prefix team- --dry-run    # Preview changes without applying
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(prefixes) > 1 {
			...
			if !repair {
				...
				fmt.Fprintf(os.Stderr, "Example: fbd rename-prefix %s --repair\n", newPrefix)
				os.Exit(1)
			}
		...
		if jsonlPath != "" {
			...
			if err != nil {
				...
				if _, err := writeJSONLAtomic(jsonlPath, renamedIssues); err != nil {
					...
					fmt.Fprintf(os.Stderr, "Run 'fbd export --force' to update JSONL\n")
				} else {
