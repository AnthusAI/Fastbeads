...
import (
	...
	"github.com/steveyegge/fastbeads/cmd/fbd/setup"
	"github.com/steveyegge/fastbeads/internal/recipes"
)
...
Examples:
  fbd setup cursor          # Install Cursor IDE integration
  fbd setup --list          # Show all available recipes
  fbd setup --print         # Print the template to stdout
  fbd setup -o rules.md     # Write template to custom path
  fbd setup --add myeditor .myeditor/rules.md  # Add custom recipe

Use 'fbd setup <recipe> --check' to verify installation status.
Use 'fbd setup <recipe> --remove' to uninstall.`,
...
func runSetup(cmd *cobra.Command, args []string) {
	...
	if setupAdd != "" {
		if len(args) != 1 {
			...
			fmt.Fprintln(os.Stderr, "Usage: fbd setup --add <name> <path>")
			os.Exit(1)
		}
...
func listRecipes() {
	...
	fmt.Println()
	fmt.Println("Use 'fbd setup <recipe>' to install.")
	fmt.Println("Use 'fbd setup --add <name> <path>' to add a custom recipe.")
}
...
func addRecipe(name, path string) error {
	...
	fmt.Println()
	fmt.Printf("Install with: fbd setup %s\n", name)
	return nil
}
...
func runRecipe(name string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		fmt.Fprintln(os.Stderr, "Use 'fbd setup --list' to see available recipes.")
		os.Exit(1)
	}
