...
// Guardrail: ensure the cmd/fbd test suite does not touch the real repo .beads state.
...
func stopRepoDaemon(repoRoot string) {
	...
	socketPath := filepath.Join(beadsDir, "fbd.sock")
	...
	// Shell out to fbd daemon stop. We can't call the daemon functions directly
	// from TestMain because they have complex dependencies. Using exec is cleaner.
	cmd := exec.Command("fbd", "daemon", "stop")
	cmd.Dir = repoRoot
