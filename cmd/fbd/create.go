...
import (
	...
	"github.com/steveyegge/fastbeads/internal/validation"
)
...
var createCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if strings.HasPrefix(strings.ToLower(title), "test") && !silent && !debug.IsQuiet() {
			...
			fmt.Fprintf(os.Stderr, "  For testing, consider using: BEADS_DB=/tmp/test.db ./fbd create \"Test issue\"\n")
		}
		...
		if deferStr != "" {
			...
			if t.Before(time.Now()) && !silent && !debug.IsQuiet() {
				fmt.Fprintf(os.Stderr, "%s Defer date %q is in the past. Issue will appear in fbd ready immediately.\n",
		...
		// so the issue appears in fbd list when hydration is enabled (bd-fix-routing)
		if repoPath != "." {
...
func init() {
	...
	createCmd.Flags().String("prefix", "", "Create issue in rig by prefix (e.g., --prefix bd- or --prefix fbd or --prefix beads)")
	...
	//   --defer=+1h         Hidden from fbd ready for 1 hour
	...
	createCmd.Flags().String("defer", "", "Defer until date (issue hidden from fbd ready until then). Same formats as --due")
