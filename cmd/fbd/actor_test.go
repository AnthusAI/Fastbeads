...
func TestGetActorWithGit(t *testing.T) {
	...
	origActor := actor
	origFbdActor, fbdActorSet := os.LookupEnv("FBD_ACTOR")
	...
	defer func() {
		actor = origActor
		if fbdActorSet {
			os.Setenv("FBD_ACTOR", origFbdActor)
		} else {
			os.Unsetenv("FBD_ACTOR")
		}
		if bdActorSet {
	...
	tests := []struct {
		...
		{
			name:       "FBD_ACTOR takes priority when no flag",
			actorFlag:  "",
			fbdActor:   "fbd-actor",
			bdActor:    "bd-actor",
			beadsActor: "beads-actor",
		...
		{
			name:       "BD_ACTOR takes priority when no FBD_ACTOR",
			actorFlag:  "",
			fbdActor:   "",
			bdActor:    "bd-actor",
			beadsActor: "beads-actor",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (tt.expected == tt.user || tt.expected == "unknown") && gitUserName != "" && tt.fbdActor == "" && tt.bdActor == "" && tt.beadsActor == "" && tt.actorFlag == "" {
			...
			if tt.fbdActor != "" {
				os.Setenv("FBD_ACTOR", tt.fbdActor)
			} else {
				os.Unsetenv("FBD_ACTOR")
			}
...
func TestGetActorWithGit_PriorityOrder(t *testing.T) {
	...
	// Test: flag > FBD_ACTOR > BD_ACTOR > BEADS_ACTOR
	actor = "from-flag"
	os.Setenv("FBD_ACTOR", "from-fbd-actor")
	...
	// Test: FBD_ACTOR > BD_ACTOR > BEADS_ACTOR (no flag)
	actor = ""
	result = getActorWithGit()
	if result != "from-fbd-actor" {
		t.Errorf("Expected FBD_ACTOR to take priority over BD_ACTOR/BEADS_ACTOR, got %q", result)
	}

	// Test: BD_ACTOR when FBD_ACTOR is empty
	os.Unsetenv("FBD_ACTOR")
	result = getActorWithGit()
	...
	// Test: BEADS_ACTOR when FBD_ACTOR and BD_ACTOR are empty
	os.Unsetenv("BD_ACTOR")
	result = getActorWithGit()
