...
Examples:
  fbd cook mol-feature.formula.json                    # Compile-time: keep {{vars}}
  fbd cook mol-feature --var name=auth                 # Runtime: substitute vars
  fbd cook mol-feature --mode=runtime --var name=auth  # Explicit runtime mode
  fbd cook mol-feature --dry-run                       # Preview steps
  fbd cook mol-release.formula.json --persist          # Write to database
  fbd cook mol-release.formula.json --persist --force  # Replace existing
...
func persistCookFormula(ctx context.Context, resolved *formula.Formula, protoID string, force bool, vars, bondPoints []string) error {
	...
	fmt.Printf("\nTo use: fbd mol pour %s --var <name>=<value>\n", result.ProtoID)
	return nil
}
