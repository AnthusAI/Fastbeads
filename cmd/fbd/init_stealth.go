...
func setupStealthMode(verbose bool) error {
	...
	if verbose {
		...
		fmt.Printf("  Claude settings: %s\n\n", ui.RenderAccent("configured with fbd onboard instruction"))
...
func setupGitExclude(verbose bool) error {
	...
	if !hasBeads || !hasClaude {
		newContent += "\n# Beads stealth mode (added by fbd init --stealth)\n"
	}
...
func setupForkExclude(verbose bool) error {
	...
	newContent += "\n# Beads fork protection (fbd init)\n"
	for _, p := range toAdd {
		newContent += p + "\n"
	}
...
func setupGlobalGitIgnore(homeDir string, projectPath string, verbose bool) error {
	...
	if !hasBeads || !hasClaude {
		newContent += fmt.Sprintf("\n# Beads stealth mode: %s (added by fbd init --stealth)\n", projectPath)
	}
