...
Commands:
  fbd sync              Export to JSONL (prep for push)
  fbd sync --import     Import from JSONL (after pull)
  fbd sync --status     Show sync state
  fbd sync --resolve    Resolve conflicts (uses configured strategy)
  fbd sync --force      Force full export/import (skip incremental)
  fbd sync --full       Full sync: pull → merge → export → commit → push (legacy)

Conflict Resolution:
  fbd sync --resolve              Use configured conflict.strategy
  fbd sync --resolve --ours       Keep local versions
  fbd sync --resolve --theirs     Keep remote versions
  fbd sync --resolve --manual     Interactive resolution with prompts
...
The --full flag provides the legacy full sync behavior for backwards compatibility.`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		// errors in post-checkout hook's `fbd sync --import-only`.
		...
		if jsonlPath == "" {
			FatalError("not in a fbd workspace (no .beads directory found)")
		}
		...
		if squash {
			if dryRun {
				...
				fmt.Println("  Run 'fbd sync' (without --squash) to commit all accumulated changes")
			}
			return
		}
		...
		if inMerge, err := gitHasUnmergedPaths(); err != nil {
			...
			FatalErrorWithHint("unmerged paths or merge in progress", "resolve conflicts, run 'fbd import' if needed, then 'fbd sync' again")
		}
...
func doPullFirstSync(ctx context.Context, jsonlPath string, renameOnImport, noGitHistory, dryRun, noPush, noPull bool, message string, acceptRebase bool, sbc *SyncBranchContext) error {
	...
	if fbd := beads.FindBeadsDir(); fbd != "" {
		_ = ClearSyncState(fbd)
	}
...
func doExportSync(ctx context.Context, jsonlPath string, force, dryRun bool) error {
	...
	if shouldUseDolt {
		...
		if !ok {
			...
			if err := rs.Commit(ctx, "fbd sync: auto-commit"); err != nil {
...
func init() {
	...
	syncCmd.Flags().Bool("squash", false, "Accumulate changes in JSONL without committing (run 'fbd sync' later to commit all)")
