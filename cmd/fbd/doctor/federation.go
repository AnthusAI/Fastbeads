...
func CheckFederationRemotesAPI(path string) DoctorCheck {
	...
	if serverPID == 0 {
		...
		return DoctorCheck{
			...
			Fix:      "Run 'fbd daemon start --federation' to enable peer-to-peer sync",
			Category: CategoryFederation,
		}
	}
...
func CheckFederationSyncStaleness(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd federation sync' to synchronize with peers",
		Category: CategoryFederation,
	}
}
...
func CheckFederationConflicts(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd federation sync --strategy ours|theirs' to resolve conflicts",
		Category: CategoryFederation,
	}
}
...
func CheckDoltServerModeMismatch(path string) DoctorCheck {
	...
	if err != nil {
		...
		if _, lockErr := os.Stat(lockFile); lockErr == nil && serverPID == 0 {
			return DoctorCheck{
				...
				Fix:      "Close other fbd processes or start daemon with --federation",
				Category: CategoryFederation,
			}
		}
		return DoctorCheck{
	...
	if peerCount > 0 && serverPID == 0 {
		return DoctorCheck{
			...
			Fix:      "Run 'fbd daemon start --federation' to enable server mode",
			Category: CategoryFederation,
		}
	}
