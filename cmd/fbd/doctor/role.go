...
// database config for users who ran "fbd config set role maintainer"
// (without the "beads." prefix) or "fbd config set beads.role maintainer"
...
func CheckBeadsRole(path string) DoctorCheck {
	...
	// Users may have set it via "fbd config set role maintainer" (stored in SQLite)
	...
	return DoctorCheck{
		...
		Detail:   "Run 'fbd init' to configure your role (maintainer or contributor).",
		Fix:      "fbd config set beads.role maintainer",
...
func validateRole(role string) DoctorCheck {
	if role != "maintainer" && role != "contributor" {
		return DoctorCheck{
			...
			Detail:   "Valid values are 'maintainer' or 'contributor'. Run 'fbd init' to reconfigure.",
			Fix:      "fbd init",
			Category: CategoryData,
		}
	}
