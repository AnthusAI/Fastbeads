...
// This prevents fork bombs when tests call functions that execute fbd subcommands.
var ErrTestBinary = fmt.Errorf("running as test binary - cannot execute fbd subcommands")
...
// getBdBinary returns the path to the fbd binary to use for fix operations.
...
func getBdBinary() (string, error) {
	...
	if err == nil {
		...
		// when tests call functions that execute fbd subcommands
	...
	bdPath, err := exec.LookPath("fbd")
	if err != nil {
		return "", fmt.Errorf("fbd binary not found in PATH: %w", err)
	}
