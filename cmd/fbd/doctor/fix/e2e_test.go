...
// E2E tests that need to execute 'fbd' subcommands cannot run in test mode.
...
func TestGitHooks_E2E(t *testing.T) {
	// Skip if fbd binary not available or running as test binary
	skipIfTestBinary(t)
	if _, err := exec.LookPath("fbd"); err != nil {
		t.Skip("fbd binary not in PATH, skipping e2e test")
	}
	...
	t.Run("installs hooks in git repo", func(t *testing.T) {
		...
		// Check hook content has fbd reference
		content, err := os.ReadFile(preCommit)
		if err != nil {
			t.Fatalf("failed to read hook: %v", err)
		}
		if !strings.Contains(string(content), "fbd") {
			t.Error("hook doesn't contain fbd reference")
		}
	})
}
...
func TestMergeDriver_E2E(t *testing.T) {
	...
	t.Run("fixes incorrect config", func(t *testing.T) {
		...
		runGit(t, dir, "config", "merge.beads.driver", "fbd merge %L %O %A %R")
...
func TestGetBdBinary_Errors(t *testing.T) {
	t.Run("returns current executable when available", func(t *testing.T) {
		...
		if err != nil {
			// This is expected in test environment if fbd isn't the test binary
...
// TestGitHooksWithExistingHooks_E2E tests preserving existing non-fbd hooks
func TestGitHooksWithExistingHooks_E2E(t *testing.T) {
	...
	t.Run("preserves existing non-fbd hooks", func(t *testing.T) {
		dir := setupTestGitRepo(t)
		...
		// Verify fbd hook was installed (should contain fbd reference)
		if !strings.Contains(hookContent, "fbd") {
			t.Error("hook should contain fbd reference after installation")
		}

		// Note: The exact preservation behavior depends on 'fbd hooks install' implementation
