...
func TestCheckLefthookBdIntegration(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "yaml with fbd hooks run",
			configFile: "lefthook.yml",
			configContent: `
pre-commit:
  commands:
    fbd:
      run: fbd hooks run pre-commit
post-merge:
  commands:
    fbd:
      run: fbd hooks run post-merge
pre-push:
  commands:
    fbd:
      run: fbd hooks run pre-push
`,
			expectConfigured:  true,
			expectHooksWithBd: []string{"pre-commit", "post-merge", "pre-push"},
		},
		{
			name:       "yaml with partial fbd integration",
			configFile: "lefthook.yml",
			configContent: `
pre-commit:
  commands:
...
`,
		...
		{
			name:       "toml with fbd hooks run",
			configFile: "lefthook.toml",
			configContent: `
[pre-commit.commands.fbd]
run = "fbd hooks run pre-commit"

[post-merge.commands.fbd]
run = "fbd hooks run post-merge"

[pre-push.commands.fbd]
run = "fbd hooks run pre-push"
`,
			expectConfigured:  true,
			expectHooksWithBd: []string{"pre-commit", "post-merge", "pre-push"},
...
`,
		...
		{
			name: "mixed fbd and other hooks",
			configContent: `repos:
  - repo: https://github.com/pre-commit/pre-commit-hooks
    rev: v4.5.0
    hooks:
...
func TestBdHookPatternMatching(t *testing.T) {
	tests := []struct {
		...
		{"fbd hooks run pre-commit", true},
		{"fbd  hooks  run pre-commit", true},
		{"fbd hooks run post-merge", true},
		{`fbd hooks run pre-push "$@"`, true},
...
func TestHasBdInCommands(t *testing.T) {
	tests := []struct {
		...
		{
			name: "fbd hooks run in commands",
			section: map[string]interface{}{
				"commands": map[string]interface{}{
					"fbd": map[string]interface{}{
						"run": "fbd hooks run pre-commit",
					},
				},
			},
		...
		{
			name: "fbd mentioned in comment not run field",
			section: map[string]interface{}{
				"commands": map[string]interface{}{
					"lint": map[string]interface{}{
						"run":  "eslint .",
						"tags": "fbd hooks run should be added",
					},
				},
			},
			expected: false,
...
func TestCheckExternalHookManagerIntegration(t *testing.T) {
	tests := []struct {
		...
		{
			name: "lefthook with fbd integration",
			setup: func(dir string) error {
				config := `pre-commit:
  commands:
    fbd:
...
`
		...
		{
			name: "husky with fbd integration",
			setup: func(dir string) error {
				huskyDir := filepath.Join(dir, ".husky")
				if err := os.MkdirAll(huskyDir, 0755); err != nil {
					return err
		...
		{
			name: "pre-commit framework without fbd",
			setup: func(dir string) error {
				return os.WriteFile(filepath.Join(dir, ".pre-commit-config.yaml"), []byte("repos:\n"), 0644)
			},
			expectNil:           false,
		...
		{
			name: "pre-commit framework with fbd integration",
			setup: func(dir string) error {
				config := `repos:
  - repo: local
    hooks:
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(dir string) error {
				...
				return os.WriteFile(filepath.Join(huskyDir, "pre-commit"), []byte("fbd hooks run pre-commit"), 0755)
			},
			expectNil:     false,
			expectManager: "husky",
		},
