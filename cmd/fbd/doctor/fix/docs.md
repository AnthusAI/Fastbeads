# Noridoc: cmd/fbd/doctor/fix

Path: @/cmd/fbd/doctor/fix

### Overview

The `cmd/fbd/doctor/fix` directory contains automated remediation functions for issues detected by the `fbd doctor` command. Each module handles a specific category of issues (deletions manifest, database config, sync branch, etc.) and provides functions to automatically fix problems found in beads workspaces.

### How it fits into the larger codebase

- **Integration with Doctor Detection**: The `@/cmd/fbd/doctor.go` command runs checks to identify workspace problems, then calls functions from this package when `--fix` flag is used. The `CheckDatabaseJSONLSync()` function in `@/cmd/fbd/doctor/database.go` (lines 299-486) detects sync issues and provides direction-specific guidance about which fix to run. When DB count differs from JSONL count, it now recommends `fbd doctor --fix` to run `DBJSONLSync()` with the appropriate direction.
...
- **Data Persistence Points**: Each fix module directly modifies persistent workspace state: deletions manifest, database files, JSONL files, and git branch configuration. Changes are written to disk and persisted in the git repository. The sync fix is unique in that it delegates persistence to `fbd export` or `fbd sync --import-only` commands.

- **Deletion Tracking Architecture**: The deletions manifest (`@/internal/deletions/deletions.go`) is an append-only log tracking issue deletions. The fix in `deletions.go` is critical to maintaining the integrity of this log by preventing tombstones from being incorrectly re-added to it after `fbd migrate-tombstones` runs.
...
1. **Bidirectional Detection** (lines 23-97):
   ...
   - Determines sync direction based on issue counts:
     - If `dbCount > jsonlCount`: DB has newer data → runs `fbd export` to sync JSONL
     - If `jsonlCount > dbCount`: JSONL has newer data → runs `fbd sync --import-only` to import
...
5. **Command Execution** (lines 106-120):
   - Gets fbd binary path safely via `getBdBinary()` to prevent fork bombs in tests
   - Executes `fbd export` or `fbd sync --import-only` with workspace directory as working directory
   - Streams stdout/stderr to user for visibility

**Problem Solved (bd-68e4)**:

Previously, when the database contained more issues than the JSONL export, the doctor would recommend `fbd sync --import-only`, which imports JSONL into DB. Since JSONL hadn't changed and the database had newer data, this command was a no-op (0 created, 0 updated), leaving users unable to sync their JSONL file with the database. The bidirectional detection now recognizes this case and runs `fbd export` instead.
...
1. **HydrateDeletionsManifest()** (lines 16-96):
   - Entry point called by `fbd doctor --fix` when "Deletions Manifest" issue is detected
...
The bug occurred because `fbd migrate-tombstones` converts deletion records from the legacy `deletions.jsonl` file into inline tombstone entries in `issues.jsonl`. Without the fix, the sequence would be:

1. User runs `fbd migrate-tombstones` → creates tombstones in JSONL with `status: "tombstone"`
2. User runs `fbd sync` → triggers `fbd doctor hydrate`
...
The `@/cmd/fbd/migrate_tombstones.go` command creates tombstones using `convertDeletionRecordToTombstone()` (lines 268-284). These tombstones have `Status == types.StatusTombstone`. The fix works because migrate-tombstones sets this status correctly (verified by `TestMigrateTombstones_TombstonesAreValid()` in migrate_tombstones_test.go).
