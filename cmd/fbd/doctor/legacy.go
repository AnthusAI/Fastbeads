...
// and recommends migration to fbd prime hooks for better token efficiency.
//
// Old pattern: /beads:quickstart, /beads:ready (~10.5k tokens per session)
// New pattern: fbd prime hooks (~50-2k tokens per session)
func CheckLegacyBeadsSlashCommands(repoPath string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix: "Migrate to fbd prime hooks for better token efficiency:\n" +
			"\n" +
			"Migration Steps:\n" +
			...
			"     - Add: \"Run 'fbd prime' for workflow context\" (for users without hooks)\n" +
			"\n" +
			"Benefits:\n" +
...
// migration to fbd prime hooks for better token efficiency.
//
// Old pattern: Document MCP tool names for direct tool calls (~10.5k tokens per scan)
// New pattern: fbd prime hooks with CLI commands (~50-2k tokens)
func CheckLegacyMCPToolReferences(repoPath string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix: "Migrate to fbd prime hooks for better token efficiency:\n" +
			...
			"     - mcp__beads_beads__list  → fbd list\n" +
			"     - mcp__beads_beads__show  → fbd show <id>\n" +
			"     - mcp__beads_beads__ready → fbd ready\n" +
			"  3. fbd prime hooks auto-inject context on session start\n" +
			"\n" +
			"Benefits:\n" +
			"  • fbd prime + hooks: ~50-2k tokens vs ~10.5k for MCP tool scan\n" +
...
// and recommends adding it if missing, suggesting fbd onboard or fbd setup claude.
...
func CheckAgentDocumentation(repoPath string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix: "Add agent documentation:\n" +
			"  • Run 'fbd onboard' to create AGENTS.md with workflow guidance\n" +
			"  • Or run 'fbd setup claude' to add Claude-specific documentation\n" +
			"\n" +
			...
			"\n" +
			"Recommended: Include fbd workflow in your project documentation so\n" +
	...
	if cfg.JSONLExport == "beads.jsonl" {
		...
		if legacyExists && !canonicalExists {
			return DoctorCheck{
				...
				Fix: "Run 'fbd doctor --fix' to auto-migrate, or manually:\n" +
		...
		if !legacyExists && canonicalExists {
			...
			return DoctorCheck{
				...
				Fix:     "Run 'fbd doctor --fix' to update the configuration",
			}
		}
	}
...
func CheckDatabaseConfig(repoPath string) DoctorCheck {
	...
	if cfg.JSONLExport != "" {
		if cfg.JSONLExport == "deletions.jsonl" || cfg.JSONLExport == "interactions.jsonl" || cfg.JSONLExport == "molecules.jsonl" {
			return DoctorCheck{
				...
				Fix:     "Run 'fbd doctor --fix' to reset metadata.json jsonl_export to issues.jsonl, then commit the change.",
			}
		}
	...
	return DoctorCheck{
		...
		Fix: "Run 'fbd doctor --fix' to auto-detect and fix mismatches, or manually:\n" +
...
// CheckFreshClone detects if this is a fresh clone that needs 'fbd init'.
...
func CheckFreshClone(repoPath string) DoctorCheck {
	...
	fixCmd := "fbd init"
	if prefix != "" {
		fixCmd = fmt.Sprintf("fbd init --prefix %s", prefix)
	}
	if backend == configfile.BackendDolt {
		fixCmd = "fbd init --backend dolt"
		if prefix != "" {
			fixCmd = fmt.Sprintf("fbd init --backend dolt --prefix %s", prefix)
		}
	}
	...
	return DoctorCheck{
		...
		Detail: "This appears to be a freshly cloned repository.\n" +
			...
			"  Run 'fbd init' to create the database and import existing issues.",
