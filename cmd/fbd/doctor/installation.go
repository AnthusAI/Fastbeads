...
func CheckInstallation(path string) DoctorCheck {
	...
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		...
		return DoctorCheck{
			...
			Fix:     fmt.Sprintf("Run 'fbd init --prefix %s' to initialize beads", prefix),
		}
	}
...
func CheckMultipleDatabases(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd migrate' to consolidate databases or manually remove old .db files",
	}
}
...
func CheckPermissions(path string) DoctorCheck {
	...
	if err := os.WriteFile(testFile, []byte("test"), 0600); err != nil {
		return DoctorCheck{
			...
			Fix:     "Run 'fbd doctor --fix' to fix permissions",
		}
	}
...
func CheckUntrackedBeadsFiles(path string) DoctorCheck {
	...
	// In sync-branch mode, JSONL files are intentionally untracked in working branches.
	// They are committed only to the dedicated sync branch via fbd sync.
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd doctor --fix' to stage and commit untracked files, or manually: git add .beads/*.jsonl && git commit",
	}
}
