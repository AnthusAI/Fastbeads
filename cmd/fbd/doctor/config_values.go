...
func CheckConfigValues(repoPath string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Edit config files to fix invalid values. Run 'fbd config' to view current settings.",
	}
}
...
func findConfigPath(repoPath string) string {
	...
	if configDir, err := os.UserConfigDir(); err == nil {
		userConfigPath := filepath.Join(configDir, "fbd", "config.yaml")
...
func checkYAMLConfigValues(repoPath string) []string {
	...
	if v.IsSet("routing.mode") {
		...
		// so routed issues are visible in fbd list via multi-repo hydration
		if mode == "auto" {
				...
				if !hasHydration {
					issues = append(issues,
						"routing.mode=auto with routing targets but repos.additional not configured. "+
							"Issues created via routing will not be visible in fbd list. "+
							"Run 'fbd repo add <routing-target>' to enable hydration.")
				} else {
						...
						if !additionalSet[expandedContributor] {
							issues = append(issues, fmt.Sprintf(
								"routing.contributor=%q is not in repos.additional. "+
									"Run 'fbd repo add %s' to make routed issues visible.",
