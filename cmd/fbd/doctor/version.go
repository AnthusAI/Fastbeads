...
// getUpgradeCommand returns the appropriate upgrade command based on how fbd was installed.
...
// command is "brew upgrade beads" — even for the legacy tap that used "fbd.rb".
func upgradeCommandForPath(execPath string) string {
	...
	// Matches both homebrew-core formula "beads" and legacy tap formula "fbd"
	if strings.Contains(lowerPath, "/cellar/beads/") ||
		strings.Contains(lowerPath, "/cellar/fbd/") {
...
// localVersionFile is the gitignored file that stores the last fbd version used locally.
...
// Version tracking uses .local_version file (gitignored) to track the last fbd version used.
//
...
func CheckMetadataVersionTracking(path string, currentVersion string) DoctorCheck {
	...
	if err != nil {
		if os.IsNotExist(err) {
			// File doesn't exist yet - will be created on next fbd command
			return DoctorCheck{
				Name:    "Version Tracking",
				Status:  StatusWarning,
				Message: "Version tracking not initialized",
				Detail:  "The .local_version file will be created on next fbd command",
				Fix:     "Run any fbd command (e.g., 'fbd ready') to initialize version tracking",
			}
		}
	...
	if lastVersion == "" {
		return DoctorCheck{
			...
			Fix:     "Run any fbd command to initialize version tracking",
		}
	}
	...
	if !IsValidSemver(lastVersion) {
		return DoctorCheck{
			...
			Fix:     "Run any fbd command to reset version tracking to current version",
		}
	}
	...
	if versionDiff > 0 {
		...
		if majorDiff >= 1 || (majorDiff == 0 && minorDiff >= 10) {
			return DoctorCheck{
				...
				Fix:     "Run 'fbd upgrade review' to see recent changes",
			}
		}
