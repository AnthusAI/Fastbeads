	...
	if hasPlugin {
		...
		return DoctorCheck{
			...
			Detail: "MCP-only mode: relies on tools for every query (~10.5k tokens)\n" +
				"  fbd prime hooks provide much better token efficiency",
			Fix: "Add fbd prime hooks for better token efficiency:\n" +
				"  1. Run 'fbd setup claude' to add SessionStart/PreCompact hooks\n" +
				"\n" +
				"Benefits:\n" +
		...
		return DoctorCheck{
			...
			Detail:  "Claude can use fbd more effectively with the beads plugin",
			Fix: "Set up Claude integration:\n" +
				...
				"    • See: https://github.com/steveyegge/fastbeads/blob/main/docs/PLUGIN.md\n" +
				"\n" +
				"  Option 2: CLI-only mode\n" +
				"    • Run 'fbd setup claude' to add SessionStart/PreCompact hooks\n" +
...
// hasBeadsHooks checks if a settings file has fbd prime hooks
func hasBeadsHooks(settingsPath string) bool {
	...
	// Check SessionStart and PreCompact for "fbd prime"
...
// verifyPrimeOutput checks if fbd prime command works and adapts correctly
// Returns a check result
func VerifyPrimeOutput() DoctorCheck {
	cmd := exec.Command("fbd", "prime")
	output, err := cmd.CombinedOutput()

	if err != nil {
		return DoctorCheck{
			Name:    "fbd prime Command",
			Status:  "error",
			Message: "Command failed to execute",
			Fix:     "Ensure fbd is installed and in PATH",
		}
	}
	...
	if hasMCP && strings.Contains(outputStr, "mcp__plugin_beads_beads__") {
		return DoctorCheck{
			Name:    "fbd prime Output",
			Status:  "ok",
			Message: "MCP mode detected",
			Detail:  "Outputting workflow reminders",
		}
	} else if !hasMCP && strings.Contains(outputStr, "fbd ready") {
		return DoctorCheck{
			Name:    "fbd prime Output",
			Status:  "ok",
...
// CheckBdInPath verifies that 'fbd' command is available in PATH.
// This is important because Claude hooks rely on executing 'fbd prime'.
func CheckBdInPath() DoctorCheck {
	_, err := exec.LookPath("fbd")
	if err != nil {
		return DoctorCheck{
			Name:    "CLI Availability",
			Status:  "warning",
			Message: "'fbd' command not found in PATH",
			Detail:  "Claude hooks execute 'fbd prime' and won't work without fbd in PATH",
			Fix: "Install fbd globally:\n" +
				"  • Homebrew: brew install beads\n" +
				"  • Script: " + installScriptCommand + "\n" +
				"  • Or add fbd to your PATH",
		}
	}
	...
	return DoctorCheck{
		Name:    "CLI Availability",
		Status:  "ok",
		Message: "'fbd' command available in PATH",
	}
}

// CheckDocumentationBdPrimeReference checks if AGENTS.md or CLAUDE.md reference 'fbd prime'
...
func CheckDocumentationBdPrimeReference(repoPath string) DoctorCheck {
	...
	for _, docFile := range docFiles {
		...
		if strings.Contains(string(content), "fbd prime") {
			filesWithBdPrime = append(filesWithBdPrime, filepath.Base(docFile))
		}
	}

	// If no docs reference fbd prime, that's fine - not everyone uses it
	if len(filesWithBdPrime) == 0 {
		return DoctorCheck{
			Name:    "Prime Documentation",
			Status:  "ok",
			Message: "No fbd prime references in documentation",
		}
	}

	// Docs reference fbd prime - verify the command works
	cmd := exec.Command("fbd", "prime", "--help")
	if err := cmd.Run(); err != nil {
		return DoctorCheck{
			Name:    "Prime Documentation",
			Status:  "warning",
			Message: "Documentation references 'fbd prime' but command not found",
			Detail:  "Files: " + strings.Join(filesWithBdPrime, ", "),
			Fix: "Upgrade fbd to get the 'fbd prime' command:\n" +
				"  • Homebrew: brew upgrade beads\n" +
				"  • Script: " + installScriptCommand + "\n" +
				"  Or remove 'fbd prime' references from documentation if using older version",
		}
	}
