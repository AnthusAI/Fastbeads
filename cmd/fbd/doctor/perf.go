...
func RunPerformanceDiagnostics(path string) {
	...
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		...
		fmt.Fprintf(os.Stderr, "Run 'fbd init' to initialize beads\n")
		os.Exit(1)
	}
	...
	if showDuration > 0 {
		fmt.Printf("  fbd show <random-issue>    %dms\n", showDuration.Milliseconds())
	}
	...
	fmt.Printf("  fbd list (complex filters) %dms\n", searchDuration.Milliseconds())
