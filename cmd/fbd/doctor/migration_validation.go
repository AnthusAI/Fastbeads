...
func CheckMigrationReadiness(path string) (DoctorCheck, MigrationValidationResult) {
	...
	if _, err := os.Stat(beadsDir); os.IsNotExist(err) {
		...
		return DoctorCheck{
			...
			Fix:      "Run 'fbd init' first to create a beads installation",
	...
	if jsonlPath == "" {
		...
		return DoctorCheck{
			...
			Fix:      "Run 'fbd export' to create JSONL file from database",
			Category: CategoryMaintenance,
		}, result
	}
	...
	if err != nil {
		...
		return DoctorCheck{
			...
			Detail:   err.Error(),
			Fix:      "Run 'fbd doctor --fix' to repair JSONL from database",
			Category: CategoryMaintenance,
		}, result
	}
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd migrate dolt' to start migration",
		Category: CategoryMaintenance,
	}, result
}
...
func CheckMigrationCompletion(path string) (DoctorCheck, MigrationValidationResult) {
	...
	if result.Backend != configfile.BackendDolt {
		...
		return DoctorCheck{
			...
			Fix:      "Run 'fbd migrate dolt' to migrate to Dolt",
			Category: CategoryMaintenance,
		}, result
	}
	...
	if len(result.Errors) > 0 {
		...
		return DoctorCheck{
			...
			Detail:   strings.Join(result.Errors, "\n"),
			Fix:      "Re-run 'fbd migrate dolt' or check for data issues",
			Category: CategoryMaintenance,
		}, result
	}
...
func CheckDoltLocks(path string) DoctorCheck {
	...
	if locked {
		return DoctorCheck{
			...
			Detail:   detail,
			Fix:      "Changes will be auto-committed on next fbd command",
			Category: CategoryMaintenance,
		}
	}
