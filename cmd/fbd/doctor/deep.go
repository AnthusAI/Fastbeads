...
func checkParentConsistency(db *sql.DB) DoctorCheck {
	...
	check.Fix = "Run 'fbd doctor --fix' to remove orphaned dependencies"
	return check
}
...
func checkDependencyIntegrity(db *sql.DB) DoctorCheck {
	...
	check.Fix = "Run 'fbd repair-deps' to remove broken dependencies"
	return check
}
...
func checkEpicCompleteness(db *sql.DB) DoctorCheck {
	...
	check.Fix = "Run 'fbd close <epic-id>' to close completed epics"
	return check
}
...
func checkMoleculeIntegrity(db *sql.DB) DoctorCheck {
	...
	if len(brokenMolecules) > 0 {
		...
		check.Fix = "Run 'fbd repair-deps' to clean up orphaned relationships"
		return check
	}
