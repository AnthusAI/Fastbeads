...
func CheckIDFormat(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd migrate hash-ids' to upgrade (prevents ID collisions in multi-worker scenarios)",
	}
}
...
func CheckDependencyCycles(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd dep cycles' to see full cycle paths, then 'fbd dep remove' to break cycles",
	}
}
...
func CheckTombstones(path string) DoctorCheck {
	...
	if alreadyExpired > 0 {
		return DoctorCheck{
			...
			Fix:     "Run 'fbd compact' to prune expired tombstones",
		}
	}
		...
		if err == nil {
			...
			if count > 0 {
				return DoctorCheck{
					...
					Fix:     "Run 'fbd migrate tombstones' to convert to inline tombstones",
				}
			}
			return DoctorCheck{
				Name:    "Deletions Manifest",
...
func CheckRepoFingerprint(path string) DoctorCheck {
	...
	if backend == configfile.BackendDolt {
		...
		if storedRepoID == "" {
			return DoctorCheck{
				...
				Detail:  "Storage: Dolt",
				Fix:     "Run 'fbd migrate --update-repo-id' to add fingerprint metadata",
			}
		}
		...
		if storedRepoID != currentRepoID {
			return DoctorCheck{
				...
				Fix:     "Run 'fbd migrate --update-repo-id' if URL changed, or 'rm -rf .beads && fbd init --backend dolt' if wrong database",
			}
		}
	...
	if err != nil {
		if err == sql.ErrNoRows || strings.Contains(err.Error(), "no such table") {
			...
			return DoctorCheck{
				...
				Fix:     "Run 'fbd migrate --update-repo-id' to add fingerprint",
			}
		}
		return DoctorCheck{
			Name:    "Repo Fingerprint",
	...
	if storedRepoID != currentRepoID {
		return DoctorCheck{
			...
			Message: "Database belongs to different repository",
			Detail:  fmt.Sprintf("stored: %s, current: %s", storedRepoID[:8], currentRepoID[:8]),
			Fix:     "Run 'fbd migrate --update-repo-id' if URL changed, or 'rm -rf .beads && fbd init' if wrong database",
		}
	}
