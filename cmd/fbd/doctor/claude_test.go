...
func TestCheckBdInPath(t *testing.T) {
	...
	// Note: This test will pass if fbd is in PATH (which it likely is during development)
	// In CI environments, the test may show "warning" if fbd isn't installed
	check := CheckBdInPath()
...
func TestCheckDocumentationBdPrimeReference(t *testing.T) {
	tests := []struct {
		...
		{
			name: "documentation without fbd prime",
			fileContent: map[string]string{
				"AGENTS.md": "# Agents\n\nUse fbd ready to see ready issues.",
			},
			expectedStatus: "ok",
			expectDetail:   false,
		},
		{
			name: "AGENTS.md references fbd prime",
			fileContent: map[string]string{
				"AGENTS.md": "# Agents\n\nRun `fbd prime` to get context.",
			},
			expectedStatus: "ok", // Will be ok if fbd is installed, warning otherwise
			expectDetail:   true,
		},
		{
			name: "CLAUDE.md references fbd prime",
			fileContent: map[string]string{
				"CLAUDE.md": "# Claude\n\nUse fbd prime for workflow context.",
			},
			expectedStatus: "ok",
			expectDetail:   true,
		},
		{
			name: ".claude/CLAUDE.md references fbd prime",
			fileContent: map[string]string{
				".claude/CLAUDE.md": "Run fbd prime to see workflow.",
			},
			expectedStatus: "ok",
			expectDetail:   true,
		},
		{
			name: "claude.local.md references fbd prime (local-only)",
			fileContent: map[string]string{
				"claude.local.md": "Run fbd prime for context.",
			},
			expectedStatus: "ok",
			expectDetail:   true,
		},
		{
			name: ".claude/claude.local.md references fbd prime (local-only)",
			fileContent: map[string]string{
				".claude/claude.local.md": "Use fbd prime for workflow context.",
			},
			expectedStatus: "ok",
			expectDetail:   true,
		},
		{
			name: "multiple files reference fbd prime",
			fileContent: map[string]string{
				"AGENTS.md": "Use fbd prime",
				"CLAUDE.md": "Run fbd prime",
			},
			expectedStatus: "ok",
			expectDetail:   true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// The status depends on whether fbd is installed, so we accept both ok and warning
			if check.Status != "ok" && check.Status != "warning" {
				t.Errorf("Expected status 'ok' or 'warning', got %s", check.Status)
			}
			...
			if tt.expectDetail && check.Status == "ok" && check.Detail == "" {
				t.Error("Expected Detail field to be set when files reference fbd prime")
			}
...
func TestCheckDocumentationBdPrimeReferenceNoFiles(t *testing.T) {
	...
	if check.Message != "No fbd prime references in documentation" {
...
func TestHasClaudeHooksProjectLevel(t *testing.T) {
	hooksContent := `{
		"hooks": {
			"SessionStart": [
				{"matcher": "", "hooks": [{"type": "command", "command": "fbd prime"}]}
			]
		}
	}`
	...
	t.Run("hooks but not fbd prime", func(t *testing.T) {
		tmpDir := t.TempDir()
		t.Chdir(tmpDir)
		setTempHome(t, t.TempDir())
		...
		if hasClaudeHooks() {
			t.Error("expected NOT to detect hooks when fbd prime not present")
		}
	})
}
...
func TestHasBeadsHooksWithBdPrime(t *testing.T) {
	...
	// Write settings with fbd prime hook
	settingsContent := `{
		"hooks": {
			"SessionStart": [
				{
	...
	if result != true {
		t.Error("Expected true for settings file with fbd prime hook")
	}
}
...
func TestHasBeadsHooksWithoutBdPrime(t *testing.T) {
	...
	// Write settings with hooks but not fbd prime
	settingsContent := `{
		"hooks": {
			"SessionStart": [
				{
	...
	if result != false {
		t.Error("Expected false for settings file without fbd prime hook")
	}
}
