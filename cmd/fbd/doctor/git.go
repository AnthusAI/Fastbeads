...
import (
	...
	"github.com/steveyegge/fastbeads/cmd/fbd/doctor/fix"
...
const (
	hooksExamplesURL = "https://github.com/steveyegge/fastbeads/tree/main/examples/git-hooks"
	hooksUpgradeURL  = "https://github.com/steveyegge/fastbeads/issues/615"
)

// bdShimMarker identifies fbd shim hooks (GH#946)
const bdShimMarker = "# bd-shim"

// bdInlineHookMarker identifies inline hooks created by fbd init (GH#1120)
// These hooks have the logic embedded directly rather than calling fbd hooks run
const bdInlineHookMarker = "# fbd (beads)"

// bdHooksRunPattern matches hooks that call fbd hooks run
...
func CheckGitHooks() DoctorCheck {
	...
	recommendedHooks := map[string]string{
		"pre-commit": "Flushes pending fbd changes to JSONL before commit",
	...
	if len(externalManagers) > 0 {
		// First, check if fbd shims are installed (GH#946)
		// If the actual hooks are fbd shims, they're calling fbd regardless of what
		...
		if hasBdShims, bdHooks := areBdShimsInstalled(hooksDir); hasBdShims {
			return DoctorCheck{
				Name:    "Git Hooks",
				Status:  StatusOK,
				Message: "fbd shims installed (ignoring external manager config)",
				Detail:  fmt.Sprintf("fbd hooks run: %s", strings.Join(bdHooks, ", ")),
			}
		}

		// External manager detected - check if it's configured to call fbd
		...
		if integration != nil {
			...
			if integration.DetectionOnly {
				return DoctorCheck{
					Name:    "Git Hooks",
					Status:  StatusOK,
					Message: fmt.Sprintf("%s detected (cannot verify fbd integration)", integration.Manager),
					Detail:  "Ensure your hook config calls 'fbd hooks run <hook>'",
				}
			}

			if integration.Configured {
				// Check if any hooks are missing fbd integration
				if len(integration.HooksWithoutBd) > 0 {
					return DoctorCheck{
						Name:    "Git Hooks",
						Status:  StatusWarning,
						Message: fmt.Sprintf("%s hooks not calling fbd", integration.Manager),
						Detail:  fmt.Sprintf("Missing fbd: %s", strings.Join(integration.HooksWithoutBd, ", ")),
						Fix:     "Add or upgrade to 'fbd hooks run <hook>'. See " + hooksUpgradeURL,
					}
				}

				// All hooks calling fbd - success
				return DoctorCheck{
					Name:    "Git Hooks",
					Status:  StatusOK,
					Message: fmt.Sprintf("All hooks via %s", integration.Manager),
					Detail:  fmt.Sprintf("fbd hooks run: %s", strings.Join(integration.HooksWithBd, ", ")),
				}
			}

			// External manager exists but doesn't call fbd at all
			return DoctorCheck{
				Name:    "Git Hooks",
				Status:  StatusWarning,
				Message: fmt.Sprintf("%s not calling fbd", fix.ManagerNames(externalManagers)),
				Detail:  "Configure hooks to call fbd commands",
				Fix:     "Add or upgrade to 'fbd hooks run <hook>'. See " + hooksUpgradeURL,
			}
		}
	}
	...
	hookInstallMsg := "Install hooks with 'fbd hooks install'. See " + hooksExamplesURL
...
// areBdShimsInstalled checks if the installed hooks are fbd shims, call fbd hooks run,
// or are inline fbd hooks created by fbd init.
// This helps detect when fbd hooks are installed directly but an external manager config exists.
// Returns (true, installedHooks) if fbd hooks are detected, (false, nil) otherwise.
// (GH#946, GH#1120)
func areBdShimsInstalled(hooksDir string) (bool, []string) {
	...
	for _, hookName := range hooks {
		...
		// Check for bd-shim marker, fbd hooks run call, or inline fbd hook marker (from fbd init)
...
func CheckGitUpstream(path string) DoctorCheck {
	...
	if behind > 0 && ahead == 0 {
		return DoctorCheck{
			...
			Detail:  fmt.Sprintf("Branch: %s, upstream: %s", branch, upstream),
			Fix:     "Run 'git pull --rebase' (then re-run fbd sync / fbd doctor)",
		}
	}
...
// (added in version 0.29.0). Without it, users experience circular "fbd sync" failures (issue #532).
func CheckSyncBranchHookCompatibility(path string) DoctorCheck {
	...
	if !strings.Contains(hookStr, "bd-hooks-version:") {
			...
			if integration != nil {
				...
				if integration.DetectionOnly {
					return DoctorCheck{
						Name:    "Sync Branch Hook Compatibility",
						Status:  StatusOK,
						Message: fmt.Sprintf("Managed by %s (cannot verify fbd integration)", names),
						Detail:  "Ensure pre-push hook calls 'fbd hooks run pre-push' for sync-branch",
					}
				}

				if integration.Configured {
					// Has fbd integration - check if pre-push is covered
					hasPrepush := false
					...
					if hasPrepush {
						var detail string
						// Only report hooks that ARE in config but lack fbd integration
						if len(integration.HooksWithoutBd) > 0 {
							detail = fmt.Sprintf("Hooks without fbd: %s", strings.Join(integration.HooksWithoutBd, ", "))
						}
						return DoctorCheck{
							Name:    "Sync Branch Hook Compatibility",
							Status:  StatusOK,
							Message: fmt.Sprintf("Managed by %s with fbd integration", integration.Manager),
							Detail:  detail,
						}
					}

					// Has fbd integration but missing pre-push
					return DoctorCheck{
						Name:    "Sync Branch Hook Compatibility",
						Status:  StatusWarning,
						Message: fmt.Sprintf("Managed by %s (missing pre-push fbd integration)", integration.Manager),
						Detail:  "pre-push hook needs 'fbd hooks run pre-push' for sync-branch",
						Fix:     fmt.Sprintf("Add or upgrade to 'fbd hooks run pre-push' in %s. See %s", integration.Manager, hooksExamplesURL),
					}
				}
			}

			// External manager detected but no fbd integration found
			return DoctorCheck{
				Name:    "Sync Branch Hook Compatibility",
				Status:  StatusWarning,
				Message: fmt.Sprintf("Managed by %s (no fbd integration detected)", names),
				Detail:  fmt.Sprintf("Pre-push hook managed by %s but no 'fbd hooks run' found", names),
				Fix:     fmt.Sprintf("Add or upgrade to 'fbd hooks run <hook>' in %s. See %s", names, hooksExamplesURL),
			}
		}
		...
		return DoctorCheck{
			Name:    "Sync Branch Hook Compatibility",
			Status:  StatusWarning,
			Message: "Pre-push hook is not a fbd hook",
			Detail:  "Cannot verify sync-branch compatibility with custom hooks",
			Fix: "Either run 'fbd hooks install --force' to use fbd hooks,\n" +
	...
	if hookVersion == "" {
		return DoctorCheck{
			...
			Fix:     "Run 'fbd hooks install --force' to update hooks",
		}
	}

	// MinSyncBranchHookVersion added sync-branch bypass logic
	// If hook version < MinSyncBranchHookVersion, it will cause circular "fbd sync" failures
	if CompareVersions(hookVersion, MinSyncBranchHookVersion) < 0 {
		return DoctorCheck{
			...
			Detail:  fmt.Sprintf("Hook version %s lacks sync-branch bypass (requires %s+). This causes circular 'fbd sync' failures during push.", hookVersion, MinSyncBranchHookVersion),
			Fix:     "Run 'fbd hooks install --force' to update hooks",
		}
	}
...
func CheckMergeDriver(path string) DoctorCheck {
	...
	if err != nil {
		...
		return DoctorCheck{
			...
			Fix:     "Run 'fbd init' to configure the merge driver, or manually: git config merge.beads.driver \"fbd merge %A %O %A %B\"",
		}
	}

	currentConfig := strings.TrimSpace(string(output))
	correctConfig := "fbd merge %A %O %A %B"
	...
	if strings.Contains(currentConfig, "%L") || strings.Contains(currentConfig, "%R") {
		return DoctorCheck{
			...
			Fix:     "Run 'fbd doctor --fix' to update to correct config, or manually: git config merge.beads.driver \"fbd merge %A %O %A %B\"",
		}
	}
	...
	if currentConfig != correctConfig {
		return DoctorCheck{
			...
			Fix:     fmt.Sprintf("Run 'fbd doctor --fix' to update config, or manually: git config merge.beads.driver \"%s\"", correctConfig),
		}
	}
...
func CheckSyncBranchConfig(path string) DoctorCheck {
	...
	// that will cause fbd sync to fail trying to create a worktree for a branch
	...
	if syncBranch != "" && currentBranch == syncBranch {
		return DoctorCheck{
			...
			Detail:  fmt.Sprintf("Currently on branch '%s' which is configured as the sync branch. fbd sync cannot create a worktree for a branch that's already checked out.", syncBranch),
	...
	if hasRemote {
		return DoctorCheck{
			...
			Fix:     "Run 'fbd migrate sync beads-sync' to set up sync branch workflow",
		}
	}
...
func CheckSyncBranchHealth(path string) DoctorCheck {
	...
	cmd.Dir = path
	if err := cmd.Run(); err != nil {
		// Local branch doesn't exist - that's fine, fbd sync will create it
		return DoctorCheck{
			Name:    "Sync Branch Health",
			Status:  StatusOK,
	...
	if err != nil {
		...
		return DoctorCheck{
			...
			Fix:     "Run 'fbd doctor --fix' to reset sync branch",
		}
	}
...
func CheckGitHooksDoltCompatibility(path string) DoctorCheck {
	...
	// Shim hooks (bd-shim) delegate to 'fbd hook' which handles Dolt correctly
	if strings.Contains(contentStr, bdShimMarker) {
		return DoctorCheck{
			Name:    "Git Hooks Dolt Compatibility",
			Status:  StatusOK,
			Message: "Shim hooks (Dolt handled by fbd hook command)",
		}
	}

	// Check if it's a fbd inline hook
	if !strings.Contains(contentStr, bdInlineHookMarker) && !strings.Contains(contentStr, "fbd") {
		return DoctorCheck{
			Name:    "Git Hooks Dolt Compatibility",
			Status:  StatusOK,
			Message: "N/A (not a fbd hook)",
		}
	}
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd hooks install --force' to update hooks for Dolt compatibility",
	}
}

// fixGitHooks fixes missing or broken git hooks by calling fbd hooks install.
...
// This is the shared core logic used by both 'fbd orphans' and 'fbd doctor' commands.
...
// message but forgot to run "fbd close".
func CheckOrphanedIssues(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd show <id>' to check if implemented, then 'fbd close <id>' if done",
		Category: CategoryGit,
	}
}
