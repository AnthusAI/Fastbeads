...
func CheckStaleClosedIssues(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd doctor --fix' to cleanup, or 'fbd admin cleanup --force' for more options",
		Category: CategoryMaintenance,
	}
}
...
func CheckExpiredTombstones(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd doctor --fix' to prune, or 'fbd admin cleanup --force' for more options",
		Category: CategoryMaintenance,
	}
}
...
func CheckStaleMolecules(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Detail:   detail,
		Fix:      "Run 'fbd mol stale' to review, then 'fbd close <id>' for each",
		Category: CategoryMaintenance,
	}
}
...
func CheckCompactionCandidates(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd compact --analyze' to review candidates",
		Category: CategoryMaintenance,
	}
}
...
// When users run "fbd mol pour" on formulas that should use "fbd mol wisp", the resulting
...
func CheckPersistentMolIssues(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Detail:   detail,
		Fix:      "Run 'fbd delete <id> --force' to remove, or use 'fbd mol wisp' instead of 'fbd mol pour'",
		Category: CategoryMaintenance,
	}
}
...
func CheckStaleMQFiles(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd doctor --fix' to delete, or 'rm -rf .beads/mq/'",
		Category: CategoryMaintenance,
	}
}
...
// If they're in JSONL without the ephemeral flag, they'll pollute fbd ready.
...
func CheckPatrolPollution(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:      "Run 'fbd doctor --fix' to clean up patrol pollution",
		Category: CategoryMaintenance,
	}
}
