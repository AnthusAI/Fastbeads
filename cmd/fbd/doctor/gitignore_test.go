...
func TestCheckGitignore_VariousStatuses(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedFix:    "Run: fbd init (safe to re-run) or fbd doctor --fix",
...
`
			...
			expectedStatus: StatusWarning,
			expectedFix:    "Run: fbd doctor --fix or fbd init (safe to re-run)",
...
// TestRequiredPatterns_ContainsSyncStatePatterns verifies that fbd doctor
...
// TestRequiredPatterns_ContainsLastTouched verifies that fbd doctor validates
...
// TestRequiredPatterns_ContainsJSONLLock verifies that fbd doctor validates
