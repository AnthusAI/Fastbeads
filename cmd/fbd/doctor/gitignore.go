...
func CheckGitignore() DoctorCheck {
	...
	if err != nil {
		return DoctorCheck{
			...
			Fix:     "Run: fbd init (safe to re-run) or fbd doctor --fix",
		}
	}
	...
	if len(missing) > 0 {
		return DoctorCheck{
			...
			Fix:     "Run: fbd doctor --fix or fbd init (safe to re-run)",
		}
	}
...
// cause issues.jsonl to be ignored, breaking fbd sync.
...
func CheckIssuesTracking() DoctorCheck {
	...
	if _, err := os.Stat(issuesPath); os.IsNotExist(err) {
		// File doesn't exist yet - not an error, fbd init may not have been run
		return DoctorCheck{
	...
	if err == nil {
		...
		return DoctorCheck{
			Name:    "Issues Tracking",
			Status:  "warning",
			Message: "issues.jsonl is ignored by git (fbd sync will fail)",
			Detail:  detail,
...
func CheckRedirectNotTracked() DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd doctor --fix' to untrack, or manually: git rm --cached .beads/redirect",
	}
}
	...
	if _, err := os.Stat(dbPath); os.IsNotExist(err) {
		...
		if len(matches) == 0 {
			return DoctorCheck{
				...
				Detail:  fmt.Sprintf("Target: %s", resolvedTarget),
				Fix:     "Run 'fbd init' in the target directory or check redirect path",
			}
		}
	}
...
func CheckRedirectTargetSyncWorktree() DoctorCheck {
	...
	if _, err := os.Stat(worktreePath); os.IsNotExist(err) {
		return DoctorCheck{
			...
			Fix:     fmt.Sprintf("Run 'fbd sync' in %s to create the worktree", targetRepoRoot),
		}
	}
...
func CheckLastTouchedNotTracked() DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd doctor --fix' to untrack, or manually: git rm --cached .beads/last-touched",
	}
}
...
func CheckSyncBranchGitignore() DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd doctor --fix' or 'fbd sync' to set git index flags",
	}
}
