...
import (
	...
	storagefactory "github.com/steveyegge/fastbeads/internal/storage/factory"
	"gopkg.in/yaml.v3"
)
...
func CheckDatabaseVersion(path string, cliVersion string) DoctorCheck {
	...
	if backend == configfile.BackendDolt {
		...
		if _, err := os.Stat(doltPath); os.IsNotExist(err) {
			...
			if issuesErr == nil || beadsErr == nil {
				return DoctorCheck{
					...
					Detail:  "Storage: Dolt",
					Fix:     "Run 'fbd init --backend dolt' to create and hydrate the dolt database",
				}
			}
			return DoctorCheck{
				Name:    "Database",
				...
				Detail:  "Storage: Dolt",
				Fix:     "Run 'fbd init --backend dolt' to create database",
			}
		}
		...
		if err != nil {
			return DoctorCheck{
				...
				Fix:     "Run 'fbd doctor --fix' to recover from JSONL backup, or manually: rm -rf .beads/dolt && fbd init --backend dolt",
			}
		}
		...
		if err != nil {
			return DoctorCheck{
				...
				Detail:  fmt.Sprintf("Storage: Dolt\n\nError: %v", err),
				Fix:     "Database may be corrupted. Run 'fbd doctor --fix' to recover from JSONL backup",
			}
		}
		if dbVersion == "" {
			return DoctorCheck{
				...
				Detail:  "Storage: Dolt",
				Fix:     "Run 'fbd migrate' or re-run 'fbd init --backend dolt' to set version metadata",
			}
		}
		...
		if dbVersion != cliVersion {
			return DoctorCheck{
				...
				Detail:  "Storage: Dolt",
				Fix:     "Update fbd CLI and re-run (dolt metadata will be updated automatically by the daemon)",
			}
		}
	...
	if _, err := os.Stat(dbPath); os.IsNotExist(err) {
		...
		if jsonlPath != "" {
			...
			fix := "Run 'fbd init' to hydrate the database from JSONL"
			if prefix != "" {
				fix = fmt.Sprintf("Run 'fbd init' to hydrate the database (detected prefix: %s)", prefix)
			}
		...
		return DoctorCheck{
			...
			Fix:     "Run 'fbd init' to create database",
		}
	}
	...
	if dbVersion == "unknown" {
		return DoctorCheck{
			...
			Fix:     "Database may be corrupted. Try 'fbd migrate'",
		}
	}
	...
	if dbVersion == "pre-0.17.5" {
		return DoctorCheck{
			...
			Detail:  "Storage: SQLite",
			Fix:     "Run 'fbd migrate' to upgrade database schema",
		}
	}
	...
	if dbVersion != cliVersion {
		return DoctorCheck{
			...
			Message: fmt.Sprintf("version %s (CLI: %s)", dbVersion, cliVersion),
			Detail:  "Storage: SQLite",
			Fix:     "Run 'fbd migrate' to sync database with CLI version",
		}
	}
...
func CheckSchemaCompatibility(path string) DoctorCheck {
	...
	if err != nil {
		return DoctorCheck{
			...
			Message: "Failed to open database",
			Detail:  err.Error(),
			Fix:     "Database may be corrupted. Try 'fbd migrate' or restore from backup",
		}
	}
	defer db.Close()
	...
	if len(missingElements) > 0 {
		return DoctorCheck{
			...
			Fix:     "Run 'fbd migrate' to upgrade schema, or if daemon is running an old version, run 'fbd daemons killall' to restart",
		}
	}
...
func CheckDatabaseIntegrity(path string) DoctorCheck {
	...
	if jsonlErr == nil && jsonlCount > 0 {
		return DoctorCheck{
			...
			Fix:     "Run 'fbd doctor --fix' to recover from JSONL backup",
		}
	}
	...
	return DoctorCheck{
		...
		Detail:  strings.Join(results, "; "),
		Fix:     "Run 'fbd doctor --fix' to back up the corrupt DB and rebuild from JSONL (if available), or restore from backup",
	}
}
...
func CheckDatabaseJSONLSync(path string) DoctorCheck {
	...
	if err != nil {
		...
		if jsonlErr == nil && jsonlCount > 0 {
			return DoctorCheck{
				...
				Detail:  err.Error(),
				Fix:     fmt.Sprintf("Run 'fbd import -i %s --rename-on-import' to recover issues from JSONL", filepath.Base(jsonlPath)),
			}
		}
		return DoctorCheck{
			Name:    "DB-JSONL Sync",
	...
	if jsonlErr != nil {
		fixMsg := "Run 'fbd doctor --fix' to attempt recovery"
		if strings.Contains(jsonlErr.Error(), "malformed") {
			fixMsg = "Run 'fbd doctor --fix' to back up and regenerate the JSONL from the database"
		}
		return DoctorCheck{
			Name:    "DB-JSONL Sync",
			Status:  StatusWarning,
	...
	if len(issues) > 0 {
		...
		var detail string
		if dbCount > jsonlCount {
			fixMsg = "Run 'fbd doctor --fix' to automatically export DB to JSONL, or manually run 'fbd export'"
		} else if jsonlCount > dbCount {
			fixMsg = "Run 'fbd doctor --fix' to automatically import JSONL to DB, or manually run 'fbd sync --import-only'"
		} else {
			// Equal counts but other issues (like prefix mismatch or status mismatch)
			fixMsg = "Run 'fbd doctor --fix' to fix automatically, or manually run 'fbd sync --import-only' or 'fbd export' depending on which has newer data"
		}
		if strings.Contains(strings.Join(issues, " "), "Prefix mismatch") {
			fixMsg = "Run 'fbd import -i .beads/issues.jsonl --rename-on-import' to fix prefixes"
		}
		...
		if strings.Contains(strings.Join(issues, " "), "Status mismatch") {
			fixMsg = "Run 'fbd export -o .beads/issues.jsonl' to update JSONL from DB (DB is usually source of truth)"
	...
	if jsonlInfo.ModTime().After(dbInfo.ModTime()) {
		...
		if timeDiff > 30*time.Second {
			return DoctorCheck{
				...
				Fix:     "Run 'fbd sync --import-only' to import JSONL updates",
			}
		}
	}
...
// FixDBJSONLSync fixes database-JSONL sync issues by running fbd sync --import-only
	...
	case strings.Contains(errMsg, "not a database") || strings.Contains(errMsg, "file is not a database"):
		...
		if jsonlAvailable {
			recoverySteps = fmt.Sprintf("Database file is corrupted beyond repair.\n\n"+
				...
				"2. Rebuild from JSONL (%d issues): fbd doctor --fix --force --source=jsonl\n"+
				"3. Verify: fbd stats", jsonlCount)
		} else {
			recoverySteps = "Database file is corrupted and no JSONL backup found.\n" +
				...
				"2. Rebuild database: fbd doctor --fix --force"
		}
	...
	case strings.Contains(errMsg, "migration") || strings.Contains(errMsg, "validation failed"):
		...
		if jsonlAvailable {
			recoverySteps = fmt.Sprintf("Database has validation errors (possibly orphaned dependencies).\n\n"+
				...
				"2. Rebuild from JSONL (%d issues): fbd doctor --fix --force --source=jsonl\n"+
				"3. Verify: fbd stats\n\n"+
				"Alternative: fbd doctor --fix --force (attempts to repair in-place)", jsonlCount)
		} else {
			recoverySteps = "Database validation failed and no JSONL backup available.\n" +
				"Try: fbd doctor --fix --force"
		}

	default:
		errorType = "Failed to open database"
		if jsonlAvailable {
			recoverySteps = fmt.Sprintf("Run 'fbd doctor --fix --source=jsonl' to rebuild from JSONL (%d issues)", jsonlCount)
		} else {
			recoverySteps = "Run 'fbd doctor --fix --force' to attempt recovery"
		}
	}
	return
}
...
func isNoDbModeConfigured(beadsDir string) bool {
	...
	if strings.EqualFold(cfg.Storage, "jsonl") {
		return true
	}
	return cfg.NoDb
}
...
// permanently deletes data. Users must explicitly run 'fbd cleanup' to prune.
//
...
func CheckDatabaseSize(path string) DoctorCheck {
	...
	if closedCount > threshold {
		return DoctorCheck{
			...
			Fix:     "Consider running 'fbd cleanup --older-than 90' to prune old closed issues",
		}
	}
