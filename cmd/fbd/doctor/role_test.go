...
func TestCheckBeadsRole_NotConfigured(t *testing.T) {
	...
	if check.Fix != "fbd config set beads.role maintainer" {
		t.Errorf("expected fix 'fbd config set beads.role maintainer', got %q", check.Fix)
	}
}
...
func TestCheckBeadsRole_InvalidValue(t *testing.T) {
	...
	if check.Fix != "fbd init" {
		t.Errorf("expected fix 'fbd init', got %q", check.Fix)
	}
}
