	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectWarning {
				...
				if !strings.Contains(check.Fix, "fbd setup claude") {
					t.Error("Expected fix message to mention 'fbd setup claude'")
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectWarning {
				...
				if !strings.Contains(check.Fix, "fbd list") {
...
func TestCheckLegacyJSONLFilename(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "beads merge artifacts ignored (fbd-ov1)",
...
func TestCheckFreshClone(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			jsonlIssues:    []string{"fbd-abc", "fbd-def"},
			hasDatabase:    true,
			expectedStatus: "ok",
		},
		{
		...
		{
			name:           "fresh clone with issues.jsonl (fbd-4ew)",
			hasBeadsDir:    true,
			jsonlFile:      "issues.jsonl",
			jsonlIssues:    []string{"fbd-abc", "fbd-def", "fbd-ghi"},
			hasDatabase:    false,
			expectedStatus: "warning",
			expectPrefix:   "fbd",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedStatus == "warning" {
				...
				if !strings.Contains(check.Fix, "fbd init") {
					t.Error("Expected fix to mention 'fbd init'")
				}
			}
		})
	}
