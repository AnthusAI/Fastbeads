...
func CheckMergeArtifacts(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd doctor --fix' to remove merge artifacts",
	}
}
...
func CheckOrphanedDependencies(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Detail:  detail,
		Fix:     "Run 'fbd doctor --fix' to remove orphaned dependencies",
	}
}
...
func CheckDuplicateIssues(path string, gastownMode bool, gastownThreshold int) DoctorCheck {
	...
	// Find duplicates by content hash (matching fbd duplicates algorithm)
	...
	for _, issue := range issues {
		...
		// Content key matches fbd duplicates: title + description + design + acceptanceCriteria + status
	...
	if totalDuplicates > threshold {
		return DoctorCheck{
			...
			Fix:     "Run 'fbd duplicates' to review and merge duplicates",
		}
	}
...
func CheckTestPollution(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Fix:     "Run 'fbd doctor --check=pollution' to review and clean test issues",
	}
}
...
func CheckChildParentDependencies(path string) DoctorCheck {
	...
	return DoctorCheck{
		...
		Detail:   detail,
		Fix:      "Run 'fbd doctor --fix --fix-child-parent' to remove (if unintentional)",
		Category: CategoryMetadata,
	}
}
