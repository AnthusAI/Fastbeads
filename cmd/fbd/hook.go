...
func resolveStorageConfig(beadsDir string) (*configfile.Config, string, error) {
...
// hookCmd is the main "fbd hook" command that git hooks call into.
// This is distinct from "fbd hooks" (plural) which manages hook installation.
...
in sync with the installed fbd version.
...
const (
	ChainBefore  HookChainStrategy = "before"  // Run existing hook before fbd hook
	ChainAfter   HookChainStrategy = "after"   // Run existing hook after fbd hook
...
func runChainedHookWithConfig(hookName string, args []string, cfg *HookConfig) int {
	...
	// Check if .old is itself a fbd shim - skip to prevent infinite recursion
...
func hookPreCommit() int {
	...
	hookCfg := loadHookConfig(beadsDir)

	// Run chained hook based on strategy
	if hookCfg.ChainStrategy == ChainBefore {
		if exitCode := runChainedHookWithConfig("pre-commit", nil, hookCfg); exitCode != 0 {
			return exitCode
		}
	}
	...
	if hookGetSyncBranch() != "" {
		if hookCfg.ChainStrategy == ChainAfter {
			return runChainedHookWithConfig("pre-commit", nil, hookCfg)
		}
		return 0
	}

	storageCfg, storageMode, err := resolveStorageConfig(beadsDir)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Warning: failed to resolve storage mode: %v\n", err)
		if hookCfg.ChainStrategy == ChainAfter {
			return runChainedHookWithConfig("pre-commit", nil, hookCfg)
		}
		return 0
	}
	if storageMode == configfile.StorageFiles || storageMode == configfile.StorageJSONL {
		stageStorageFiles(beadsDir, storageCfg, storageMode)
		if hookCfg.ChainStrategy == ChainAfter {
			return runChainedHookWithConfig("pre-commit", nil, hookCfg)
		}
		return 0
	...
	if backend == configfile.BackendDolt {
		...
		if hookCfg.ChainStrategy == ChainAfter && exitCode == 0 {
			return runChainedHookWithConfig("pre-commit", nil, hookCfg)
		}
		return exitCode
	}
...
// runJSONLExport runs the actual JSONL export via fbd sync.
...
func stageStorageFiles(beadsDir string, cfg *configfile.Config, mode string) {
	if os.Getenv("BEADS_NO_AUTO_STAGE") != "" {
		return
	}
	...
	case configfile.StorageFiles:
		issuesDir := cfg.IssuesDirPath(beadsDir)
		if _, err := os.Stat(issuesDir); err == nil {
			var gitAdd *exec.Cmd
			if rcErr == nil {
				gitAdd = rc.GitCmdCWD(ctx, "add", issuesDir)
			} else {
				// #nosec G204 -- issuesDir is a controlled path
				gitAdd = exec.Command("git", "add", issuesDir)
			}
			_ = gitAdd.Run()
		}
	case configfile.StorageJSONL:
		...
		if _, err := os.Stat(jsonlPath); err == nil {
			var gitAdd *exec.Cmd
			if rcErr == nil {
				gitAdd = rc.GitCmdCWD(ctx, "add", jsonlPath)
			} else {
				// #nosec G204 -- jsonlPath is a controlled path
				gitAdd = exec.Command("git", "add", jsonlPath)
			}
			_ = gitAdd.Run()
		}
	}
...
func hookPostMerge(args []string) int {
	...
	// Run chained hook based on strategy
	if hookCfg.ChainStrategy == ChainBefore {
		if exitCode := runChainedHookWithConfig("post-merge", args, hookCfg); exitCode != 0 {
			return exitCode
		}
	}
	...
	if isRebaseInProgress() {
		if hookCfg.ChainStrategy == ChainAfter {
			return runChainedHookWithConfig("post-merge", args, hookCfg)
		}
		return 0
	}

	_, storageMode, err := resolveStorageConfig(beadsDir)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Warning: failed to resolve storage mode: %v\n", err)
		if hookCfg.ChainStrategy == ChainAfter {
			return runChainedHookWithConfig("post-merge", args, hookCfg)
...
func hookPostMergeDolt(beadsDir string) int {
	...
	if len(conflicts) > 0 {
		fmt.Fprintf(os.Stderr, "Warning: %d conflict(s) detected during Dolt merge; resolve with 'fbd federation conflicts' or Dolt conflict tooling\n", len(conflicts))
...
func hookPostCheckout(args []string) int {
	...
	// Run chained hook based on strategy
	if hookCfg.ChainStrategy == ChainBefore {
		if exitCode := runChainedHookWithConfig("post-checkout", args, hookCfg); exitCode != 0 {
			return exitCode
		}
	}
