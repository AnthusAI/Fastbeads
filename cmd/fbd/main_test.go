...
func TestImportOpenToClosedTransition(t *testing.T) {
	...
	testStore := newTestStoreWithPrefix(t, dbPath, "fbd")
...
// TestBlockedEnvVars tests that FBD_BACKEND/BD_BACKEND and FBD_DATABASE_BACKEND/BD_DATABASE_BACKEND are blocked (bd-hevyw).
func TestBlockedEnvVars(t *testing.T) {
	tests := []struct {
		...
		{"FBD_BACKEND blocked", "FBD_BACKEND", "sqlite"},
		{"FBD_DATABASE_BACKEND blocked", "FBD_DATABASE_BACKEND", "sqlite"},
	...
	t.Run("no env vars set", func(t *testing.T) {
		t.Setenv("FBD_BACKEND", "")
		t.Setenv("FBD_DATABASE_BACKEND", "")
		t.Setenv("BD_BACKEND", "")
		t.Setenv("BD_DATABASE_BACKEND", "")
		// Unset them (t.Setenv("", "") sets to empty which Getenv returns as "")
		os.Unsetenv("FBD_BACKEND")
		os.Unsetenv("FBD_DATABASE_BACKEND")
