...
problem where 'fbd doctor --fix' can't run because it can't open the database.
...
After repair, normal fbd commands should work again.

Examples:
  fbd repair              # Repair database in current directory
  fbd repair --dry-run    # Show what would be cleaned without making changes
  fbd repair --path /other/repo  # Repair database in another location
  fbd repair --json       # Output results as JSON`,
	Run: runRepair,
}
...
func runRepair(cmd *cobra.Command, args []string) {
	...
	fmt.Println()
	fmt.Printf("%s Repair complete. Try running 'fbd doctor' to verify.\n", ui.RenderPass("✓"))
