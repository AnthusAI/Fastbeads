...
Examples:
  fbd where           # Show active beads location
  fbd where --json    # Output in JSON format
`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if beadsDir == "" {
			if jsonOutput {
				...
				fmt.Fprintln(os.Stderr, "Hint: run 'fbd init' to create a database in the current directory")
			}
			os.Exit(1)
		}
...
func detectPrefixFromDir(beadsDir string) string {
	...
	for _, line := range lines {
		...
		// Extract prefix from ID (e.g., "bd-123" -> "fbd")
