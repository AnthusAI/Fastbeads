...
// TestBackendListText tests `fbd backend list` text output.
func TestBackendListText(t *testing.T) {
	...
	if !strings.Contains(output, "fbd init --backend") {
		t.Error("output should contain usage hint")
	}
}

// TestBackendListJSON tests `fbd backend list --json` output.
...
// TestBackendShowSQLite tests `fbd backend show` with SQLite backend.
...
// TestBackendShowJSONL tests `fbd backend show` with JSONL-only mode.
