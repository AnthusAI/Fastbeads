...
func handleToDoltMigration(dryRun bool, autoYes bool) {
	...
	if beadsDir == "" {
		exitWithError("no_beads_directory", "No .beads directory found. Run 'fbd init' first.",
			"run 'fbd init' to initialize fbd")
	}
	...
	if isRunning, pid := isDaemonRunning(pidFile); isRunning {
		exitWithError("daemon_running",
			fmt.Sprintf("fbd daemon is running (PID %d). Stop it before migrating to avoid race conditions.", pid),
			"run 'fbd daemon stop' first, then retry migration")
	}
	...
	if _, err := os.Stat(sqlitePath); os.IsNotExist(err) {
		exitWithError("no_sqlite_database", "No SQLite database found to migrate",
			fmt.Sprintf("run 'fbd init' first (expected: %s)", sqlitePath))
	}
	...
	if hooksNeedDoltUpdate(beadsDir) {
		...
		if !jsonOutput {
			...
			fmt.Println("  Run 'fbd hooks install --force' to update them.")
		}
	}
...
func hooksNeedDoltUpdate(beadsDir string) bool {
	...
	if !strings.Contains(contentStr, "fbd") {
		return false // Not a fbd hook
	}
	...
	return true // fbd inline hook without Dolt check
}
...
func printFinalStatus(backend string, imported, skipped int, backupPath, newPath, oldPath string, toDolt bool) {
	if jsonOutput {
		...
		fmt.Println("  • Verify data: fbd list")
		fmt.Println("  • After verification, you can delete the old database:")
		if toDolt {
			fmt.Printf("    rm %s\n", oldPath)
			fmt.Println()
			fmt.Println("To switch back to SQLite: fbd migrate --to-sqlite")
		} else {
			fmt.Printf("    rm -rf %s\n", oldPath)
			fmt.Println()
			fmt.Println("To switch back to Dolt: fbd migrate --to-dolt")
		}
	}
}
