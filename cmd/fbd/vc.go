...
Note: 'fbd history', 'fbd diff', and 'fbd branch' also work for quick access.
...
Examples:
  fbd vc merge feature-xyz                    # Merge feature-xyz into current branch
  fbd vc merge feature-xyz --strategy ours    # Merge, preferring our changes on conflict
  fbd vc merge feature-xyz --strategy theirs  # Merge, preferring their changes on conflict`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(conflicts) > 0 {
			...
			fmt.Printf("\nResolve conflicts with: fbd vc merge %s --strategy [ours|theirs]\n\n", branchName)
			return
		}
...
Examples:
  fbd vc commit -m "Added new feature issues"
  fbd vc commit --message "Fixed priority on several issues"
  echo "Multi-line message" | fbd vc commit --stdin`,
	Run: func(cmd *cobra.Command, args []string) {
		ctx := rootCtx
