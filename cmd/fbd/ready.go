...
Note: 'fbd list --ready' is NOT equivalent - it only filters by status=open.

Use --mol to filter to a specific molecule's steps:
  fbd ready --mol bd-patrol   # Show ready steps within molecule

Use --gated to find molecules ready for gate-resume dispatch:
  fbd ready --gated           # Find molecules where a gate closed
...
This is useful for agents executing molecules to see which steps can run next.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		filter := types.WorkFilter{
			Status:          "open", // Only show open issues, not in_progress (matches fbd list --ready)
			Type:            issueType,
			Limit:           limit,
...
// MoleculeReadyOutput is the JSON output for fbd ready --mol
