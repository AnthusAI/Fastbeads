...
This command helps debug issues where fbd is using an unexpected database
...
// extractPrefix extracts the prefix from an issue ID (e.g., "bd-123" -> "fbd")
...
var versionChanges = []VersionChange{
	...
	},
	{
		...
		Date:    "2026-02-08",
		Changes: []string{
			"NEW: fbd search --has/--no flags for content and null-check filtering",
			"NEW: fbd promote command for wisp-to-bead promotion",
			"NEW: fbd todo command for lightweight task management",
			"NEW: fbd find-duplicates for AI-powered duplicate detection",
			"NEW: fbd validate integrated into fbd doctor --check=validate",
	...
	},
	{
		...
		Date:    "2026-02-05",
		Changes: []string{
			"NEW: --label-pattern and --label-regex flags for fbd list and fbd ready - glob and regex filtering on labels",
			"NEW: Simple query language for complex fbd list filtering",
			...
			"NEW: --metadata flag for fbd update (JSON metadata from CLI)",
			...
			"FIX: Atomic fbd claim with compare-and-swap semantics",
			"FIX: Dolt lock contention - advisory flock prevents zombie processes",
			"FIX: Windows Dolt build via pure-Go regex backend",
			"CHANGE: fbd ready excludes in_progress issues (shows only claimable work)",
		},
	},
	{
		Version: "0.49.3",
	...
	},
	{
		...
		Date:    "2026-01-31",
		Changes: []string{
			"NEW: GitLab backend - Bidirectional issue sync with GitLab (fbd gitlab sync/status/projects)",
			"NEW: Key-value store - fbd kv get/set/delete/list for persistent key-value storage",
			...
			"NEW: fbd backend and fbd sync mode subcommands for storage inspection",
			"NEW: Dolt auto-detect server mode during fbd init",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: fbd activity --details/-d - Full issue information in activity feed (#1317)",
			"NEW: fbd export --id/--parent - Targeted exports with filters (#1292)",
			"NEW: fbd update --append-notes - Append to existing notes (#1304)",
			"NEW: fbd show --id - For IDs that look like flags",
			"NEW: fbd doctor --server - Dolt server mode health checks",
			...
			"FIX: fbd list --json optimization - Fetch only needed dependencies (#1316)",
	...
	},
	{
		...
		Date:    "2026-01-21",
		Changes: []string{
			"NEW: Dolt federation - Peer-to-peer issue sync with fbd federation sync command",
			"NEW: SQLite to Dolt migration - fbd migrate dolt converts existing repos",
			"NEW: fbd children <id> - Display child issues for a parent",
			"NEW: fbd rename <old> <new> - Rename issue IDs",
			"NEW: fbd view - Alias for fbd show command",
			"NEW: fbd config validate - Validate sync configuration",
			"NEW: Jujutsu (jj) VCS support - Beads now works with jj repositories",
			"NEW: Per-field merge strategies for conflict resolution",
			"NEW: -m flag as alias for --description in fbd create",
			...
			"FIX: Molecule steps excluded from fbd ready (#1246)",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: fbd types command - List valid issue types with descriptions",
			"NEW: fbd close -m flag - Alias for --reason (git commit convention)",
			"NEW: RepoContext API - Centralized git operations context",
			"WIP: Dolt backend improvements - Bootstrap from JSONL, hook infrastructure, fbd compact --dolt",
	...
	},
	{
		...
		Date:    "2026-01-14",
		Changes: []string{
			"NEW: Dolt backend (experimental) - fbd init --backend=dolt for version-controlled storage",
			"NEW: fbd show --children flag - Display child issues inline with parent",
			"NEW: Comprehensive NixOS support - Improved flake and home-manager integration",
			"FIX: Redirect + sync-branch incompatibility - fbd sync works in redirected repos (bd-wayc3)",
			"FIX: Doctor project-level settings - Detects plugins/hooks/MCP in .claude/settings.json",
			"FIX: Contributor routing - fbd init --contributor correctly sets up routing (#1088)",
	...
	},
	{
		...
		Date:    "2026-01-12",
		Changes: []string{
			"NEW: fbd list --ready flag - Show only issues with no blockers (bd-ihu31)",
			...
			"FIX: Git worktree initialization - Prevent fbd init in worktrees (#1026)",
		},
	},
	{
		Version: "0.47.0",
		...
		Changes: []string{
			...
			"NEW: fbd resolve-conflicts command - Mechanical JSONL conflict resolution (bd-7e7ddffa)",
			"NEW: fbd create --dry-run - Preview issue creation without side effects (bd-0hi7)",
			"NEW: fbd ready --gated - Find molecules waiting on gates (bd-lhalq)",
			"NEW: Gate auto-discovery - Auto-discover workflow run ID in fbd gate check (bd-fbkd)",
			"NEW: Multi-repo custom types - fbd doctor discovers types across repos (bd-62g22)",
			...
			"FIX: fbd init hangs on Windows when not in git repo (#991)",
			...
			"FIX: fbd edit parses EDITOR with args (GH#987)",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: Deep pre-commit integration - fbd doctor checks pre-commit configs (bd-28r5)",
			"NEW: Rig identity bead type - New 'rig' type for Gas Town tracking (gt-zmznh)",
			"NEW: --filter-parent alias - Alternative to --parent in fbd list (bd-3p4u)",
			...
			"FIX: Update prefix routing - fbd update routes like fbd show (bd-618f)",
		},
	},
	{
		Version: "0.44.0",
		Date:    "2026-01-04",
		Changes: []string{
			"NEW: Recipe-based setup - fbd init refactored to modular recipes (bd-i3ed)",
			"NEW: Gate evaluation phases 2-4 - Timer, GitHub, cross-rig gate support",
			"NEW: fbd gate check/discover/add-waiter/show - Gate workflow commands",
			"NEW: --blocks flag for fbd dep add - Natural dependency syntax (GH#884)",
			"NEW: --blocked-by/--depends-on aliases for fbd dep add (bd-09kt)",
			...
			"NEW: Compound visualization - fbd mol show displays compound structure (bd-iw4z)",
			"NEW: /handoff skill - Session cycling slash command (bd-xwvo)",
			"FIX: fbd ready now shows in_progress issues (#894)",
	...
	},
	{
		...
		Changes: []string{
			...
			"NEW: fbd lint command - Template validation against schema",
			"NEW: fbd ready --pretty - Formatted human-friendly output",
			"FIX: Cross-rig routing for fbd close and fbd update",
			"FIX: Agent ID validation accepts any rig prefix (GH#827)",
			"FIX: fbd sync in bare repo worktrees - Exit 128 error (GH#827)",
			"FIX: fbd --no-db dep tree shows complete tree (GH#836)",
		},
	},
	{
		Version: "0.42.0",
		...
		Changes: []string{
			...
			"NEW: fbd preflight command - PR readiness checks (Phase 1)",
			"NEW: --claim flag for fbd update - Atomic work queue semantics",
			"NEW: fbd state/set-state commands - Label-based state management",
			"NEW: fbd activity --town - Cross-rig aggregated activity feed",
			...
			"FIX: fbd sync with sync-branch - Worktree copy direction fixed (GH#810, #812)",
			...
			"FIX: fbd list defaults - Non-closed issues, 50 limit (GH#788)",
	...
	},
	{
		...
		Date:    "2025-12-29",
		Changes: []string{
			"NEW: fbd swarm commands - Create/status/validate for multi-agent batch coordination",
			"NEW: fbd repair command - Detect and repair orphaned foreign key references",
			"NEW: fbd compact --purge-tombstones - Dependency-aware tombstone cleanup",
			"NEW: fbd init --from-jsonl - Preserve manual JSONL edits on reinit",
			"NEW: fbd human command - Focused help menu for humans",
			"NEW: fbd show --short - Compact output mode for scripting",
			"NEW: fbd delete --reason - Audit trail for deletions",
			...
			"FIX: fbd sync in bare repo worktrees (GH#785)",
			"FIX: fbd delete --cascade recursive deletion (GH#787)",
			"FIX: fbd doctor pre-push hook detection (GH#799)",
			...
			"CHANGED: Code organization - Split large cmd/fbd files to meet 800-line limit",
		},
	},
	{
		Version: "0.39.1",
		Date:    "2025-12-27",
		Changes: []string{
			"NEW: fbd where command - Show active beads location after following redirects",
			"NEW: --parent flag for fbd update - Reparent issues between epics",
			"NEW: Redirect info in fbd prime - Shows when database is redirected",
			"FIX: fbd doctor follows redirects - Multi-clone compatibility",
			"FIX: Remove 8-char prefix limit - fbd rename-prefix allows longer prefixes",
	...
	},
	{
		...
		Date:    "2025-12-27",
		Changes: []string{
			"NEW: fbd orphans command - Detect issues mentioned in commits but never closed",
			"NEW: fbd admin parent command - Consolidated cleanup/compact/reset under fbd admin",
			"NEW: --prefix flag for fbd create - Create issues in other rigs from any directory",
			"CHANGED: fbd mol catalog → fbd formula list - Aligns with formula terminology",
			"CHANGED: fbd info --thanks - Contributors list moved under fbd info",
			"CHANGED: Removed unused fbd pin/unpin/hook commands - Use gt mol commands",
			"CHANGED: fbd doctor --check=pollution - Test pollution check integrated into doctor",
	...
	},
	{
		...
		Date:    "2025-12-27",
		Changes: []string{
			"NEW: Prefix-based routing - fbd commands auto-route to correct rig via routes.jsonl",
			"NEW: Cross-rig ID auto-resolve - fbd dep add auto-resolves IDs across rigs",
			"NEW: fbd mol pour/wisp moved under fbd mol subcommand - cleaner command hierarchy",
			"NEW: fbd show displays comments - Comments now visible in issue details",
			"NEW: created_by field on issues - Track issue creator for audit trail",
			"NEW: Database corruption recovery in fbd doctor --fix - Auto-repair corrupted databases",
			"NEW: JSONL integrity check in fbd doctor - Detect and fix malformed JSONL",
			"NEW: Git hygiene checks in fbd doctor - Detect stale branches and sync issues",
			...
			"CHANGED: Wisp/Ephemeral name finalized as 'wisp' - fbd mol wisp is the canonical command",
			...
			"FIX: --resolution alias restored for fbd close - Backwards compatibility",
			"FIX: fbd graph works with daemon running - Graph generation no longer conflicts",
	...
	},
	{
		...
		Date:    "2025-12-26",
		Changes: []string{
			"BREAKING: Ephemeral API rename - Wisp→Ephemeral: JSON 'wisp'→'ephemeral', fbd wisp→fbd ephemeral",
			"NEW: fbd gate create/show/list/close/wait - Async coordination primitives for agent workflows",
			"NEW: fbd gate eval - Evaluate timer gates and GitHub gates (gh:run, gh:pr, mail)",
			"NEW: fbd gate approve - Human gate approval command",
			"NEW: fbd close --suggest-next - Show newly unblocked issues after close",
			"NEW: fbd ready/blocked --parent - Scope by epic or parent bead",
			...
			"NEW: fbd formula list/show - Discover and inspect available formulas",
			"NEW: fbd mol stale - Detect complete-but-unclosed molecules",
			"NEW: Stale molecules check in fbd doctor - Proactive detection",
			"NEW: Distinct ID prefixes - bd-proto-xxx, bd-mol-xxx, bd-wisp-xxx",
			"NEW: no-git-ops config - fbd config set no-git-ops true for manual git control",
			...
			"CHANGED: fbd mol run removed - Orchestration moved to gt commands",
			...
			"FIX: installed_plugins.json v2 format - fbd doctor handles new Claude Code format",
			"FIX: git.IsWorktree() hang on Windows - fbd init no longer hangs outside git repos",
			"FIX: Skill files deleted by fbd sync - .claude/ files now preserved",
			"FIX: doctor false positives - Skips interactions.jsonl and molecules.jsonl",
			"FIX: fbd sync commits non-.beads files - Now only commits .beads/ directory",
	...
	},
	{
		...
		Date:    "2025-12-24",
		Changes: []string{
			"NEW: Formula system - fbd cook <formula> for declarative workflow templates",
			"NEW: Gate issue type - fbd gate create/open/close for async coordination",
			"NEW: fbd list --pretty --watch - Built-in colorized viewer with live updates",
			"NEW: fbd search --after/--before/--priority/--content - Enhanced search filters",
			"NEW: fbd compact --prune - Standalone tombstone pruning",
			"NEW: fbd export --priority - Exact priority filter for exports",
			"NEW: --resolution alias for --reason on fbd close",
			"NEW: Config-based close hooks - Custom scripts on issue close",
			"CHANGED: fbd mol spawn removed - Use fbd pour/fbd wisp create only",
			"CHANGED: fbd ready excludes workflow types by default",
	...
	},
	{
		...
		Date:    "2025-12-23",
		Changes: []string{
			"NEW: fbd activity command - Real-time state feed for molecule monitoring",
			"NEW: Dynamic molecule bonding - fbd mol bond --ref <id> attaches protos at runtime",
			...
			"NEW: fbd list --parent flag - Filter issues by parent",
			"NEW: Molecule navigation - fbd mol next/prev/current for step traversal",
			"NEW: Entity tracking types - Creator and Validations fields for work attribution",
			"IMPROVED: fbd doctor --fix replaces manual commands",
			"IMPROVED: fbd dep tree shows external dependencies",
			...
			"FIX: fbd create -f works with daemon mode",
	...
	},
	{
		...
		Date:    "2025-12-22",
		Changes: []string{
			"NEW: Wisp commands - fbd wisp create/list/gc for ephemeral molecule management",
			"NEW: Chemistry UX - fbd pour, fbd mol bond --wisp/--pour for phase control",
			"NEW: Cross-project deps - external:<repo>:<id> syntax, fbd ship command",
			"BREAKING: fbd repo add/remove now writes to .beads/config.yaml (not DB)",
	...
	},
	{
		...
		Changes: []string{
			...
			"BREAKING: CLI flag changed from --ephemeral to --wisp (fbd cleanup)",
	...
	},
	{
		...
		Date:    "2025-12-21",
		Changes: []string{
			"NEW: Wisp molecules - use 'fbd wisp create' for ephemeral wisps",
			"NEW: Wisp issues live only in SQLite, never export to JSONL (prevents zombie resurrection)",
			"NEW: Use 'fbd pour' for persistent mols, 'fbd wisp create' for ephemeral wisps",
			"NEW: fbd mol squash compresses wisp children into digest issue",
			"NEW: --summary flag on fbd mol squash for agent-provided AI summaries",
	...
	},
	{
		...
		Changes: []string{
			...
			"FIX: Pin field not in allowed update fields - fbd update --pinned now works",
		},
	},
	{
		Version: "0.32.0",
		Date:    "2025-12-20",
		Changes: []string{
			"REMOVED: fbd mail commands (send, inbox, read, ack, reply) - Mail is orchestration, not data plane",
			...
			"FIX: Symlink preservation in atomicWriteFile - fbd setup no longer clobbers nix/home-manager configs",
	...
	},
	{
		...
		Date:    "2025-12-20",
		Changes: []string{
			"NEW: fbd defer/fbd undefer commands - Deferred status for icebox issues",
			"NEW: Agent audit trail - .beads/interactions.jsonl with fbd audit record/label",
			...
			"CHANGED: fbd stats merged into fbd status - stats is now alias, colorized output",
			"CHANGED: Thin hook shims - Hooks delegate to fbd hooks run, no more version drift",
	...
	},
	{
		...
		Date:    "2025-12-19",
		Changes: []string{
			"FIX: fbd graph no longer crashes with nil pointer on epics",
	...
	},
	{
		...
		Date:    "2025-12-18",
		Changes: []string{
			"fbd graph command shows dependency counts using subgraph formatting",
	...
	},
	{
		...
		Date:    "2025-12-18",
		Changes: []string{
			"REMOVED: YAML simple template system - --from-template flag removed from fbd create",
			"REMOVED: Embedded templates (bug.yaml, epic.yaml, feature.yaml) - Use Beads templates instead",
			"Templates are now purely Beads-based - Create epic with 'template' label, use fbd template instantiate",
		},
	},
	{
		Version: "0.30.4",
		Date:    "2025-12-18",
		Changes: []string{
			"fbd template instantiate - Create beads issues from Beads templates",
			"--assignee flag for template instantiate - Auto-assign during instantiation",
			"fbd mail inbox --identity fix - Now properly filters by identity parameter",
	...
	},
	{
		...
		Date:    "2025-12-16",
		Changes: []string{
			"fbd setup droid - Factory.ai (Droid) IDE support",
	...
	},
	{
		...
		Date:    "2025-12-16",
		Changes: []string{
			"fbd reset command - Complete beads removal from a repository",
			"fbd update --type flag - Change issue type after creation",
			"fbd q silent mode - Quick-capture without output for scripting",
			"fbd show displays dependent issue status - Shows status for blocked-by/blocking issues",
			...
			"fbd migrate-tombstones command - Converts legacy deletions.jsonl to inline tombstones",
	...
	},
	{
		...
		Changes: []string{
			...
			"fbd migrate-tombstones - Convert legacy deletions.jsonl to inline tombstones",
			"fbd doctor tombstone health checks - Detects orphaned/expired tombstones",
			...
			"fbd thanks command - List contributors to your project",
	...
	},
	{
		...
		Date:    "2025-12-03",
		Changes: []string{
			"--estimate flag for fbd create/update - Add time estimates to issues in minutes",
			"fbd doctor improvements - SQLite integrity check, config validation, stale sync branch detection",
			"fbd doctor --output flag - Export diagnostics to file for sharing/debugging",
			"fbd doctor --dry-run flag - Preview fixes without applying them",
			"fbd doctor per-fix confirmation mode - Approve each fix individually",
			"--readonly flag - Read-only mode for worker sandboxes",
			"fbd sync safety improvements - Auto-push after merge, diverged history handling",
	...
	},
	{
		...
		Date:    "2025-12-01",
		Changes: []string{
			"fbd daemon --local flag - Run daemon without git operations for multi-repo/worktree setups",
			"fbd daemon --foreground flag - Run in foreground for systemd/supervisord integration",
			"fbd migrate-sync command - Migrate to sync.branch workflow for cleaner main branch",
	...
	},
	{
		...
		Changes: []string{
			...
			"Fresh Clone Initialization - fbd init auto-detects prefix from existing JSONL, works without --prefix flag",
	...
	},
	{
		...
		Changes: []string{
			...
			"Contributor Fork Workflows - `fbd init --contributor` auto-configures sync.remote=upstream",
			...
			"Fresh Clone Detection - fbd doctor suggests 'fbd init' when JSONL exists but no DB",
		},
	},
	{
		Version: "0.26.0",
		Date:    "2025-11-27",
		Changes: []string{
			"fbd doctor --check-health - Lightweight health checks for startup hooks (exit 0 on success)",
			...
			"fbd sync Commit Scope Fixed - Only commits .beads/ files, not other staged files",
		},
	},
	{
		Version: "0.25.1",
		...
		Changes: []string{
			...
			"fbd sync commit scope fixed - Now commits entire .beads/ directory before pull",
			"fbd prime ephemeral branch detection - Auto-detects ephemeral branches and adjusts workflow",
	...
	},
	{
		...
		Changes: []string{
			...
			"Stealth Mode - `fbd init --stealth` for invisible beads usage",
			"Ephemeral Branch Sync - `fbd sync --from-main` to sync from main without pushing",
		},
	},
	{
		Version: "0.24.4",
		...
		Changes: []string{
			...
			"Sorting for fbd list/search - New `--sort` and `--reverse` flags",
	...
	},
	{
		...
		Date:    "2025-11-22",
		Changes: []string{
			"fbd search filters - Date and priority filters added",
			"fbd count - New command for counting and grouping issues",
	...
	},
	{
		...
		Date:    "2025-11-20",
		Changes: []string{
			"fbd doctor --fix - Automatic repair functionality",
			"fbd clean - Remove temporary merge artifacts",
			".beads/README.md Generation - Auto-generated during fbd init",
	...
	},
	{
		...
		Changes: []string{
			...
			"`fbd info --whats-new` - Quick upgrade summaries for agents (shows last 3 versions)",
			"`fbd hooks install` - Embedded git hooks command (replaces external script)",
			"`fbd cleanup` - Bulk deletion for agent-driven compaction",
			"`fbd new` alias added - Agents often tried this instead of `fbd create`",
			"`fbd list` now one-line-per-issue by default - Prevents agent miscounting (use --long for old format)",
			...
			"Auto-import when database missing - `fbd import` now auto-initializes",
...
func showWhatsNew() {
	...
	fmt.Println("💡 Tip: Use `fbd info --whats-new --json` for machine-readable output")
	fmt.Println()
}
