...
// This gets appended to AGENTS.md and @AGENTS.md during fbd init
...
func setupClaudeSettings(verbose bool) error {
	...
	if content, err := os.ReadFile(settingsPath); err == nil {
		if err := json.Unmarshal(content, &existingSettings); err != nil {
			...
			return fmt.Errorf("existing %s contains invalid JSON: %w\nPlease fix the JSON syntax manually before running fbd init", settingsPath, err)
		}
	...
	onboardPrompt := "Before starting any work, run 'fbd onboard' to understand the current project state and available issues."
	...
	if promptValue, exists := existingSettings["prompt"]; exists {
		if promptStr, ok := promptValue.(string); ok {
			if strings.Contains(promptStr, "fbd onboard") {
				if verbose {
					fmt.Printf("Claude settings already configured with fbd onboard instruction\n")
				}
				return nil
			}
	...
	if verbose {
		fmt.Printf("Configured Claude settings with fbd onboard instruction\n")
	}
