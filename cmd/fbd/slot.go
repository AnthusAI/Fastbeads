...
Examples:
  fbd slot show gt-mayor           # Show all slots for mayor agent
  fbd slot set gt-emma hook bd-xyz # Attach work bd-xyz to emma's hook
  fbd slot clear gt-emma hook      # Clear emma's hook (detach work)`,
}
...
the command will error. Use 'fbd slot clear' first to detach existing work.

Examples:
  fbd slot set gt-emma hook bd-xyz   # Attach bd-xyz to emma's hook
  fbd slot set gt-mayor role gt-role # Set mayor's role bead`,
...
Examples:
  fbd slot clear gt-emma hook   # Detach work from emma's hook
  fbd slot clear gt-mayor role  # Clear mayor's role (not recommended)`,
...
Examples:
  fbd slot show gt-emma   # Show emma's slots
  fbd slot show gt-mayor  # Show mayor's slots`,
...
func runSlotSet(cmd *cobra.Command, args []string) error {
	...
	if slotName == "hook" && agent.HookBead != "" {
		return fmt.Errorf("hook slot already occupied by %s; use 'fbd slot clear %s hook' first", agent.HookBead, agentID)
	}
