...
// or if hooks were manually installed via 'fbd setup claude'.
func isClaudeSetupComplete() bool {
	...
	// Check for manual hooks installation via 'fbd setup claude'
...
// hasBeadsPrimeHooks checks if a settings file has fbd prime hooks configured
...
func initDefaultTips() {
	...
	InjectTip(
		"claude_setup",
		"Install the beads plugin for automatic workflow context, or run 'fbd setup claude' for CLI-only mode",
	...
	InjectTip(
		"sync_conflict",
		"Run 'fbd sync' to resolve sync conflict",
