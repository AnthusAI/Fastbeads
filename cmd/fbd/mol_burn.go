...
permanently lost. If you want to preserve a summary, use 'fbd mol squash'.

Example:
  fbd mol burn bd-abc123              # Delete molecule with no trace
  fbd mol burn bd-abc123 --dry-run    # Preview what would be deleted
  fbd mol burn bd-abc123 --force      # Skip confirmation
  fbd mol burn bd-a1 bd-b2 bd-c3      # Batch delete multiple wisps`,
...
func burnWispMolecule(ctx context.Context, resolvedID string, dryRun, force bool) {
	...
	if dryRun {
		...
		fmt.Printf("\nNo digest will be created (use 'fbd mol squash' to create one).\n")
		return
	}
	...
	if !force && !jsonOutput {
		...
		fmt.Printf("Use 'fbd mol squash' instead if you want to preserve a summary.\n")
		fmt.Printf("\nContinue? [y/N] ")
...
func burnPersistentMolecule(ctx context.Context, resolvedID string, dryRun, force bool) {
	...
	if dryRun {
		...
		fmt.Printf("No digest will be created (use 'fbd mol squash' to create one).\n")
		return
	}
