...
The `cmd/fbd` directory contains the complete CLI application for the Beads issue tracker. It implements the `fbd` command-line tool, which users interact with to create, query, manage, and synchronize issues across distributed systems.
...
- **Entry Point**: The CLI defined here (`cmd/fbd/main.go`) is the user-facing interface to the entire beads system. All user interactions flow through this package.
...
- **Version Reporting**: The version command (`@/cmd/fbd/version.go`) reports full build information - it resolves git commit and branch from ldflags set at build time via the Makefile (`@/Makefile`) and goreleaser config (`@/.goreleaser.yml`). This enables users to identify exactly what code their binary was built from.
...
1. **Version Variables** (lines 15-23 in `@/cmd/fbd/version.go`):
...
4. **Output Formatting**:
   - **Text Output** (lines 52-58 in `version.go`): Shows format like `fbd version 0.29.0 (dev: main@7e70940)` when both commit and branch are available
...
- Without explicit ldflags, users running `fbd version` would only see semantic version and build type, not which commit they had
...
**Testing Coverage** (`@/cmd/fbd/version_test.go`):
