...
Examples:
  fbd mol pour mol-feature --var name=auth    # Persistent feature work
  fbd mol pour mol-review --var pr=123        # Persistent code review`,
...
func runPour(cmd *cobra.Command, args []string) {
	...
	if err == nil {
		...
		if sg.Phase == "vapor" {
			...
			fmt.Fprintf(os.Stderr, "  Consider using: fbd mol wisp %s", args[0])
			for _, v := range varFlags {
