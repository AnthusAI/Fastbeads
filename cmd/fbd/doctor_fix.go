...
func previewFixes(result doctorResult) {
	...
	fmt.Println("Run 'fbd doctor --fix' to apply these fixes")
}
	...
	for _, check := range fixes {
		...
		case "Child-Parent Dependencies":
			...
			if !doctorFixChildParent {
				fmt.Printf("  ⚠ Child→parent deps require explicit opt-in: fbd doctor --fix --fix-child-parent\n")
				continue
			}
			err = fix.ChildParentDependencies(path, doctorVerbose)
		case "Duplicate Issues":
			// No auto-fix: duplicates require user review
			fmt.Printf("  ⚠ Run 'fbd duplicates' to review and merge duplicates\n")
			continue
		case "Test Pollution":
			// No auto-fix: test cleanup requires user review
			fmt.Printf("  ⚠ Run 'fbd doctor --check=pollution' to review and clean test issues\n")
			continue
		case "Git Conflicts":
		...
		case "Compaction Candidates":
			...
			fmt.Printf("  ⚠ Run 'fbd compact --analyze' to review candidates\n")
			continue
		case "Large Database":
			// No auto-fix: pruning deletes data, must be user-controlled
			fmt.Printf("  ⚠ Run 'fbd cleanup --older-than 90' to prune old closed issues\n")
			continue
