...
This project uses **Beads (fbd)** for issue tracking. Use the fbd CLI or MCP tools for all task management.

## Core Workflow Rules

1. **Track ALL work in fbd** - Never use markdown TODOs or comment-based task lists
2. **Check ready work first** - Run ` + "`fbd ready`" + ` to find unblocked issues
...
5. **Sync at session end** - Run ` + "`fbd sync`" + ` before ending your session

## Quick Command Reference

### Finding Work
` + "```bash" + `
fbd ready              # Show unblocked issues ready for work
fbd list --status open # List all open issues
fbd show <id>          # View issue details
fbd blocked            # Show blocked issues and their blockers
` + "```" + `

### Creating Issues
` + "```bash" + `
fbd create "Title" --description="Details" -t bug|feature|task -p 0-4 --json
fbd create "Found bug" --description="Details" --deps discovered-from:bd-42 --json
` + "```" + `

### Working on Issues
` + "```bash" + `
fbd update <id> --status in_progress  # Claim work
fbd update <id> --priority 1          # Change priority
fbd close <id> --reason "Completed"   # Mark complete
` + "```" + `

### Dependencies
` + "```bash" + `
fbd dep add <issue> <depends-on>      # Add dependency (issue depends on depends-on)
fbd dep add <issue> <depends-on> --type=related  # Soft link
` + "```" + `

### Syncing
` + "```bash" + `
fbd sync  # ALWAYS run at session end - commits and pushes changes
` + "```" + `
...
- ✅ Check ` + "`fbd ready`" + ` before asking "what should I work on?"
- ✅ Run ` + "`fbd sync`" + ` at end of session
- ❌ Do NOT create markdown TODO lists
- ❌ Do NOT use external issue trackers
- ❌ Do NOT duplicate tracking systems

For more details, run ` + "`fbd --help`" + ` or see the project's AGENTS.md file.
`
...
func CheckJunie() {
	...
	if guidelinesExists {
		...
		fmt.Println("  Run: fbd setup junie (to complete installation)")
		os.Exit(1)
	}
	...
	fmt.Println("  Run: fbd setup junie")
	os.Exit(1)
}
