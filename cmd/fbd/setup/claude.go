...
func checkClaude(env claudeEnv) error {
	...
	default:
		...
		_, _ = fmt.Fprintln(env.stdout, "  Run: fbd setup claude")
...
func removeClaude(env claudeEnv, project bool) error {
	...
	removeHookCommand(hooks, "PreCompact", "fbd prime")
	removeHookCommand(hooks, "SessionStart", "fbd prime --stealth")
	removeHookCommand(hooks, "PreCompact", "fbd prime --stealth")
...
func addHookCommand(hooks map[string]interface{}, event, command string) bool {
	...
	// Check if fbd hook already registered
	...
	for _, event := range []string{"SessionStart", "PreCompact"} {
		...
		for _, hook := range eventHooks {
			...
			for _, cmd := range commands {
				...
				cmd := cmdMap["command"]
				if cmd == "fbd prime" || cmd == "fbd prime --stealth" {
					return true
				}
			}
		}
