...
func checkGemini(env geminiEnv) error {
	...
	default:
		...
		_, _ = fmt.Fprintln(env.stdout, "  Run: fbd setup gemini")
...
func removeGemini(env geminiEnv, project bool) error {
	...
	removeHookCommand(hooks, "PreCompress", "fbd prime")
	removeHookCommand(hooks, "SessionStart", "fbd prime --stealth")
	removeHookCommand(hooks, "PreCompress", "fbd prime --stealth")
...
// hasGeminiBeadsHooks checks if a settings file has fbd prime hooks for Gemini CLI
func hasGeminiBeadsHooks(settingsPath string) bool {
	...
	// Check SessionStart and PreCompress for "fbd prime"
	for _, event := range []string{"SessionStart", "PreCompress"} {
		...
		for _, hook := range eventHooks {
			...
			for _, cmd := range commands {
				...
				if cmdStr == "fbd prime" || cmdStr == "fbd prime --stealth" {
					return true
				}
			}
		}
