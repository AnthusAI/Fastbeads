...
func TestAiderInstructionsWorkflowPattern(t *testing.T) {
	...
	if !strings.Contains(instructions, "/run fbd ready") {
		t.Error("Should mention /run fbd ready")
	}
	if !strings.Contains(instructions, "/run fbd sync") {
		t.Error("Should mention /run fbd sync")
	}
