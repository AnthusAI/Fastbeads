...
func TestJunieMCPConfig(t *testing.T) {
	...
	if !ok || command != "fbd" {
		t.Errorf("Expected command 'fbd', got %v", beads["command"])
	}
...
func TestJunieGuidelinesWorkflowPattern(t *testing.T) {
	...
	if !strings.Contains(guidelines, "fbd ready") {
		t.Error("Should mention fbd ready")
	}
	if !strings.Contains(guidelines, "fbd sync") {
		t.Error("Should mention fbd sync")
	}
