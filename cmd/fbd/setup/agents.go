...
## Issue Tracking with fbd (beads)

**IMPORTANT**: This project uses **fbd (beads)** for ALL issue tracking. Do NOT use markdown TODOs, task lists, or other tracking methods.
...
` + "```bash" + `
fbd create "Issue title" --description="Detailed context" -t bug|feature|task -p 0-4 --json
fbd create "Issue title" --description="What this issue is about" -p 1 --deps discovered-from:bd-123 --json
` + "```" + `
...
fbd update bd-42 --priority 1 --json
` + "```" + `

**Complete work:**

` + "```bash" + `
fbd close bd-42 --reason "Completed" --json
` + "```" + `
...
1. **Check ready work**: ` + "`fbd ready`" + ` shows unblocked issues
2. **Claim your task**: ` + "`fbd update <id> --status in_progress`" + `
3. **Work on it**: Implement, test, document
4. **Discover new work?** Create linked issue:
   - ` + "`fbd create \"Found bug\" --description=\"Details about what was found\" -p 1 --deps discovered-from:<parent-id>`" + `
5. **Complete**: ` + "`fbd close <id> --reason \"Done\"`" + `

### Auto-Sync

fbd automatically syncs with git:
