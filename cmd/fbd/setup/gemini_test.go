...
func TestInstallGemini_Stealth(t *testing.T) {
	...
	if cmd["command"] != "fbd prime --stealth" {
...
func TestRemoveGemini_PreservesOtherHooks(t *testing.T) {
	...
	// Verify it's the other command, not fbd prime
	hook := sessionStart[0].(map[string]interface{})
	cmds := hook["hooks"].([]interface{})
	cmd := cmds[0].(map[string]interface{})
	if cmd["command"] == "fbd prime" || cmd["command"] == "fbd prime --stealth" {
		t.Error("fbd prime hook should have been removed")
	}
}
...
func TestHasGeminiBeadsHooks(t *testing.T) {
	...
	if !hasGeminiBeadsHooks(settingsPath) {
		t.Error("expected true for settings with fbd prime hook")
	}
	...
	settings["hooks"].(map[string]interface{})["SessionStart"] = []interface{}{
		map[string]interface{}{
			"matcher": "",
			"hooks": []interface{}{
				map[string]interface{}{"type": "command", "command": "fbd prime --stealth"},
			},
		},
	}
	data, _ = json.Marshal(settings)
	...
	if !hasGeminiBeadsHooks(settingsPath) {
		t.Error("expected true for settings with fbd prime --stealth hook")
	}
}
