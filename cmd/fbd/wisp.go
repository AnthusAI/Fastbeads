...
//
// Commands:
//   fbd mol wisp list    - List all wisps in current context
...
The wisp lifecycle:
  1. Create: fbd mol wisp <proto> or fbd create --ephemeral
  2. Execute: Normal fbd operations work on wisp issues
  3. Squash: fbd mol squash <id> (clears Ephemeral flag, promotes to persistent)
  4. Or burn: fbd mol burn <id> (deletes without creating digest)

Examples:
  fbd mol wisp beads-release --var version=1.0  # Release workflow
  fbd mol wisp mol-patrol                       # Ephemeral patrol cycle
...
Examples:
  fbd mol wisp create mol-patrol                    # Ephemeral patrol cycle
  fbd mol wisp create mol-health-check              # One-time health check
  fbd mol wisp create mol-diagnostics --var target=db  # Diagnostic run`,
...
func runWispCreate(cmd *cobra.Command, args []string) {
	...
	if store == nil {
		...
		fmt.Fprintf(os.Stderr, "Hint: run 'fbd init' or 'fbd import' to initialize the database\n")
		os.Exit(1)
	}
	...
	if subgraph == nil {
		...
		if strings.HasPrefix(protoID, "mol-") {
			...
			if !found {
				...
				fmt.Fprintf(os.Stderr, "Hint: run 'fbd formula list' to see available formulas\n")
				os.Exit(1)
			}
		}
	...
	fmt.Printf("\nNext steps:\n")
	fmt.Printf("  fbd close %s.<step>       # Complete steps\n", result.NewEpicID)
	fmt.Printf("  fbd mol squash %s         # Condense to digest (promotes to persistent)\n", result.NewEpicID)
	fmt.Printf("  fbd mol burn %s           # Discard without creating digest\n", result.NewEpicID)
}
...
Old wisp detection:
  ...
  - Use 'fbd mol wisp gc' to clean up old/abandoned wisps

Examples:
  fbd mol wisp list              # List all wisps
  fbd mol wisp list --json       # JSON output for programmatic use
  fbd mol wisp list --all        # Include closed wisps`,
	Run: runWispList,
}
...
func runWispList(cmd *cobra.Command, args []string) {
	...
	if oldCount > 0 {
		...
		fmt.Println("  Hint: Use 'fbd mol wisp gc' to clean up old wisps")
	}
}
...
Abandoned wisps are deleted without creating a digest. Use 'fbd mol squash'
...
For graph-pressure staleness detection (blocking other work), see 'fbd mol stale'.

Examples:
  fbd mol wisp gc                # Clean abandoned wisps (default: 1h threshold)
  fbd mol wisp gc --dry-run      # Preview what would be cleaned
  fbd mol wisp gc --age 24h      # Custom age threshold
  fbd mol wisp gc --all          # Also clean closed wisps older than threshold`,
	Run: runWispGC,
}
...
func init() {
	// Wisp command flags (for direct create: fbd mol wisp <proto>)
	...
	// Wisp create command flags (kept for backwards compat: fbd mol wisp create <proto>)
