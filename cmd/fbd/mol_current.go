...
Use --limit or --range to view specific steps:
  fbd mol current <id> --limit 50       # Show first 50 steps
  fbd mol current <id> --range 100-150  # Show steps 100-150`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if store == nil {
			if daemonClient != nil {
				...
				fmt.Fprintf(os.Stderr, "Hint: use --no-daemon flag: fbd --no-daemon mol current\n")
			} else {
		...
		if len(args) == 1 {
			...
			if len(molecules) == 0 {
				...
				fmt.Println("  fbd mol pour <proto-id>      # Instantiate a molecule from template")
				fmt.Println("  fbd update <step-id> --status in_progress  # Claim a step")
				return
			}
		}
...
func printMoleculeProgress(mol *MoleculeProgress) {
	...
	if mol.NextStep != nil && mol.CurrentStep == nil {
		...
		fmt.Printf("  Start with: fbd update %s --status in_progress\n", mol.NextStep.ID)
	}
	...
	if hintStepID != "" {
		fmt.Printf("\n%s Run `fbd show %s` to see detailed instructions.\n", ui.RenderAccent("💡"), hintStepID)
	}
}
...
func PrintContinueResult(result *ContinueResult) {
	...
	if result.MolComplete {
		...
		fmt.Println("Consider: fbd mol squash " + result.MoleculeID + " --summary '...'")
		return
	}
	...
	if result.AutoAdvanced {
		...
		fmt.Printf("\nStart with: fbd update %s --status in_progress\n", result.NextStep.ID)
	}
}
...
func printLargeMoleculeSummary(stats *types.MoleculeProgressStats) {
	...
	fmt.Printf("  fbd mol current %s --limit 50        # First 50 steps\n", stats.MoleculeID)
	fmt.Printf("  fbd mol current %s --range 1-50     # Steps 1-50\n", stats.MoleculeID)
	fmt.Printf("  fbd mol progress %s                 # Efficient progress summary\n", stats.MoleculeID)
