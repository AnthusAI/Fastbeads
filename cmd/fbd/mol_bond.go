...
Dynamic bonding (Christmas Ornament pattern):
  ...
  Example:
    fbd mol bond mol-polecat-arm bd-patrol --ref arm-{{polecat_name}} --var polecat_name=ace
...
Examples:
  fbd mol bond mol-feature mol-deploy                    # Compound proto
  fbd mol bond mol-feature mol-deploy --type parallel    # Run in parallel
  fbd mol bond mol-feature bd-abc123                     # Attach proto to molecule
  fbd mol bond bd-abc123 bd-def456                       # Join two molecules
  fbd mol bond mol-critical-bug wisp-patrol --pour       # Persist found bug
  fbd mol bond mol-temp-check bd-feature --ephemeral          # Ephemeral diagnostic
  fbd mol bond mol-arm bd-patrol --ref arm-{{name}} --var name=ace  # Dynamic child ID`,
