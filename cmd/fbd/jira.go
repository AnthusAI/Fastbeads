...
Configuration:
  ...
  fbd config set jira.pull_prefix "hippo"       # Imported issues get hippo-1, hippo-2, etc.
  fbd config set jira.push_prefix "hippo"       # Only push hippo-* issues to Jira
  fbd config set jira.push_prefix "proj1,proj2" # Multiple prefixes (comma-separated)
...
Examples:
  fbd jira sync --pull         # Import issues from Jira
  fbd jira sync --push         # Export issues to Jira
  fbd jira sync                # Bidirectional sync (pull then push)
  fbd jira sync --dry-run      # Preview sync without changes
  fbd jira status              # Show sync status`,
}
...
Examples:
  fbd jira sync --pull                # Import from Jira
  fbd jira sync --push --create-only  # Push new issues only
  fbd jira sync --dry-run             # Preview without changes
  fbd jira sync --prefer-local        # Bidirectional, local wins`,
  ...
  - Issues pending push (no external_ref)`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !configured {
			...
			fmt.Println("  fbd config set jira.url \"https://company.atlassian.net\"")
			fmt.Println("  fbd config set jira.project \"PROJ\"")
			fmt.Println("  fbd config set jira.api_token \"YOUR_TOKEN\"")
			fmt.Println("  fbd config set jira.username \"your@email.com\"")
			return
		}
		...
		if pendingPush > 0 {
			fmt.Println()
			fmt.Printf("Run 'fbd jira sync --push' to push %d local issue(s) to Jira\n", pendingPush)
		}
	},
}
