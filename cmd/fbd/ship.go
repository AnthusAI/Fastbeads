...
Examples:
  fbd ship mol-run-assignee              # Ship the mol-run-assignee capability
  fbd ship mol-run-assignee --force      # Ship even if issue is not closed
  fbd ship mol-run-assignee --dry-run    # Preview without making changes`,
...
func runShip(cmd *cobra.Command, args []string) {
	...
	if len(issues) == 0 {
		...
		fmt.Fprintf(os.Stderr, "Hint: add the label first: fbd label add <issue-id> %s\n", exportLabel)
		os.Exit(1)
	}
