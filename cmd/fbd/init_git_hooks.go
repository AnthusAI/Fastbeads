...
// hooksInstalled checks if fbd git hooks are installed
func hooksInstalled() bool {
	...
	// Verify they're fbd hooks by checking for signature comment
	// #nosec G304 - controlled path from git directory
	preCommitContent, err := os.ReadFile(preCommit)
	if err != nil || !strings.Contains(string(preCommitContent), "fbd (beads) pre-commit hook") {
		return false
	}

	// #nosec G304 - controlled path from git directory
	postMergeContent, err := os.ReadFile(postMerge)
	if err != nil || !strings.Contains(string(postMergeContent), "fbd (beads) post-merge hook") {
		return false
	}
...
func detectExistingHooks() []hookInfo {
	...
	for i := range hooks {
		...
		if err == nil {
			...
			hooks[i].isBdHook = strings.Contains(hooks[i].content, "fbd (beads)")
			// Only detect pre-commit/prek framework if not a fbd hook
...
func installGitHooks() error {
	...
	// Check if any non-fbd hooks exist
	...
	if chainHooks {
		fmt.Printf("%s Chained fbd hooks with existing hooks\n", ui.RenderPass("✓"))
	}
...
func buildPreCommitHook(chainHooks bool, existingHooks []hookInfo) string {
	if chainHooks {
		...
		return `#!/bin/sh
#
# fbd (beads) pre-commit hook (chained)
#
# This hook chains fbd functionality with your existing pre-commit hook.
...
func preCommitHookBody() string {
	return `# Check if fbd is available
...
if ! fbd sync --flush-only >/dev/null 2>&1; then
    echo "Error: Failed to flush fbd changes to storage" >&2
...
func buildPostMergeHook(chainHooks bool, existingHooks []hookInfo) string {
	if chainHooks {
		...
		return `#!/bin/sh
#
# fbd (beads) post-merge hook (chained)
#
# This hook chains fbd functionality with your existing post-merge hook.
...
func postMergeHookBody() string {
	return `# Check if fbd is available
if ! command -v fbd >/dev/null 2>&1; then
    echo "Warning: fbd command not found, skipping post-merge import" >&2
    exit 0
fi
...
if ! fbd import -i "$BEADS_DIR/issues.jsonl" >/dev/null 2>&1; then
    echo "Warning: Failed to import fbd changes after merge" >&2
    echo "Run 'fbd import -i $BEADS_DIR/issues.jsonl' manually to see the error" >&2
fi
...
// mergeDriverInstalled checks if fbd merge driver is configured correctly
// Note: This runs during fbd init BEFORE .beads exists, so it runs git in CWD.
...
func installJJHooks() error {
	...
	if chainHooks {
		fmt.Printf("%s Chained fbd hooks with existing hooks (jj mode)\n", ui.RenderPass("✓"))
	}
...
func buildJJPreCommitHook(chainHooks bool, existingHooks []hookInfo) string {
	if chainHooks {
		...
		return `#!/bin/sh
#
# fbd (beads) pre-commit hook (chained, jujutsu mode)
#
# This hook chains fbd functionality with your existing pre-commit hook.
...
` + jjPreCommitHookBody()
	...
	return `#!/bin/sh
#
# fbd (beads) pre-commit hook (jujutsu mode)
#
# This hook ensures that any pending fbd issue changes are flushed to
...
func printJJAliasInstructions() {
	...
	fmt.Printf("  %s\n", ui.RenderAccent(`push = ["util", "exec", "--", "sh", "-c", "fbd sync --flush-only && jj git push \"$@\"", ""]`))
	fmt.Printf("\nThen use %s instead of %s\n\n", ui.RenderAccent("jj push"), ui.RenderAccent("jj git push"))
	fmt.Printf("For more details, see: https://github.com/steveyegge/fastbeads/blob/main/docs/JUJUTSU.md\n\n")
}

// installMergeDriver configures git to use fbd merge for JSONL files
// Note: This runs during fbd init BEFORE .beads exists, so it runs git in CWD.
func installMergeDriver() error {
	// Configure git merge driver (runs in CWD)
	cmd := exec.Command("git", "config", "merge.beads.driver", "fbd merge %A %O %A %B")
	if output, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to configure git merge driver: %w\n%s", err, output)
	}

	cmd = exec.Command("git", "config", "merge.beads.name", "fbd JSONL merge driver")
	if output, err := cmd.CombinedOutput(); err != nil {
	...
	if !hasBeadsMerge {
		...
		beadsMergeAttr := "\n# Use fbd merge for beads JSONL files\n.beads/issues.jsonl merge=beads\n"
