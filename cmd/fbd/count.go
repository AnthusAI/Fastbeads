...
Examples:
  fbd count                          # Count all issues
  fbd count --status open            # Count open issues
  fbd count --by-status              # Group count by status
  fbd count --by-priority            # Group count by priority
  fbd count --by-type                # Group count by issue type
  fbd count --by-assignee            # Group count by assignee
  fbd count --by-label               # Group count by label
  fbd count --assignee alice --by-status  # Count alice's issues by status
`,
