...
NOTE: Import requires direct database access and does not work with daemon mode.
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Check for positional arguments (common mistake: fbd import file.jsonl instead of fbd import -i file.jsonl)
		if len(args) > 0 {
			fmt.Fprintf(os.Stderr, "Error: Unexpected argument(s): %v\n\n", args)
			fmt.Fprintf(os.Stderr, "Did you mean: fbd import -i %s\n\n", args[0])
			...
			fmt.Fprintf(os.Stderr, "  fbd import -i .beads/issues.jsonl\n\n")
			fmt.Fprintf(os.Stderr, "Or pipe data via stdin:\n")
			fmt.Fprintf(os.Stderr, "  cat data.jsonl | fbd import\n")
			os.Exit(1)
		}
		...
		_ = noGitHistory // Accepted for compatibility with fbd sync subprocess calls
		...
		if input == "" && term.IsTerminal(int(os.Stdin.Fd())) {
			...
			fmt.Fprintf(os.Stderr, "  fbd import -i .beads/issues.jsonl          # Import from file\n")
			fmt.Fprintf(os.Stderr, "  fbd import -i .beads/issues.jsonl --dry-run # Preview changes\n")
			fmt.Fprintf(os.Stderr, "  cat data.jsonl | fbd import               # Import from pipe\n")
			fmt.Fprintf(os.Stderr, "  fbd sync --import-only                    # Import latest JSONL\n\n")
			fmt.Fprintf(os.Stderr, "For more information, run: fbd import --help\n")
			os.Exit(1)
		}
		...
		for scanner.Scan() {
			...
			if bytes.HasPrefix(trimmed, []byte("<<<<<<< ")) ||
				...
				// Attempt automatic merge using fbd merge command
				if err := attemptAutoMerge(input); err != nil {
					...
					fmt.Fprintf(os.Stderr, "  git checkout --ours .beads/issues.jsonl && fbd import -i .beads/issues.jsonl\n")
					fmt.Fprintf(os.Stderr, "  git checkout --theirs .beads/issues.jsonl && fbd import -i .beads/issues.jsonl\n\n")
		...
		if err != nil {
			...
			if result != nil && result.PrefixMismatch {
				...
				fmt.Fprintf(os.Stderr, "\nOr use 'fbd rename-prefix' after import to fix the database.\n")
				os.Exit(1)
			}
		...
		// This ensures that running `fbd import` marks the database as fresh for staleness detection
		...
		// 2. Without mtime update, fbd sync refuses to export (thinks JSONL is newer)
		...
		if dedupeAfter {
			...
			for i, group := range duplicateGroups {
				...
				fmt.Fprintf(os.Stderr, "  Suggested: fbd merge %s --into %s\n\n",
					strings.Join(sources, " "), target.ID)
			}

			fmt.Fprintf(os.Stderr, "Run 'fbd duplicates --auto-merge' to merge all duplicates.\n")
		}
	},
}
...
// preventing fbd doctor and validatePreExport from incorrectly warning that JSONL is newer.
//
...
// attemptAutoMerge attempts to resolve git conflicts using fbd merge 3-way merge.
...
func attemptAutoMerge(conflictedPath string) error {
	...
	// Get current executable to call fbd merge
	exe, err := os.Executable()
	if err != nil {
		return fmt.Errorf("cannot resolve current executable: %w", err)
	}

	// Invoke fbd merge command
	mergeCmd := exec.Command(exe, "merge", outputPath, basePath, leftPath, rightPath) // #nosec G204 -- executes current fbd binary for deterministic merge
	mergeOutput, err := mergeCmd.CombinedOutput()
	if err != nil {
		// Check exit code - fbd merge returns 1 if there are conflicts, 2 for errors
...
func init() {
	...
	importCmd.Flags().Bool("no-git-history", false, "Skip git history backfill for deletions (passed by fbd sync)")
