...
Examples:
  fbd migrate-tombstones           # Migrate deletions to tombstones
  fbd migrate-tombstones --dry-run # Preview what would be migrated
  fbd migrate-tombstones --verbose # Show detailed progress`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if jsonOutput {
			...
			fmt.Println("\nNext steps:")
			fmt.Println("  1. Run 'fbd sync' to propagate tombstones to remote")
...
func init() {
	...
	migrateTombstonesAliasCmd.Deprecated = "use 'fbd migrate tombstones' instead (will be removed in v1.0.0)"
