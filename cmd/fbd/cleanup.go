...
// showCleanupDeprecationHint shows a hint about fbd doctor --fix
func showCleanupDeprecationHint() {
	fmt.Fprintln(os.Stderr, ui.RenderMuted("💡 Tip: 'fbd doctor --fix' can now cleanup stale issues and prune tombstones"))
}
...
It does NOT remove temporary files - use 'fbd clean' for that.
...
Delete issues closed more than 30 days ago:
  fbd cleanup --older-than 30 --force
...
Preview what would be deleted/pruned:
  fbd cleanup --dry-run
  fbd cleanup --older-than 90 --dry-run

Hard delete: permanently remove issues/tombstones older than 3 days:
  fbd cleanup --older-than 3 --hard --force
...
SEE ALSO:
  fbd clean      Remove temporary git merge artifacts
  fbd compact    Run compaction on issues`,
