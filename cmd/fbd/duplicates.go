...
Example:
  fbd duplicates                    # Show all duplicate groups
  fbd duplicates --auto-merge       # Automatically merge all duplicates
  fbd duplicates --dry-run          # Show what would be merged`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		for _, group := range duplicateGroups {
			...
			cmd := fmt.Sprintf("# Duplicate: %s (same content as %s)\n# Suggested action: fbd close %s && fbd dep add %s %s --type related",
		...
		if jsonOutput {
			...
			for i, group := range duplicateGroups {
				...
				fmt.Printf("  %s fbd close %s && fbd dep add %s %s --type related\n\n",
...
func formatDuplicateGroupsJSON(groups [][]*types.Issue, refCounts map[string]int, structuralScores map[string]*issueScore) []map[string]interface{} {
	...
	for _, group := range groups {
		...
		result = append(result, map[string]interface{}{
			...
			"suggested_action":  fmt.Sprintf("fbd close %s && fbd dep add %s %s --type related", strings.Join(sources, " "), strings.Join(sources, " "), target.ID),
