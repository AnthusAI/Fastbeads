...
func TestScripts(t *testing.T) {
	...
	// Create minimal engine with default commands plus fbd
	timeout := 2 * time.Second
	engine := script.NewEngine()
	engine.Cmds["fbd"] = script.Program(exe, nil, timeout)

	// Add binDir to PATH so 'sh -c fbd ...' works in test scripts
