...
the issues will not resurrect from remote branches.`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !force {
			...
			fmt.Printf("To proceed, run: %s\n\n", ui.RenderWarn("fbd delete "+issueID+" --force"))
			return
		}
		// Actually delete
...
func deleteBatch(_ *cobra.Command, issueIDs []string, force bool, dryRun bool, cascade bool, jsonOutput bool, hardDelete bool, reason string) {
		...
		if dryRun {
			...
			if cascade {
				fmt.Printf("To proceed with cascade deletion, run: %s\n",
					ui.RenderWarn("fbd delete "+strings.Join(issueIDs, " ")+" --cascade --force"))
			} else {
				fmt.Printf("To proceed, run: %s\n",
					ui.RenderWarn("fbd delete "+strings.Join(issueIDs, " ")+" --force"))
			}
		}
		return
	}
	...
	// Use 'fbd cleanup --hard' after syncing to fully purge old tombstones.
	if hardDelete {
		if !jsonOutput {
			fmt.Println(ui.RenderWarn("⚠️  HARD DELETE MODE: Pruning tombstones from JSONL"))
			fmt.Println("  Note: Tombstones kept in DB to prevent resurrection. Run 'fbd sync' then 'fbd cleanup --hard' to fully purge.")
		}
