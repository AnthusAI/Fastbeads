...
// GetSyncMode returns the configured sync mode, checking config.yaml first (where fbd config set writes),
...
func GetSyncMode(ctx context.Context, s storage.Storage) string {
	// First check config.yaml (where fbd config set writes for sync.* keys)
...
// respected even if not yet propagated to the database via 'fbd sync mode set'.
