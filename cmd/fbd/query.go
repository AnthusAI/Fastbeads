...
import (
	...
	"github.com/steveyegge/fastbeads/internal/query"
...
Examples:
  fbd query "status=open AND priority>1"
  fbd query "status=open AND priority<=2 AND updated>7d"
  fbd query "(status=open OR status=blocked) AND priority<2"
  fbd query "type=bug AND label=urgent"
  fbd query "NOT status=closed"
  fbd query "assignee=none AND type=task"
  fbd query "created>30d AND status!=closed"
  fbd query "label=frontend OR label=backend"
  fbd query "title=authentication AND priority=0"`,
