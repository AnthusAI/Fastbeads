...
func gitCommit(ctx context.Context, filePath string, message string) error {
	...
	if message == "" {
		message = fmt.Sprintf("fbd sync: %s", time.Now().Format("2006-01-02 15:04:05"))
	}
...
// This ensures fbd sync doesn't accidentally commit other staged files.
...
// This allows respecting the sync.remote fbd config.
// Uses RepoContext to ensure git commands run in the correct repository.
func gitPull(ctx context.Context, configuredRemote string) error {
	...
	// 1. If configuredRemote (from sync.remote fbd config) is set, use that
...
func showPushPermissionGuidance() {
	...
	fmt.Fprintln(os.Stderr, "  fbd init --contributor")
	fmt.Fprintln(os.Stderr, "")
...
func gitPush(ctx context.Context, configuredRemote string) error {
	...
	if configuredRemote != "" {
		...
		cmd := rc.GitCmd(ctx, "push", configuredRemote, branch) //nolint:gosec // G204: configuredRemote from fbd config
		output, err := cmd.CombinedOutput()
		if err != nil {
...
func checkMergeDriverConfig() {
	...
	if strings.Contains(currentConfig, "%L") || strings.Contains(currentConfig, "%R") {
		...
		fmt.Fprintf(os.Stderr, "\n   Fix now: fbd doctor --fix\n")
		fmt.Fprintf(os.Stderr, "   Or manually: git config merge.beads.driver \"fbd merge %%A %%O %%A %%B\"\n\n")
	}
}
