...
Examples:
  ...
  fbd migrate-issues --from ~/.beads-planning --to . --dry-run

  # Move all open P1 bugs
  fbd migrate-issues --from ~/repo1 --to ~/repo2 --priority 1 --type bug --status open

  # Move specific issues with their dependencies
  fbd migrate-issues --from . --to ~/archive --id bd-abc --id bd-xyz --include closure

  # Move issues with label filter
  fbd migrate-issues --from . --to ~/feature-work --label frontend --label urgent`,
...
func init() {
	...
	migrateIssuesAliasCmd.Deprecated = "use 'fbd migrate issues' instead (will be removed in v1.0.0)"
