...
// trackBdVersion checks if fbd version has changed since last run and updates the local version file.
...
func trackBdVersion() {
	...
	if beadsDir == "" {
		// No .beads directory found - this is fine (e.g., fbd init, fbd version, etc.)
		return
	}
...
// readLocalVersion reads the last fbd version from the local version file.
...
// This is called by commands like 'fbd ready' and 'fbd list' to inform users of upgrades.
func maybeShowUpgradeNotification() {
	...
	fmt.Printf("🔄 fbd upgraded from v%s to v%s since last use\n", previousVersion, Version)
	fmt.Println("💡 Run 'fbd upgrade review' to see what changed")
	fmt.Println("💊 Run 'fbd doctor' to verify upgrade completed cleanly")
