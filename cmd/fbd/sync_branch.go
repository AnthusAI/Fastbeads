...
func getSyncBranch(ctx context.Context) (string, error) {
	...
	if syncBranch == "" {
		return "", fmt.Errorf("sync.branch not configured (run 'fbd config set sync.branch <branch-name>')")
	}
...
func mergeSyncBranch(ctx context.Context, dryRun bool) error {
	...
	// The architectural fix ensures daemon/fbd writes ONLY to the worktree JSONL
	...
	fmt.Println("2. Run 'fbd sync --import-only' to sync the database with merged JSONL")
	fmt.Println("3. Run 'fbd sync' to push changes to remote")
...
// Contributed by dand-oss (https://github.com/steveyegge/fastbeads/pull/533)
//
