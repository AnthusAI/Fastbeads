...
fbd has 70+ commands - many for AI agents, integrations, and advanced workflows.
This command shows the ~15 essential commands that human users need most often.

For the full command list, run: fbd --help
...
SUBCOMMANDS:
  ...
  human stats             Show summary statistics for human-needed beads`,
	Run: func(cmd *cobra.Command, args []string) {
		fmt.Printf("\n%s\n", ui.RenderBold("fbd - Essential Commands for Humans"))
		fmt.Printf("For all 70+ commands: fbd --help\n\n")
		...
		printCmd("init", "Initialize fbd in current directory")
		...
		fmt.Printf("  fbd create \"Fix login bug\" --priority 1\n")
		fmt.Printf("  fbd update bd-abc123 --status in_progress\n")
		fmt.Printf("  fbd close bd-abc123\n\n")

		fmt.Printf("  %s\n", ui.RenderMuted("# See what needs doing"))
		fmt.Printf("  fbd ready                    # What can I work on?\n")
		fmt.Printf("  fbd list --status open       # All open issues\n")
		fmt.Printf("  fbd blocked                  # What's stuck?\n\n")
	},
}
...
Examples:
  fbd human respond bd-123 --response "Use OAuth2 for authentication"
  fbd human respond bd-123 -r "Approved, proceed with implementation"`,
...
Examples:
  fbd human dismiss bd-123
  fbd human dismiss bd-123 --reason "No longer applicable"`,
	Args: cobra.ExactArgs(1),
	Run: func(cmd *cobra.Command, args []string) {
