...
Examples:
  fbd export --status open -o open-issues.jsonl
  fbd export --format obsidian                    # outputs to ai_docs/changes-log.md
  fbd export --format obsidian -o custom.md       # outputs to custom.md
  fbd export --type bug --priority-max 1
  fbd export --created-after 2025-01-01 --assignee alice`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if output != "" && !force && !isFilteredExport {
			...
			if err == nil && len(jsonlIDs) > 0 {
				...
				if len(missingIDs) > 0 {
					...
					fmt.Fprintf(os.Stderr, "  1. You need to run 'fbd import -i %s' to sync the latest changes\n", output)
		...
		// This prevents clearing dirty flags when exporting to stdout or custom paths (e.g., fbd export -o backup.jsonl)
