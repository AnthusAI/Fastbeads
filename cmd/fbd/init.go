...
import (
	...
	"github.com/steveyegge/fastbeads/cmd/fbd/doctor"
...
var initCmd = &cobra.Command{
	Use:     "init",
	GroupID: "setup",
	Short:   "Initialize fbd in the current directory",
	Long: `Initialize fbd in the current directory by creating a .beads/ directory
...
of scanning git history. Use this after manual JSONL cleanup (e.g., fbd compact --purge-tombstones)
...
With --stealth: configures per-repository git settings for invisible beads usage:
  ...
  • Claude Code settings with fbd onboard instruction
...
--server-port, and --server-user. Password should be set via FBD_DOLT_PASSWORD/BEADS_DOLT_PASSWORD environment
variable.`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		// Check FBD_DB/BEADS_DB environment variable if --db flag not set
		...
		// FBD_DIR/BEADS_DIR takes precedence, otherwise use CWD/.beads (with redirect support).
		...
		if envBeadsDir := env.GetEnvAlias("DIR"); envBeadsDir != "" {
		...
		//
		// Precedence: --db > FBD_DB/BEADS_DB > FBD_DIR/BEADS_DIR > default (.beads/beads.db)
		...
		if isWorktree {
			...
			fmt.Fprintf(os.Stderr, "Error: cannot run 'fbd init' from within a git worktree\n\n")
			...
			fmt.Fprintf(os.Stderr, "     fbd init\n\n")
			fmt.Fprintf(os.Stderr, "  2. Then create worktrees with beads support:\n")
			fmt.Fprintf(os.Stderr, "     fbd worktree create <path> --branch <branch-name>\n\n")
			fmt.Fprintf(os.Stderr, "For more information, see: https://github.com/steveyegge/fastbeads/blob/main/docs/WORKTREES.md\n")
			os.Exit(1)
		}
		...
		if strings.Contains(filepath.Clean(cwd), string(filepath.Separator)+".beads"+string(filepath.Separator)) ||
			...
			fmt.Fprintf(os.Stderr, "Error: cannot initialize fbd inside a .beads directory\n")
			fmt.Fprintf(os.Stderr, "Current directory: %s\n", cwd)
			fmt.Fprintf(os.Stderr, "Please run 'fbd init' from outside the .beads directory.\n")
			os.Exit(1)
		}
		...
		if useLocalBeads {
			...
			if noDb {
				...
				cfg.Storage = configfile.StorageJSONL
				...
				if !quiet {
					fmt.Printf("\n%s fbd initialized successfully in --no-db mode!\n\n", ui.RenderPass("✓"))
					...
					fmt.Printf("Run %s to get started.\n\n", ui.RenderAccent("fbd --no-db quickstart"))
				}
				return
			}
		...
		// Store the fbd version in metadata (for version mismatch detection)
		...
		if useLocalBeads {
			...
			if backend == configfile.BackendDolt {
				...
				// E.g., prefix "gt" → database "beads_gt", prefix "fbd" → database "beads_bd".
				if prefix != "" {
		...
		// the worktree created by fbd sync will check out main, preventing the user from
		// checking out main in their working directory (git error: "'main' is already checked out")
		//
		// When --branch is not specified, fbd sync will commit directly to the current branch
				...
				if issueCount > 0 {
					...
					if err := importFromGit(ctx, initDBPath, store, jsonlPath, gitRef); err != nil {
						if !quiet {
							...
							fmt.Fprintf(os.Stderr, "Try manually: git show %s:%s | fbd import -i /dev/stdin\n", gitRef, jsonlPath)
						}
						// Non-fatal - continue with empty database
					} else if !quiet {
		...
		if !skipHooks && !hooksInstalled() {
			...
			if isJJ && !isColocated {
				...
				if err := installJJHooks(); err != nil && !quiet {
					...
					fmt.Fprintf(os.Stderr, "You can try again with: %s\n\n", ui.RenderAccent("fbd doctor --fix"))
				} else if !quiet {
				...
				if backend == configfile.BackendDolt {
					...
					if err == nil {
						if err := installHooksWithOptions(embeddedHooks, false, false, false, true); err != nil && !quiet {
							...
							fmt.Fprintf(os.Stderr, "You can try again with: %s\n\n", ui.RenderAccent("fbd hooks install --beads"))
						} else if !quiet {
		...
		// Only install merge driver for JSONL storage mode.
		if !skipMergeDriver && isGitRepo() && !mergeDriverInstalled() {
			installDriver := false
			if cfg, err := configfile.Load(beadsDir); err == nil {
				if cfg == nil {
					cfg = configfile.DefaultConfig()
				}
				if mode, err := cfg.ResolveStorageMode(beadsDir); err == nil && mode == configfile.StorageJSONL {
					installDriver = true
				}
			}
			if installDriver {
		...
		fmt.Printf("\n%s fbd initialized successfully!\n\n", ui.RenderPass("✓"))
		...
		fmt.Printf("  Issue prefix: %s\n", ui.RenderAccent(prefix))
		fmt.Printf("  Issues will be named: %s\n\n", ui.RenderAccent(prefix+"-<hash> (e.g., "+prefix+"-a3f2dd)"))
		fmt.Printf("Run %s to get started.\n\n", ui.RenderAccent("fbd quickstart"))

		// Run fbd doctor diagnostics to catch setup issues early
...
func readFirstIssueFromJSONL(path string) (*types.Issue, error) {
	// #nosec G304 -- helper reads JSONL file chosen by current fbd command
...
To use the existing database:
  Just run fbd commands normally (e.g., %s)

To completely reinitialize (data loss warning):
  rm -rf %s && fbd init --backend dolt --prefix %s

Aborting.`, ui.RenderWarn("⚠"), location, ui.RenderAccent("fbd list"), beadsDir, prefix)
		}
	}
...
To reinitialize the canonical location (data loss warning):
  rm %s && fbd init --prefix %s

Aborting.`, ui.RenderWarn("⚠"), redirectTarget, targetDBPath, ui.RenderAccent("fbd list"), targetDBPath, prefix)
		}
...
To completely reinitialize (data loss warning):
  rm -rf %s && fbd init --prefix %s

Aborting.`, ui.RenderWarn("⚠"), dbPath, ui.RenderAccent("fbd list"), beadsDir, prefix)
	}
...
func checkExistingBeadsData(prefix string) error {
	// Check FBD_DIR/BEADS_DIR environment variable first (matches FindBeadsDir pattern)
	// When FBD_DIR/BEADS_DIR is set, it takes precedence over CWD and worktree checks
	if envBeadsDir := env.GetEnvAlias("DIR"); envBeadsDir != "" {
