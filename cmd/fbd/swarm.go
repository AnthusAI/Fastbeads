// Package main implements the fbd CLI swarm management commands.
package main
...
Examples:
  fbd swarm validate gt-epic-123           # Validate epic structure
  fbd swarm validate gt-epic-123 --verbose # Include detailed issue graph`,
...
Examples:
  fbd swarm status gt-epic-123       # Show swarm status by epic
  fbd swarm status gt-swarm-456      # Show status via swarm molecule
  fbd swarm status gt-epic-123 --json  # Machine-readable output`,
	Args: cobra.ExactArgs(1),
	Run: func(cmd *cobra.Command, args []string) {
		ctx := rootCtx
...
Examples:
  fbd swarm create gt-epic-123                          # Create swarm for epic
  fbd swarm create gt-epic-123 --coordinator=witness/   # With specific coordinator
  fbd swarm create gt-task-456                          # Auto-wrap single issue`,
	Args: cobra.ExactArgs(1),
	Run: func(cmd *cobra.Command, args []string) {
...
Examples:
  fbd swarm list         # List all swarms
  fbd swarm list --json  # Machine-readable output`,
	Run: func(cmd *cobra.Command, args []string) {
		ctx := rootCtx
