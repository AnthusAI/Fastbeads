...
func TestSQLiteToDolt_JSONLRoundTrip(t *testing.T) {
	...
	if initErr != nil {
		...
		t.Fatalf("fbd init --backend dolt failed: %v\n%s", initErr, initOut)
	}
	...
	if out, err := runBDExecAllowErrorWithEnv(t, ws2, env, "import", "-i", jsonl2in); err != nil {
		t.Fatalf("fbd import (dolt) failed: %v\n%s", err, out)
	}
	...
	if out, err := runBDExecAllowErrorWithEnv(t, ws2, env, "export", "-o", jsonl2out); err != nil {
		t.Fatalf("fbd export (dolt) failed: %v\n%s", err, out)
	}
...
func TestDoltToSQLite_JSONLRoundTrip(t *testing.T) {
	...
	// Inject tombstone record for B into JSONL (Dolt backend may not support fbd delete tombstones).
	now := time.Now().UTC()
	...
	if out, err := runBDExecAllowErrorWithEnv(t, ws2, env, "import", "-i", jsonl2in); err != nil {
		t.Fatalf("fbd import (sqlite) failed: %v\n%s", err, out)
	}

	jsonl2out := filepath.Join(ws2, ".beads", "roundtrip.jsonl")
	if out, err := runBDExecAllowErrorWithEnv(t, ws2, env, "export", "-o", jsonl2out); err != nil {
		t.Fatalf("fbd export (sqlite) failed: %v\n%s", err, out)
	}
