...
import (
	...
	internalbeads "github.com/steveyegge/fastbeads/internal/beads"
...
agents from forgetting fbd workflow after context compaction.

Config options:
- no-git-ops: When true, outputs stealth mode (no git commands in session close protocol).
  Set via: fbd config set no-git-ops true
...
func outputMCPContext(w io.Writer, stealthMode bool) error {
	...
	if stealthMode || localOnly {
		...
		closeProtocol = "Before saying \"done\": fbd sync --flush-only"
	} else if autoSync && !ephemeral && !noPush {
		// Daemon is auto-syncing - no fbd sync needed
		closeProtocol = "Before saying \"done\": git status → git add → git commit → git push (beads auto-synced by daemon)"
	} else if ephemeral {
		closeProtocol = "Before saying \"done\": git status → git add → fbd sync --from-main → git commit (no push - ephemeral branch)"
	} else if noPush {
		closeProtocol = "Before saying \"done\": git status → git add → fbd sync → git commit (push disabled - run git push manually)"
	} else {
		closeProtocol = "Before saying \"done\": git status → git add → fbd sync → git commit → fbd sync → git push"
	}
...
## Core Rules
- **Default**: Use beads for ALL task tracking (` + "`fbd create`" + `, ` + "`fbd ready`" + `, ` + "`fbd close`" + `)
...
func outputCLIContext(w io.Writer, stealthMode bool) error {
	...
	if stealthMode || localOnly {
		// Stealth mode or local-only: only flush to JSONL, no git operations
		closeProtocol = `[ ] fbd sync --flush-only    (export beads to JSONL only)`
		syncSection = `### Sync & Collaboration
- ` + "`fbd sync --flush-only`" + ` - Export to JSONL`
		completingWorkflow = `**Completing work:**
` + "```bash" + `
fbd close <id1> <id2> ...    # Close all completed issues at once
fbd sync --flush-only        # Export to JSONL
` + "```"
		...
		// Daemon is auto-syncing - simplified protocol (no fbd sync needed)
		closeProtocol = `[ ] 1. git status              (check what changed)
[ ] 2. git add <files>         (stage code changes)
[ ] 3. git commit -m "..."     (commit code)
[ ] 4. git push                (push to remote)`
		closeNote = "**Note:** Daemon is auto-syncing beads changes. No manual `fbd sync` needed."
		syncSection = `### Sync & Collaboration
- Daemon handles beads sync automatically (auto-commit + auto-push + auto-pull enabled)
- ` + "`fbd sync --status`" + ` - Check sync status`
		completingWorkflow = `**Completing work:**
` + "```bash" + `
fbd close <id1> <id2> ...    # Close all completed issues at once
...
` + "```"
		...
		closeProtocol = `[ ] 1. git status              (check what changed)
[ ] 2. git add <files>         (stage code changes)
[ ] 3. fbd sync --from-main     (pull beads updates from main)
[ ] 4. git commit -m "..."     (commit code changes)`
		closeNote = "**Note:** This is an ephemeral branch (no upstream). Code is merged to main locally, not pushed."
		syncSection = `### Sync & Collaboration
- ` + "`fbd sync --from-main`" + ` - Pull beads updates from main (for ephemeral branches)
- ` + "`fbd sync --status`" + ` - Check sync status without syncing`
		completingWorkflow = `**Completing work:**
` + "```bash" + `
fbd close <id1> <id2> ...    # Close all completed issues at once
fbd sync --from-main         # Pull latest beads from main
git add . && git commit -m "..."  # Commit your changes
# Merge to main when ready (local merge, not push)
` + "```"
		gitWorkflowRule = "Git workflow: run `fbd sync --from-main` at session end"
	} else if noPush {
		closeProtocol = `[ ] 1. git status              (check what changed)
[ ] 2. git add <files>         (stage code changes)
[ ] 3. fbd sync                 (commit beads changes)
[ ] 4. git commit -m "..."     (commit code)
[ ] 5. fbd sync                 (commit any new beads changes)`
		closeNote = "**Note:** Push disabled via config. Run `git push` manually when ready."
		syncSection = `### Sync & Collaboration
- ` + "`fbd sync`" + ` - Sync with git remote (run at session end)
- ` + "`fbd sync --status`" + ` - Check sync status without syncing`
		completingWorkflow = `**Completing work:**
` + "```bash" + `
fbd close <id1> <id2> ...    # Close all completed issues at once
fbd sync                     # Sync beads (push disabled)
# git push                  # Run manually when ready
` + "```"
		gitWorkflowRule = "Git workflow: run `fbd sync` at session end (push disabled)"
	} else {
		closeProtocol = `[ ] 1. git status              (check what changed)
[ ] 2. git add <files>         (stage code changes)
[ ] 3. fbd sync                 (commit beads changes)
...
` + "```bash" + `
fbd close <id1> <id2> ...    # Close all completed issues at once
fbd sync                     # Push to remote
` + "```"
		gitWorkflowRule = "Git workflow: hooks auto-sync, run `fbd sync` at session end"
	}
...
> **Context Recovery**: Run ` + "`fbd prime`" + ` after compaction, clear, or new session
...
- Persistence you don't need beats lost context
- ` + gitWorkflowRule + `
- Session management: check ` + "`fbd ready`" + ` for available work

## Essential Commands

### Finding Work
- ` + "`fbd ready`" + ` - Show issues ready to work (no blockers)
- ` + "`fbd list --status=open`" + ` - All open issues
- ` + "`fbd list --status=in_progress`" + ` - Your active work
- ` + "`fbd show <id>`" + ` - Detailed issue view with dependencies

### Creating & Updating
- ` + "`fbd create --title=\"Summary of this issue\" --description=\"Why this issue exists and what needs to be done\" --type=task|bug|feature --priority=2`" + ` - New issue
  - Priority: 0-4 or P0-P4 (0=critical, 2=medium, 4=backlog). NOT "high"/"medium"/"low"
- ` + "`fbd update <id> --status=in_progress`" + ` - Claim work
- ` + "`fbd update <id> --assignee=username`" + ` - Assign to someone
- ` + "`fbd update <id> --title/--description/--notes/--design`" + ` - Update fields inline
- ` + "`fbd close <id>`" + ` - Mark complete
- ` + "`fbd close <id1> <id2> ...`" + ` - Close multiple issues at once (more efficient)
- ` + "`fbd close <id> --reason=\"explanation\"`" + ` - Close with reason
- **Tip**: When creating multiple issues/tasks/epics, use parallel subagents for efficiency
- **WARNING**: Do NOT use ` + "`fbd edit`" + ` - it opens $EDITOR (vim/nano) which blocks agents

### Dependencies & Blocking
- ` + "`fbd dep add <issue> <depends-on>`" + ` - Add dependency (issue depends on depends-on)
- ` + "`fbd blocked`" + ` - Show all blocked issues
- ` + "`fbd show <id>`" + ` - See what's blocking/blocked by this issue

` + syncSection + `

### Project Health
- ` + "`fbd stats`" + ` - Project statistics (open/closed/blocked counts)
- ` + "`fbd doctor`" + ` - Check for issues (sync problems, missing hooks)
...
fbd show <id>       # Review issue details
fbd update <id> --status=in_progress  # Claim it
` + "```" + `
...
` + "```bash" + `
# Run fbd create commands in parallel (use subagents for many items)
fbd create --title="Implement feature X" --description="Why this issue exists and what needs to be done" --type=feature
fbd create --title="Write tests for X" --description="Why this issue exists and what needs to be done" --type=task
fbd dep add beads-yyy beads-xxx  # Tests depend on Feature (Feature blocks tests)
` + "```" + `
`
	_, _ = fmt.Fprint(w, context)
	return nil
