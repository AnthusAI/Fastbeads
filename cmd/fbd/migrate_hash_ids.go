...
EXAMPLES:
  fbd migrate-hash-ids --dry-run       # Preview changes
  fbd migrate-hash-ids                 # Perform migration (creates backup)
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if jsonOutput {
			...
			if dryRun {
				...
				fmt.Println("\nNext steps:")
				fmt.Println("  1. Run 'fbd export' to update JSONL file")
...
func init() {
	...
	migrateHashIDsAliasCmd.Deprecated = "use 'fbd migrate hash-ids' instead (will be removed in v1.0.0)"
