// Package main implements the fbd CLI dependency management commands.
package main
...
func warnIfCyclesExist(s storage.Storage) {
	...
	fmt.Fprintf(os.Stderr, "\nRun 'fbd dep cycles' for detailed analysis.\n\n")
}
...
When called with an issue ID and --blocks flag, creates a blocking dependency:
  fbd dep <blocker-id> --blocks <blocked-id>

This is equivalent to:
  fbd dep add <blocked-id> <blocker-id>

Examples:
  fbd dep bd-xyz --blocks bd-abc    # bd-xyz blocks bd-abc
  fbd dep add bd-abc bd-xyz         # Same as above (bd-abc depends on bd-xyz)`,
...
The depends-on-id can be provided as:
  - A positional argument: fbd dep add issue-123 issue-456
  - A flag: fbd dep add issue-123 --blocked-by issue-456
  - A flag: fbd dep add issue-123 --depends-on issue-456
...
Examples:
  fbd dep add bd-42 bd-41                              # Positional args
  fbd dep add bd-42 --blocked-by bd-41                 # Flag syntax (same effect)
  fbd dep add bd-42 --depends-on bd-41                 # Alias (same effect)
  fbd dep add gt-xyz external:beads:mol-run-assignee   # Cross-project dependency`,
...
Examples:
  fbd dep list gt-abc                     # Show what gt-abc depends on
  fbd dep list gt-abc --direction=up      # Show what depends on gt-abc
  fbd dep list gt-abc --direction=up -t tracks  # Show what tracks gt-abc (convoy tracking)`,
...
Examples:
  fbd dep tree gt-0iqq                    # Show what blocks gt-0iqq
  fbd dep tree gt-0iqq --direction=up     # Show what gt-0iqq blocks
  fbd dep tree gt-0iqq --status=open      # Only show open issues
  fbd dep tree gt-0iqq --depth=3          # Limit to 3 levels deep`,
	Args: cobra.ExactArgs(1),
	Run: func(cmd *cobra.Command, args []string) {
		ctx := rootCtx
...
func init() {
	...
	depCmd.Flags().StringP("blocks", "b", "", "Issue ID that this issue blocks (shorthand for: fbd dep add <blocked> <blocker>)")
