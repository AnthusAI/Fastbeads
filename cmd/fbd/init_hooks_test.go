...
func TestInstallGitHooks_NoExistingHooks(t *testing.T) {
	...
	runInDir(t, tmpDir, func() {
		...
		if _, err := os.Stat(preCommitPath); err == nil {
			...
			if !strings.Contains(string(content), "fbd (beads)") {
				t.Error("pre-commit hook doesn't contain fbd marker")
			}
		...
		if _, err := os.Stat(postMergePath); err == nil {
			...
			if !strings.Contains(string(content), "fbd (beads)") {
				t.Error("post-merge hook doesn't contain fbd marker")
			}
		}
	})
}
...
func TestInstallGitHooks_ExistingHookBackup(t *testing.T) {
	...
	runInDir(t, tmpDir, func() {
		...
		if !hasExisting {
			t.Error("should detect existing non-fbd hook")
		}
	})
}
