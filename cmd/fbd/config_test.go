...
// run `fbd config set no-db true` without first having a database.
...
func setupTestDB(t *testing.T) (*sqlite.SQLiteStorage, func()) {
	...
	ctx := context.Background()
	if err := store.SetConfig(ctx, "issue_prefix", "fbd"); err != nil {
		store.Close()
		os.RemoveAll(tmpDir)
...
// not SQLite, so that fbd doctor can find it (GH#1531).
