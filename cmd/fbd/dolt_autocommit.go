...
type doltAutoCommitParams struct {
	// Command is the top-level fbd command name (e.g., "create", "update").
	Command string
...
func formatDoltAutoCommitMessage(cmd string, actor string, issueIDs []string) string {
	...
	if len(ids) == 0 {
		return fmt.Sprintf("fbd: %s (auto-commit) by %s", cmd, actor)
	}
	return fmt.Sprintf("fbd: %s (auto-commit) by %s [%s]", cmd, actor, strings.Join(ids, ", "))
}
