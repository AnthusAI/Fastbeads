...
import (
	...
	"github.com/steveyegge/fastbeads/internal/hooks"
	"github.com/steveyegge/fastbeads/internal/molecules"
...
var (
	...
	versionUpgradeDetected = false // Set to true if fbd version changed since last run
	previousVersion        = ""    // The last fbd version user had (empty = first run or unknown)
...
var (
	...
	// explicitly (e.g., fbd sync in dolt-native mode, hook flows, fbd vc commit).
...
var readOnlyCommands = map[string]bool{
	...
	"current":    true, // fbd sync mode current
...
// Priority: --actor flag > FBD_ACTOR env > BD_ACTOR env > BEADS_ACTOR env > git config user.name > $USER > "unknown"
...
func getActorWithGit() string {
	...
	// Check env var overrides (FBD_ACTOR, BD_ACTOR, BEADS_ACTOR)
	if actorEnv := env.GetEnvAlias("ACTOR"); actorEnv != "" {
		return actorEnv
	}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&actor, "actor", "", "Actor name for audit trail (default: $FBD_ACTOR/$BD_ACTOR/$BEADS_ACTOR, git user.name, $USER)")
	...
	// should eventually be consolidated into 'fbd doctor' and 'fbd doctor --fix' to simplify
...
var rootCmd = &cobra.Command{
	Use:   "fbd",
	Short: "fbd - Dependency-aware issue tracker",
	...
	PersistentPreRun: func(cmd *cobra.Command, args []string) {
		...
		if !cmd.Flags().Changed("no-db") {
			...
			if !noDb && strings.EqualFold(config.GetString("storage"), "jsonl") {
				noDb = true
			}
		} else {
		...
		// like "fbd version" that don't need database access.
		noDbCommands := []string{
		...
		// Also skip for --version flag on root command (cmdName would be "fbd")
		if v, _ := cmd.Flags().GetBool("version"); v {
			return
		}

		// Signal orchestrator daemon about fbd activity (best-effort, for exponential backoff)
		...
		if dbPath == "" {
			...
			if foundDB := beads.FindDatabasePath(); foundDB != "" {
				...
				// This enables `fbd show` after cold-start when DB is missing.
				...
				if isReadOnlyCommand(cmd.Name()) && beadsDir != "" {
					...
					if _, err := os.Stat(jsonlPath); err == nil {
						...
						if configuredBackend == configfile.BackendDolt {
							...
							fmt.Fprintf(os.Stderr, "\nHint: run 'fbd doctor --fix' to diagnose and repair\n")
							os.Exit(1)
						}
				...
				if cmd.Name() != "import" && cmd.Name() != "setup" && !isYamlOnlyConfigOp && !canAutoBootstrap {
					...
					if beadsDir != "" {
						...
						if _, err := os.Stat(jsonlPath); err == nil {
							...
							fmt.Fprintf(os.Stderr, "  • Run 'fbd init' to create database and import issues\n")
							fmt.Fprintf(os.Stderr, "  • Use 'fbd --no-db %s' for JSONL-only mode\n", cmd.Name())
					...
					fmt.Fprintf(os.Stderr, "      or use 'fbd --no-db' to work with JSONL only (no SQLite)\n")
					fmt.Fprintf(os.Stderr, "      or set FBD_DIR/BEADS_DIR to point to your .beads directory\n")
					os.Exit(1)
				}
		...
		if backend == configfile.BackendDolt {
			...
			if err != nil && useReadOnly {
				...
				// This handles the case where user runs "fbd list" before "fbd init"
	...
	PersistentPostRun: func(cmd *cobra.Command, args []string) {
		...
		if noDb {
			if store != nil {
				// Determine beads directory (respect FBD_DIR/BEADS_DIR)
				var beadsDir string
				if envDir := env.GetEnvAlias("DIR"); envDir != "" {
	...
	for _, name := range blockedEnvVars {
		if os.Getenv(name) != "" {
			return fmt.Errorf("%s env var is not supported and has been removed to prevent data fragmentation.\n"+
				"The storage backend is set in .beads/metadata.json. To change it, use: fbd migrate dolt (or fbd migrate sqlite)", name)
		}
	}
	return nil
}
...
func main() {
	// FBD_NAME/BD_NAME overrides the binary name in help text (e.g. FBD_NAME=ops makes
	// "ops --help" show "ops" instead of "fbd"). Useful for multi-instance
	// setups where wrapper scripts set BEADS_DIR for routing.
	if name := env.GetEnvAlias("NAME"); name != "" {
		rootCmd.Use = name
	}
