...
var typesCmd = &cobra.Command{
	...
	Long: `List all valid issue types that can be used with fbd create --type.
...
Examples:
  fbd types              # List all types with descriptions
  fbd types --json       # Output as JSON
`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(customTypes) > 0 {
			...
			fmt.Println("Configure with: fbd config set types.custom \"type1,type2,...\"")
		}
	},
}
