...
Similar to how 'git status' shows working tree state, 'fbd status' gives you
...
Examples:
  fbd status                    # Show summary with activity
  fbd status --no-activity      # Skip git activity (faster)
  fbd status --json             # JSON format output
  fbd status --assigned         # Show issues assigned to current user
  fbd stats                     # Alias for fbd status`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Printf("\nFor more details, use 'fbd list' to see individual issues.\n")
		fmt.Println()
