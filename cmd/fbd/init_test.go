...
func TestInitCommand(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			prefix:         "",
			quiet:          false,
			wantOutputText: "fbd initialized successfully",
		},
		{
...
func TestInitMergeDriverAutoConfiguration(t *testing.T) {
	...
	t.Run("skip merge driver with flag", func(t *testing.T) {
		...
		// Run fbd init with --skip-merge-driver
	...
	t.Run("non-git repo skips merge driver silently", func(t *testing.T) {
		...
		// Run fbd init - should succeed even without git
		rootCmd.SetArgs([]string{"init", "--prefix", "test", "--quiet"})
		if err := rootCmd.Execute(); err != nil {
	...
	t.Run("detect already-installed merge driver", func(t *testing.T) {
		...
		if err := runCommandInDir(tmpDir, "git", "config", "merge.beads.driver", "fbd merge %A %O %A %B"); err != nil {
		...
		// Run fbd init - should detect existing config
		rootCmd.SetArgs([]string{"init", "--prefix", "test", "--quiet"})
		if err := rootCmd.Execute(); err != nil {
			t.Fatalf("Init failed: %v", err)
		}
	...
	t.Run("verify git config has correct settings", func(t *testing.T) {
		...
		if !strings.Contains(name, "fbd") {
			t.Errorf("Expected merge.beads.name to contain 'fbd', got %q", name)
		}
	})
	...
	t.Run("auto-repair stale merge driver with invalid placeholders", func(t *testing.T) {
		...
		// This simulates a user who initialized with fbd version <0.24.0
		if err := runCommandInDir(tmpDir, "git", "config", "merge.beads.driver", "fbd merge %L %R"); err != nil {
		...
		// Run fbd init - should detect stale config and repair it
		rootCmd.SetArgs([]string{"init", "--prefix", "test", "--quiet"})
		if err := rootCmd.Execute(); err != nil {
			t.Fatalf("Init failed: %v", err)
		}
	...
	t.Run("detect canonical issues.jsonl filename in gitattributes", func(t *testing.T) {
		...
		// Run fbd init - should detect existing correct config and NOT reinstall
		rootCmd.SetArgs([]string{"init", "--prefix", "test", "--quiet"})
		if err := rootCmd.Execute(); err != nil {
			t.Fatalf("Init failed: %v", err)
		}
...
func TestSetupClaudeSettings_InvalidJSON(t *testing.T) {
  ...
  "permissions": {
	...
	if strings.Contains(string(content), "fbd onboard") {
		t.Error("File should NOT contain fbd onboard prompt after error")
	}
}
...
func TestSetupClaudeSettings_ValidJSON(t *testing.T) {
  ...
  "hooks": {
	...
	if !strings.Contains(contentStr, "fbd onboard") {
		t.Error("File should contain fbd onboard prompt")
	}
...
func TestInitPromptSkippedWithFlags(t *testing.T) {
	t.Run("contributor flag skips prompt and runs wizard", func(t *testing.T) {
		...
		// Run fbd init with --contributor flag (quiet to suppress wizard output)
...
func TestInitPromptNonGitRepo(t *testing.T) {
	...
	// Run fbd init - should succeed without prompting (no git repo)
	rootCmd.SetArgs([]string{"init", "--prefix", "test", "--quiet"})
	if err := rootCmd.Execute(); err != nil {
		t.Fatalf("Init should succeed in non-git directory: %v", err)
	}
...
func TestInitPromptExistingRole(t *testing.T) {
	t.Run("existing role is preserved on reinit with --force", func(t *testing.T) {
		...
		// Run fbd init (non-interactive, so prompt is skipped)
		rootCmd.SetArgs([]string{"init", "--prefix", "test", "--quiet"})
		if err := rootCmd.Execute(); err != nil {
			t.Fatalf("Init failed: %v", err)
		}
...
// TestInitWithRedirect verifies that fbd init creates the database in the redirect target,
...
func TestInitRedirect(t *testing.T) {
	...
	// Verifies that fbd init errors when the redirect target already has a database,
...
// BEADS_DIR Tests
// =============================================================================
// These tests verify that fbd init respects the BEADS_DIR environment variable
...
// TestInit_WithBEADS_DIR_DoltBackend verifies that fbd init with Dolt backend
...
func TestInit_WithBEADS_DIR_DoltBackend(t *testing.T) {
	...
	// Run fbd init with Dolt backend
