...
// TestInstallHooksChainingSkipsBdShim verifies that fbd hooks install --chain
// does NOT rename existing fbd shims to .old (which would cause infinite recursion).
// See: https://github.com/steveyegge/fastbeads/issues/843
func TestInstallHooksChainingSkipsBdShim(t *testing.T) {
	...
	runInDir(t, tmpDir, func() {
		...
		// Create an existing hook that IS a fbd shim
		existingHook := filepath.Join(gitDir, "pre-commit")
		shimContent := "#!/bin/sh\n# bd-shim v1\nexec fbd hooks run pre-commit \"$@\"\n"
		...
		oldPath := existingHook + ".old"
		if _, err := os.Stat(oldPath); !os.IsNotExist(err) {
			t.Errorf("fbd shim was renamed to .old - this would cause infinite recursion!")
		}
...
// .old hooks that are fbd shims (to prevent infinite recursion).
// See: https://github.com/steveyegge/fastbeads/issues/843
func TestRunChainedHookSkipsBdShim(t *testing.T) {
	...
	runInDir(t, tmpDir, func() {
		...
		// Create a .old hook that IS a fbd shim (simulating the problematic state)
		...
		if exitCode != 0 {
			t.Errorf("runChainedHook() = %d, want 0 (should skip fbd shim)", exitCode)
		}
	})
}

// TestGetHookVersionRecognizesInlineHooks verifies that getHookVersion()
// correctly identifies inline fbd hooks created by fbd init.
// See: https://github.com/steveyegge/fastbeads/issues/1120
func TestGetHookVersionRecognizesInlineHooks(t *testing.T) {
	tmpDir := t.TempDir()

	// Test inline hook from fbd init
...
if ! command -v fbd >/dev/null 2>&1; then
    echo "Warning: fbd command not found" >&2
    exit 0
fi
...
`
	...
	if !info.IsBdHook {
		t.Error("getHookVersion() IsBdHook = false, want true for inline fbd hook")
	}
	if info.IsShim {
		t.Error("getHookVersion() IsShim = true, want false for inline fbd hook")
	}
...
// TestInstallHooksChainingSkipsInlineHook verifies that fbd hooks install --chain
// does NOT rename existing inline fbd hooks to .old (which would destroy user's original).
// See: https://github.com/steveyegge/fastbeads/issues/1120
func TestInstallHooksChainingSkipsInlineHook(t *testing.T) {
	...
	runInDir(t, tmpDir, func() {
		...
		// Create an existing hook that IS an inline fbd hook (from fbd init)
		existingHook := filepath.Join(gitDir, "pre-commit")
		inlineContent := `#!/bin/sh
#
# fbd (beads) pre-commit hook (chained)
...
`
		...
		oldPath := existingHook + ".old"
		if _, err := os.Stat(oldPath); !os.IsNotExist(err) {
			t.Errorf("inline fbd hook was renamed to .old - this would destroy user's original hook!")
		}
...
// TestInstallHooksChainingPreservesExistingOld verifies that fbd hooks install --chain
...
func TestInstallHooksChainingPreservesExistingOld(t *testing.T) {
	...
	runInDir(t, tmpDir, func() {
		...
		// Create the user's original hook as .old (simulating fbd init already ran)
		...
		// Create a current hook that is NOT a fbd hook (e.g., user modified it)
...
// .old hooks that are inline fbd hooks (to prevent recursion).
// See: https://github.com/steveyegge/fastbeads/issues/1120
func TestRunChainedHookSkipsInlineHook(t *testing.T) {
	...
	runInDir(t, tmpDir, func() {
		...
		// Create a .old hook that IS an inline fbd hook
		oldHook := filepath.Join(gitDir, "pre-commit.old")
		inlineContent := `#!/bin/sh
#
# fbd (beads) pre-commit hook (chained)
...
`
		...
		exitCode := runChainedHook("pre-commit", nil)
		if exitCode != 0 {
			t.Errorf("runChainedHook() = %d, want 0 (should skip inline fbd hook)", exitCode)
		}
	})
}
