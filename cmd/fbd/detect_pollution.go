...
// showDetectPollutionDeprecationHint shows a hint about fbd doctor consolidation
func showDetectPollutionDeprecationHint() {
	fmt.Fprintln(os.Stderr, ui.RenderMuted("💡 Tip: Use 'fbd doctor --check=pollution' instead (this command is deprecated)"))
}

var detectPollutionCmd = &cobra.Command{
	Use:        "detect-pollution",
	GroupID:    "maint",
	Hidden:     true, // deprecated, use 'fbd doctor --check=pollution' instead
	Deprecated: "use 'fbd doctor --check=pollution' instead (will be removed in v1.0.0)",
...
EXAMPLES:
  fbd detect-pollution                 # Show potential test issues
  fbd detect-pollution --clean         # Delete test issues (with confirmation)
  fbd detect-pollution --clean --yes   # Delete without confirmation
  fbd detect-pollution --json          # Output in JSON format
...
NOTE: Review detected issues carefully before using --clean. False positives are possible.`,
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if !clean {
			fmt.Printf("Run 'fbd detect-pollution --clean' to delete these issues (with confirmation).\n")
		...
		fmt.Printf("\nCleanup complete. To restore, run: fbd import %s\n", backupPath)
	},
}
