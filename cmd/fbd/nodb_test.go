...
func TestDetectPrefix(t *testing.T) {
	...
	t.Run("invalid directory falls back to fbd", func(t *testing.T) {
		memStore := memory.New(filepath.Join(beadsDir, "issues.jsonl"))
		...
		if prefix != "fbd" {
			t.Errorf("Expected fallback prefix 'fbd', got %q", prefix)
		}
	})
}

func TestInitializeNoDbMode_SetsStoreActive(t *testing.T) {
	// This test verifies the fix for fbd comment --no-db not working.
