...
func ensureDatabaseFresh(ctx context.Context) error {
	...
	// returning an error. This allows commands like `fbd show` to work after git pull.
	...
	return fmt.Errorf(
		"Database out of sync with JSONL. Run 'fbd sync --import-only' to fix.\n\n" +
			...
			"To fix:\n" +
			"  fbd sync --import-only            # Import JSONL updates to database\n" +
			"  fbd import -i .beads/issues.jsonl  # Alternative: specify file explicitly\n\n" +
			"If in a sandboxed environment (e.g., Codex) where daemon can't be stopped:\n" +
			"  fbd --sandbox ready               # Use direct mode (no daemon)\n" +
			"  fbd ready --allow-stale           # Skip staleness check (use with caution)\n\n" +
			"Or use daemon mode (auto-imports on every operation):\n" +
			"  fbd daemon start\n" +
			"  fbd <command>     # Will auto-import before executing",
	)
}
