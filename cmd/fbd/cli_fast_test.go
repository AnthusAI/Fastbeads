...
// setupCLITestDB creates a fresh initialized fbd database for CLI tests
...
// runBDInProcess runs fbd commands in-process by calling rootCmd.Execute
...
func runBDInProcess(t *testing.T, dir string, args ...string) string {
	...
	os.Args = append([]string{"fbd"}, args...)
...
func init() {
	// Use existing fbd binary from repo root if available, otherwise build once
	bdBinary := "fbd"
	if runtime.GOOS == "windows" {
		bdBinary = "fbd.exe"
	}

	// Check if fbd binary exists in repo root (../../fbd from cmd/fbd/)
...
// runBDExec runs fbd via exec.Command for end-to-end testing
...
// TestCLI_CreateDryRun tests the --dry-run flag for fbd create command (bd-nib2)
...
// Most fbd commands accept short IDs (e.g., "5wbm") but comments add previously required
