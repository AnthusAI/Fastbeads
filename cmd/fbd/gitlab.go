// Package main provides the fbd CLI commands.
package main
...
import (
	...
	"github.com/steveyegge/fastbeads/internal/gitlab"
...
Configuration can be set via 'fbd config' or environment variables:
...
// getGitLabConfig returns GitLab configuration from fbd config or environment.
...
func validateGitLabConfig(config GitLabConfig) error {
	if config.URL == "" {
		return fmt.Errorf("gitlab.url is not configured. Set via 'fbd config gitlab.url <url>' or GITLAB_URL environment variable")
	}
	if config.Token == "" {
		return fmt.Errorf("gitlab.token is not configured. Set via 'fbd config gitlab.token <token>' or GITLAB_TOKEN environment variable")
	}
	if config.ProjectID == "" {
		return fmt.Errorf("gitlab.project_id is not configured. Set via 'fbd config gitlab.project_id <id>' or GITLAB_PROJECT_ID environment variable")
	}
