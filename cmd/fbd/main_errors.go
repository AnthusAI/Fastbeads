...
func handleFreshCloneError(err error, beadsDir string) bool {
	...
	if jsonlPath != "" && issueCount > 0 {
		...
		fmt.Fprintf(os.Stderr, "  fbd import -i %s\n\n", jsonlPath)
	} else {
		fmt.Fprintf(os.Stderr, "\nTo initialize a new database, run:\n")
		fmt.Fprintf(os.Stderr, "  fbd init --prefix <your-prefix>\n\n")
	}

	fmt.Fprintf(os.Stderr, "For more information: fbd init --help\n")
	return true
}
...
func isWispOperation(cmd *cobra.Command, args []string) bool {
	...
	// fbd mol wisp → parent is "mol", cmd is "wisp"
	// fbd mol wisp create → parent is "wisp", cmd is "create"
	if cmd.Parent() != nil {
