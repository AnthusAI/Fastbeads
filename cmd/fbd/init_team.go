...
func runTeamWizard(ctx context.Context, store storage.Storage) error {
	fmt.Printf("\n%s %s\n\n", ui.RenderBold("fbd"), ui.RenderBold("Team Workflow Setup Wizard"))
	...
	if autoSync {
		...
		fmt.Printf("%s Auto-sync disabled (manual sync with 'fbd sync')\n", ui.RenderWarn("⚠"))
	}
	...
	fmt.Println("  • Use 'fbd list' to see all team's issues")
	...
	if autoSync {
		...
		fmt.Println("  • Run 'fbd sync' manually to sync changes")
	}

	fmt.Println()
	fmt.Printf("Try it: %s\n", ui.RenderAccent("fbd create \"Team planning issue\" -p 2"))
	fmt.Println()
