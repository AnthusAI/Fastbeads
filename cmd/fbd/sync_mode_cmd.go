...
Commands:
  fbd sync mode list      List available sync modes
  fbd sync mode current   Show current sync mode
  fbd sync mode set       Set sync mode`,
}
...
Sync modes control how beads synchronizes with git and Dolt remotes.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("\nSet sync mode: fbd sync mode set <mode>")
	},
}
...
Example:
  fbd sync mode set realtime
  fbd sync mode set dolt-native`,
