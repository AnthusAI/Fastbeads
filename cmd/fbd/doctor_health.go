...
func printCheckHealthHint(issues []string) {
	fmt.Fprintf(os.Stderr, "💡 fbd doctor recommends a health check:\n")
	for _, issue := range issues {
		fmt.Fprintf(os.Stderr, "   • %s\n", issue)
	}
	fmt.Fprintf(os.Stderr, "   Run 'fbd doctor' for details, or 'fbd doctor --fix' to auto-repair\n")
	fmt.Fprintf(os.Stderr, "   (Suppress with: fbd config set %s false)\n", ConfigKeyHintsDoctor)
	os.Exit(1)
}
