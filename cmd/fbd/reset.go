...
Examples:
  fbd reset              # Show what would be deleted
  fbd reset --force      # Actually delete everything`,
	Run: runReset,
}
...
func performReset(items []resetItem, _, beadsDir string) {
	...
	if len(errors) > 0 {
		...
		fmt.Println()
		fmt.Println("To reinitialize beads, run: fbd init")
	}
}
...
func removeGitattributesEntry() error {
	...
	for _, line := range lines {
		...
		if strings.Contains(line, "Use fbd merge for beads JSONL files") {
			skipNextEmpty = true
			continue
		}
