...
// inlineHookMarker identifies inline hooks created by fbd init (GH#1120)
// These hooks have the logic embedded directly rather than using shims
const inlineHookMarker = "# fbd (beads)"
...
// CheckGitHooks checks the status of fbd git hooks in .git/hooks/
func CheckGitHooks() []HookStatus {
	...
	for _, hookName := range hooks {
		...
		if err != nil {
			...
			// Thin shims are never outdated (they delegate to fbd)
			// Legacy hooks are outdated if version differs from current fbd version
...
type hookVersionInfo struct {
	Version  string // fbd version (for legacy hooks) or shim version
	IsShim   bool   // true if this is a thin shim
	IsBdHook bool   // true if this is any type of fbd hook (shim or inline)
}
...
func getHookVersion(path string) (hookVersionInfo, error) {
	...
	// Read first few lines looking for version marker or fbd hook marker
	lineCount := 0
	...
	// Check if it's an inline fbd hook (from fbd init) - GH#1120
	// These don't have version markers but have "# fbd (beads)" comment
	if strings.Contains(content.String(), inlineHookMarker) {
		return hookVersionInfo{IsBdHook: true}, nil
	}

	// No version found and not a fbd hook
...
func FormatHookWarnings(statuses []HookStatus) string {
	...
	if missingCount > 0 {
		...
		warnings = append(warnings, "   Run: fbd hooks install")
	}
...
var hooksCmd = &cobra.Command{
	Use:     "hooks",
	GroupID: "setup",
	Short:   "Manage git hooks for fbd auto-sync",
	Long: `Install, uninstall, or list git hooks that provide automatic fbd sync.
...
var hooksInstallCmd = &cobra.Command{
	Use:   "install",
	Short: "Install fbd git hooks",
	Long: `Install git hooks for automatic fbd sync.
...
Use --chain to preserve existing hooks and run them before fbd hooks. This is
...
var hooksUninstallCmd = &cobra.Command{
	Use:   "uninstall",
	Short: "Uninstall fbd git hooks",
	Long:  `Remove fbd git hooks from .git/hooks/ directory.`,
	Run: func(cmd *cobra.Command, args []string) {
...
var hooksListCmd = &cobra.Command{
	...
	Long:  `Show the status of fbd git hooks (installed, outdated, missing).`,
	Run: func(cmd *cobra.Command, args []string) {
...
func installHooksWithOptions(embeddedHooks map[string]string, force bool, shared bool, chain bool, beadsHooks bool) error {
	...
	for hookName, hookContent := range embeddedHooks {
		...
		if _, err := os.Stat(hookPath); err == nil {
			if chain {
				// Chain mode - rename to .old so fbd hooks run can call it
				// But skip if existing hook is already a fbd hook (shim or inline) - renaming it would
				...
				if verr != nil || !versionInfo.IsBdHook {
					// Not a fbd hook - safe to rename for chaining
					oldPath := hookPath + ".old"
					// Safety check: don't overwrite existing .old file (GH#1120)
					// This prevents destroying user's original hook if fbd hooks install --chain
					// is run multiple times or after fbd init already created .old
				...
				// If it IS a fbd hook, just overwrite it (no rename needed)
			} else if !force {
...
// =============================================================================
// Hook Implementation Functions (called by thin shims via 'fbd hooks run')
// =============================================================================
...
func runChainedHook(hookName string, args []string) int {
	...
	// Check if .old is itself a fbd hook (shim or inline) - skip to prevent infinite recursion
	// This can happen if user runs `fbd hooks install --chain` multiple times,
	// renaming an existing fbd hook to .old. See: GH#843, GH#1120
	versionInfo, err := getHookVersion(oldHookPath)
	if err == nil && versionInfo.IsBdHook {
		// Skip execution - .old is a fbd hook which would call us again
		return 0
	}
...
func runPreCommitHook() int {
	...
	if _, err := os.Stat(".beads"); os.IsNotExist(err) {
		return 0 // Not a fbd workspace, nothing to do
	}
	...
	cmd := exec.Command("fbd", "sync", "--flush-only", "--no-daemon")
	if err := cmd.Run(); err != nil {
		fmt.Fprintln(os.Stderr, "Warning: Failed to flush fbd changes to JSONL")
		fmt.Fprintln(os.Stderr, "Run 'fbd sync --flush-only' manually to diagnose")
	...
	// (e.g., hooks that read the staging area) can set FBD_NO_AUTO_STAGE/BEADS_NO_AUTO_STAGE=1 to
	// disable this and stage manually. See: https://github.com/steveyegge/fastbeads/issues/826
	if env.GetEnvAlias("NO_AUTO_STAGE") != "" {
...
func runPostMergeHook() int {
	...
	// Run fbd sync --import-only --no-git-history
	// Use --no-daemon to ensure direct mode (inline import requires local store)
	cmd := exec.Command("fbd", "sync", "--import-only", "--no-git-history", "--no-daemon")
	output, err := cmd.CombinedOutput()
	if err != nil {
		fmt.Fprintln(os.Stderr, "Warning: Failed to sync fbd changes after merge")
		fmt.Fprintln(os.Stderr, string(output))
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Run 'fbd doctor --fix' to diagnose and repair")
		// Don't fail the merge, just warn
	}

	// Run quick health check
	healthCmd := exec.Command("fbd", "doctor", "--check-health")
...
func runPrePushHook(args []string) int {
	...
	// Skip if fbd sync is already in progress (prevents circular error)
	if env.GetEnvAlias("SYNC_IN_PROGRESS") != "" {
		return 0
	}
	...
	// Flush pending fbd changes
	// Use --no-daemon to ensure direct mode (inline import requires local store)
	flushCmd := exec.Command("fbd", "sync", "--flush-only", "--no-daemon")
	...
	if len(output) > 0 {
		...
		fmt.Fprintln(os.Stderr, "  • fbd JSONL updates (run 'fbd sync')")
		fmt.Fprintln(os.Stderr, "  • any other modified files (run 'git status' to review)")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Run 'fbd sync' to commit these changes:")
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "  fbd sync")
		fmt.Fprintln(os.Stderr, "")
		return 1
	}
...
func runPostCheckoutHook(args []string) int {
	...
	if isGitWorktree() {
		...
		fmt.Fprintln(os.Stderr, "║   - Run: fbd doctor                                                       ║")
	...
	output, err := cmd.CombinedOutput()
	if err != nil {
		fmt.Fprintln(os.Stderr, "Warning: Failed to sync fbd changes after checkout")
		fmt.Fprintln(os.Stderr, string(output))
		fmt.Fprintln(os.Stderr, "")
		fmt.Fprintln(os.Stderr, "Run 'fbd doctor --fix' to diagnose and repair")
...
installed fbd version - upgrading fbd automatically updates hook behavior.`,
...
func init() {
	...
	hooksInstallCmd.Flags().Bool("chain", false, "Chain with existing hooks (run them before fbd hooks)")
