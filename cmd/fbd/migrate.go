	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		if beadsDir == "" {
			if jsonOutput {
				outputJSON(map[string]interface{}{
					...
					"message": "No .beads directory found. Run 'fbd init' first.",
				})
				os.Exit(1)
			} else {
				FatalErrorWithHint("no .beads directory found", "run 'fbd init' to initialize fbd")
			}
		}
		...
		if len(databases) == 0 {
			if jsonOutput {
				...
				fmt.Fprintf(os.Stderr, "Run 'fbd init' to create a new database.\n")
			}
			return
		}
...
func handleDoltMetadataUpdate(cfg *configfile.Config, beadsDir string, dryRun bool) {
	...
	if err != nil || !info.IsDir() {
		if jsonOutput {
			...
			fmt.Fprintf(os.Stderr, "Run 'fbd init' to create a new database, or 'fbd migrate --to-dolt' to migrate from SQLite.\n")
		}
		return
	}
...
func handleUpdateRepoID(dryRun bool, autoYes bool) {
	...
	if beadsDir == "" {
		if jsonOutput {
			outputJSON(map[string]interface{}{
				"error":   "no_database",
				"message": "No beads database found. Run 'fbd init' first.",
			})
		} else {
			fmt.Fprintf(os.Stderr, "Error: no beads database found\n")
			fmt.Fprintf(os.Stderr, "Hint: run 'fbd init' to initialize fbd\n")
		}
		os.Exit(1)
	}
...
func handleInspect() {
	...
	if !dbExists {
		result := map[string]interface{}{
			...
			"warnings":            []string{"Database does not exist - run 'fbd init' first"},
		...
		if jsonOutput {
			...
			fmt.Println("\n⚠ Database does not exist - run 'fbd init' first")
		}
		return
	}
...
func handleToSeparateBranch(branch string, dryRun bool) {
	...
	if _, err := os.Stat(targetPath); os.IsNotExist(err) {
		if jsonOutput {
			outputJSON(map[string]interface{}{
				...
				"message": "Database not found. Run 'fbd init' first.",
			})
		} else {
	...
	if jsonOutput {
		...
		fmt.Printf("     fbd daemon restart\n")
		fmt.Printf("     fbd daemon start --auto-commit\n\n")
