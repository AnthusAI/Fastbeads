...
Commands:
  fbd dolt show         Show current Dolt configuration with connection test
  fbd dolt set <k> <v>  Set a configuration value
  fbd dolt test         Test server connection

Configuration keys for 'fbd dolt set':
...
Flags for 'fbd dolt set':
  --update-config  Also write to config.yaml for team-wide defaults

Examples:
  fbd dolt set mode server
  fbd dolt set database myproject
  fbd dolt set host 192.168.1.100 --update-config
  fbd dolt test`,
}
...
Examples:
  fbd dolt set mode server
  fbd dolt set database myproject
  fbd dolt set host 192.168.1.100
  fbd dolt set port 3307 --update-config`,
...
func showDoltConfig(testConnection bool) {
	...
	if backend != configfile.BackendDolt {
		...
		fmt.Println("\nTo migrate to Dolt: fbd migrate --to-dolt")
		return
	}
...
func setDoltConfig(key, value string, updateConfig bool) {
	...
	if cfg.GetBackend() != configfile.BackendDolt {
		...
		fmt.Fprintf(os.Stderr, "To migrate to Dolt: fbd migrate --to-dolt\n")
		os.Exit(1)
	}
...
func testDoltConnection() {
	...
	if testServerConnection(cfg) {
		...
		fmt.Println("  fbd dolt set mode server")
	} else {
