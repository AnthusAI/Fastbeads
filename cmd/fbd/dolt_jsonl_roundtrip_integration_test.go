...
func TestDoltJSONLRoundTrip_DepsLabelsCommentsTombstones(t *testing.T) {
	...
	// creating tombstones via `fbd delete`, but it must round-trip tombstones via JSONL).
	now := time.Now().UTC()
	...
	if initErr2 != nil {
		...
		t.Fatalf("fbd init --backend dolt (ws2) failed: %v\n%s", initErr2, initOut2)
	}
	...
	if out, err := runBDExecAllowErrorWithEnv(t, ws2, env, "export", "-o", jsonl2out); err != nil {
		t.Fatalf("fbd export (ws2) failed: %v\n%s", err, out)
	}
