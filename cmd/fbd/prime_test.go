...
func TestOutputContextFunction(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			ephemeralMode: false,
			localOnlyMode: false,
			expectText:    []string{"Beads Workflow Context", "fbd sync", "git push"},
			rejectText:    []string{"fbd sync --flush-only", "--from-main"},
		},
		{
			...
			ephemeralMode: true,
			localOnlyMode: false,
			expectText:    []string{"Beads Workflow Context", "fbd sync --from-main", "ephemeral branch"},
			rejectText:    []string{"fbd sync --flush-only", "git push"},
		},
		{
			name:          "CLI Stealth",
			mcpMode:       false,
			...
			localOnlyMode: false,
			expectText:    []string{"Beads Workflow Context", "fbd sync --flush-only"},
		...
		},
		{
			...
			ephemeralMode: false,
			localOnlyMode: true,
			expectText:    []string{"Beads Workflow Context", "fbd sync --flush-only", "No git remote configured"},
		...
		},
		{
			...
			ephemeralMode: false,
			localOnlyMode: false,
			expectText:    []string{"Beads Issue Tracker Active", "fbd sync", "git push"},
			rejectText:    []string{"fbd sync --flush-only", "--from-main"},
		},
		{
			...
			ephemeralMode: true,
			localOnlyMode: false,
			expectText:    []string{"Beads Issue Tracker Active", "fbd sync --from-main", "ephemeral branch"},
			rejectText:    []string{"fbd sync --flush-only", "git push"},
		},
		{
			name:          "MCP Stealth",
			...
			ephemeralMode: false, // stealth mode overrides ephemeral detection
			localOnlyMode: false,
			expectText:    []string{"Beads Issue Tracker Active", "fbd sync --flush-only"},
			rejectText:    []string{"git push", "git pull", "git commit", "git status", "git add"},
		},
		{
