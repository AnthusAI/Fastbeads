...
This document describes how fbd tracks and propagates deletions across repository clones.
...
```bash
fbd delete bd-42                    # Delete single issue (preview mode)
fbd delete bd-42 --force            # Actually delete
fbd delete bd-42 bd-43 bd-44 -f     # Delete multiple issues
fbd delete bd-42 --cascade -f       # Delete with all dependents
fbd delete --from-file ids.txt -f   # Delete from file (one ID per line)
fbd delete bd-42 --dry-run          # Preview what would be deleted
```

### Viewing Deleted Issues

```bash
fbd list --status=tombstone         # List all tombstones
fbd show bd-42                      # View tombstone details (if you know the ID)
```
...
2. `fbd admin compact` removes expired tombstones from `issues.jsonl`
...
Or via CLI:
```bash
fbd config set tombstone.ttl_days 60
```

### Manual Pruning

```bash
fbd admin compact                   # Prune expired tombstones (and other compaction)
```
...
```bash
fbd migrate tombstones              # Convert deletions.jsonl to inline tombstones
fbd migrate tombstones --dry-run    # Preview changes first
```
...
After migration, run `fbd sync` to propagate tombstones to other clones.
...
fbd list --status=tombstone | grep bd-xxx
...
fbd list --status=tombstone | wc -l
...
**Wisps** (ephemeral issues created by `fbd mol wisp`) are intentionally excluded from tombstone tracking.
...
When `fbd mol squash` compresses wisps into a digest:
