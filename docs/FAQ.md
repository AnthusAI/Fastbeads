...
Common questions about fbd (beads) and how to use it effectively.

## General Questions

### What is fbd?

fbd is a lightweight, git-based issue tracker designed for AI coding agents. It provides dependency-aware task management with automatic sync across machines via git.
...
1. **Typed Dependencies with Semantics**
   - fbd: Four types (`blocks`, `related`, `parent-child`, `discovered-from`) with different behaviors
...
2. **Deterministic Ready-Work Detection**
   - fbd: `fbd ready` computes transitive blocking offline in ~10ms, no network required
...
3. **Git-First, Offline, Branch-Scoped Task Memory**
   - fbd: Works offline, issues live on branches, hash IDs prevent collisions on merge
...
4. **AI-Resolvable Conflicts & Duplicate Merge**
   - fbd: Automatic collision resolution, duplicate merge with dependency consolidation and reference rewriting
   - GH: Manual close-as-duplicate, no safe bulk merge, no cross-reference updates

5. **Extensible Local Database**
   - fbd: Add SQL tables and join with issue data locally (see [EXTENDING.md](EXTENDING.md))
   - GH: No local database; would need to mirror data externally

6. **Agent-Native APIs**
   - fbd: Consistent `--json` on all commands, dedicated MCP server with auto workspace detection
   - GH: Mixed JSON/text output, GraphQL requires custom queries, no agent-focused MCP layer

**When to use each:** GitHub Issues excels for human teams in web UI with cross-repo dashboards and integrations. fbd excels for AI agents needing offline, git-synchronized task memory with graph semantics and deterministic queries.

See [GitHub issue #125](https://github.com/steveyegge/fastbeads/issues/125) for detailed comparison.

### How is this different from Taskwarrior?

Taskwarrior is excellent for personal task management, but fbd is built for AI agents:

- **Explicit agent semantics**: `discovered-from` dependency type, `fbd ready` for queue management
...
### Can I use fbd without AI agents?

Absolutely! fbd is a great CLI issue tracker for humans too. The `fbd ready` command is useful for anyone managing dependencies. Think of it as "Taskwarrior meets git."

### Is this production-ready?

**Current status: Alpha (v0.9.11)**

fbd is in active development and being dogfooded on real projects. The core functionality (create, update, dependencies, ready work, collision resolution) is stable and well-tested. However:
...
fbd create "Add OAuth"  # Sequential ID: bd-10

# Branch B also creates bd-10
git checkout -b feature-payments
fbd create "Add Stripe"  # Collision! Same sequential ID: bd-10
...
```bash
# Branch A
fbd create "Add OAuth"  # Hash ID: bd-a1b2 (from random UUID)

# Branch B
fbd create "Add Stripe"  # Hash ID: bd-f14c (different UUID, different hash)
...
fbd automatically extends hash length as your database grows to maintain low collision probability.
...
fbd create "Login UI" -p 1       # bd-a3f8e9.1
fbd create "Validation" -p 1     # bd-a3f8e9.2
fbd create "Tests" -p 1          # bd-a3f8e9.3
```
...
- Cross-cutting dependencies (use `fbd dep add` instead)

### Should I run fbd init or have my agent do it?

**Either works!** But use the right flag:

**Humans:**
```bash
fbd init  # Interactive - prompts for git hooks
```

**Agents:**
```bash
fbd init --quiet  # Non-interactive - auto-installs hooks, no prompts
```

**Workflow for humans:**
```bash
# Clone existing project with fbd:
git clone <repo>
cd <repo>
fbd init  # Auto-imports from .beads/issues.jsonl

# Or initialize new project:
cd ~/my-project
fbd init  # Creates .beads/, sets up daemon
git add .beads/
...
git clone <repo>
cd <repo>
fbd init --quiet  # No prompts, auto-installs hooks
fbd ready --json  # Start using fbd normally
```
...
**How auto-import works:** The first fbd command after `git pull` detects that `.beads/issues.jsonl` is newer than the database and automatically imports it. There's no background daemon watching for changes - the check happens when you run a fbd command.
...
```bash
fbd --no-auto-flush create "Issue"   # Disable auto-export
fbd --no-auto-import list            # Disable auto-import check
```

### What if my database feels stale after git pull?

Just run any fbd command - it will auto-import:

```bash
git pull
fbd ready     # Automatically imports fresh data from git
fbd list      # Also triggers auto-import if needed
fbd sync      # Explicit sync command for manual control
```
...
**Yes! Each project is completely isolated.** fbd uses project-local databases:

```bash
cd ~/project1 && fbd init --prefix proj1
cd ~/project2 && fbd init --prefix proj2
```

Each project gets its own `.beads/` directory with its own database and JSONL file. fbd auto-discovers the correct database based on your current directory (walks up like git).
...
- Agents in subdirectories → fbd walks up to find the project root (like git)
- **Per-project daemons** → Each project gets its own daemon at `.beads/fbd.sock` (LSP model)

**Limitation:** Issues cannot reference issues in other projects. Each database is isolated by design. If you need cross-project tracking, initialize fbd in a parent directory that contains both projects.

**Example:** Multiple agents, multiple projects, same machine:
```bash
# Agent 1 working on web app
cd ~/work/webapp && fbd ready --json    # Uses ~/work/webapp/.beads/webapp.db

# Agent 2 working on API
cd ~/work/api && fbd ready --json       # Uses ~/work/api/.beads/api.db

# No conflicts! Completely isolated databases and daemons.
```

**Architecture:** fbd uses per-project daemons (like LSP/language servers) for complete database isolation. See [ADVANCED.md#architecture-daemon-vs-mcp-vs-beads](ADVANCED.md#architecture-daemon-vs-mcp-vs-beads).
...
- Have agents claim work with `fbd update <id> --status in_progress`
- Query by assignee: `fbd ready --assignee agent-name`
- Review git diffs before merging

For true multi-agent coordination, you'd need additional tooling (like locks or a coordination server). fbd handles the simpler case: multiple humans/agents working on different tasks, syncing via git.
...
If you import an issue with the same ID but different fields, fbd treats it as an update to the existing issue. This is normal behavior - hash IDs remain stable, so same ID = same issue being updated.

For git conflicts where the same issue was modified on both branches, manually resolve the JSONL conflict (usually keeping the newer `updated_at` timestamp), then `fbd import` will apply the update.
...
2. Write a simple script to convert to fbd's JSONL format
3. Import with `fbd import -i issues.jsonl`
...
1. Export from fbd: `fbd export -o issues.jsonl --json`
...
### How does fbd handle scale?

fbd uses SQLite, which handles millions of rows efficiently. For a typical project with thousands of issues:
...
```bash
cd ~/project/frontend && fbd init --prefix fe
cd ~/project/backend && fbd init --prefix be
```

## Use Case Questions

### Can I use fbd for non-code projects?

Sure! fbd is just an issue tracker. Use it for:
...
### Can I use fbd with multiple AI agents simultaneously?

Yes! Each agent can:

1. Query ready work: `fbd ready --assignee agent-name`
2. Claim issues: `fbd update <id> --status in_progress --assignee agent-name`
3. Create discovered work: `fbd create "Found issue" --deps discovered-from:<parent-id>`
4. Sync via git commits

fbd's git-based sync means agents work independently and merge their changes like developers do.

### Does fbd work offline?

Yes! fbd is designed for offline-first operation:
...
### What dependencies does fbd have?

fbd is a single static binary with no runtime dependencies:
...
### Does fbd support Windows?

Yes! fbd has native Windows support (v0.9.0+):
...
### Can I use fbd with git worktrees?
...
- Run `fbd init --branch <name>`
- Set `fbd config set sync.branch <name>`
...
fbd handles two distinct types of integrity issues:
...
- **GitHub Issues**: [Report bugs or request features](https://github.com/steveyegge/fastbeads/issues)
- **GitHub Discussions**: [Ask questions](https://github.com/steveyegge/fastbeads/discussions)
...
The roadmap lives in fbd itself! Run:

```bash
fbd list --priority 0 --priority 1 --json
```
