# Extending fbd with Custom Tables

> **⚠️ DEPRECATED**: This document describes extending fbd via custom SQLite tables.
> This approach is being deprecated as fbd migrates from SQLite to Dolt as the
...
> These tools use fbd's CLI with `--json` flags or read/write the `.beads/` JSONL files directly.
>
> **For orchestration layers**, consider using a separate database that references
> fbd issue IDs, or wait for the forthcoming plugin architecture.

fbd is designed to be extended by applications that need more than basic issue tracking. The recommended pattern is to add your own tables to the same SQLite database that fbd uses.

## Philosophy

**fbd is focused** - It tracks issues, dependencies, and ready work. That's it.
...
### 2. Use fbd for Issue Management
...
```go
// Complex query joining fbd's issues with your execution data
query := `
SELECT
    i.id,
    i.title,
...
Here's how the VC (VibeCoder) orchestrator extends fbd using `UnderlyingDB()`:
...
type VCStorage struct {
    beads.Storage  // Embed fbd's storage
...
- ✅ VC extends fbd without forking or modifying it
- ✅ Single database = simple JOINs across layers
- ✅ Foreign keys ensure referential integrity
- ✅ fbd handles issue tracking, VC handles orchestration
- ✅ Can use fbd's CLI alongside VC's custom operations
...
Use fbd's `--json` flags for scripting:

```bash
#!/bin/bash

# Find ready work
READY=$(fbd ready --limit 1 --json)
...
# Claim issue in fbd
fbd update $ISSUE_ID --status in_progress

# Execute work...
echo "Working on $ISSUE_ID"

# Mark complete
fbd close $ISSUE_ID --reason "Completed by agent-1"
sqlite3 .beads/myapp.db <<SQL
...
The recommended way to extend fbd is using the `UnderlyingDB()` method on the storage instance. This gives you access to the same database connection that fbd uses, ensuring consistency and avoiding connection overhead:
...
⚠️ **DO NOT** modify database pool settings (SetMaxOpenConns, SetConnMaxIdleTime) or SQLite PRAGMAs (WAL mode, journal settings) as this affects fbd's core operations.

⚠️ **Keep transactions short** - Long write transactions will block fbd's core operations. Use read transactions when possible.
...
✅ **DO** use foreign keys to reference fbd's tables for referential integrity.
...
- ✅ When you need tables in the same database as fbd
...
// Auto-discover fbd's database path
dbPath := beads.FindDatabasePath()
if dbPath == "" {
    log.Fatal("No fbd database found. Run 'fbd init' first.")
}
...
- Interactive CLI commands (`fbd create`)
...
func ImportFromExternal(externalIssues []ExternalIssue) error {
    ...
    // Convert external format to fbd format
...
The key insight: **fbd is a focused issue tracker, not a framework**.
...
- [README.md](../README.md) - Complete fbd documentation
