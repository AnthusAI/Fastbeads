...
fbd create "Fix auth bug" -t bug -p 1 -l auth,backend,urgent

# Add labels to existing issues
fbd label add bd-42 security
fbd label add bd-42 breaking-change

# List issue labels
fbd label list bd-42

# Remove a label
fbd label remove bd-42 urgent

# List all labels in use
fbd label list-all

# Filter by labels (AND - must have ALL)
fbd list --label backend,auth

# Filter by labels (OR - must have AT LEAST ONE)
fbd list --label-any frontend,backend

# Combine filters
fbd list --status open --priority 1 --label security
```
...
**Example:**
```bash
fbd create "Add GraphQL endpoint" -t feature -p 2 -l backend,api
fbd create "Update login form" -t task -p 2 -l frontend,auth,ui
```
...
**Example:**
```bash
fbd list --label payments --status open  # All open payment issues
fbd list --label-any auth,security       # Security-related work
```
...
fbd ready --json | jq '.[] | select(.labels[] == "small")'
```
...
**Example:**
```bash
fbd label add bd-42 needs-review
fbd list --label needs-review --status in_progress
```
...
**Example:**
```bash
fbd list --label v1.0 --status open    # What's left for v1.0?
fbd label add bd-42 release-blocker
```
...
**Example:**
```bash
fbd list --assignee alice --label team-infra
fbd create "Memory leak in cache" -t bug -p 1 -l team-infra,help-wanted
```
...
**Example:**
```bash
fbd create "TODO: Refactor parser" -t chore -p 3 -l technical-debt,auto-generated
```
...
fbd list --label backend,urgent

# Open bugs that need review AND tests
fbd list --status open --type bug --label needs-review,needs-tests
```
...
fbd list --label-any security,auth
```
...
fbd list --label backend --label-any urgent,release-blocker

# Frontend work that needs BOTH review and tests, but in any component
fbd list --label needs-review,needs-tests --label-any frontend,ui,mobile
```
...
fbd create "Crash on login" -t bug -p 1 -l needs-triage

# During triage, add context
fbd label add bd-42 auth
fbd label add bd-42 backend
fbd label add bd-42 urgent
fbd label remove bd-42 needs-triage

# Find untriaged issues
fbd list --label needs-triage
```
...
fbd label add bd-42 needs-tests
fbd label add bd-42 needs-docs

# Before closing, verify
fbd label list bd-42
# ... write tests and docs ...
fbd label remove bd-42 needs-tests
fbd label remove bd-42 needs-docs
...
```bash
# Tag issues for v1.0
fbd label add bd-42 v1.0
fbd label add bd-43 v1.0
fbd label add bd-44 v1.0

# Track v1.0 progress
fbd list --label v1.0 --status closed    # Done
fbd list --label v1.0 --status open      # Remaining
fbd stats  # Overall progress

# Mark critical items
fbd label add bd-45 v1.0
fbd label add bd-45 release-blocker
```

### Component-Based Work Distribution
```bash
# Backend team picks up work
fbd ready --json | jq '.[] | select(.labels[]? == "backend")'

# Frontend team finds small tasks
fbd list --status open --label frontend,small

# Find help-wanted items for new contributors
fbd list --label help-wanted,good-first-issue
```
...
```bash
fbd create "Issue" -l label1,label2,label3
```

Script to add label to multiple issues:
```bash
# Add "needs-review" to all in_progress issues
fbd list --status in_progress --json | jq -r '.[].id' | while read id; do
  fbd label add "$id" needs-review
done
```
...
fbd list --status closed --label urgent --json | jq -r '.[].id' | while read id; do
  fbd label remove "$id" urgent
done
```
...
```bash
# Make changes
fbd create "Fix bug" -l backend,urgent
fbd label add bd-42 needs-review
...
git pull
fbd list --label backend  # Fresh data including labels
```
...
fbd create "Auth system rewrite" -t epic -p 1 -l auth,v2.0
fbd create "Implement JWT" -t task -p 1 -l auth,backend --deps parent-child:bd-42
fbd create "Update login UI" -t task -p 1 -l auth,frontend --deps parent-child:bd-42

# Find all v2.0 auth work
fbd list --label auth,v2.0
```
...
fbd create "Found TODO in auth.go" -t task -p 2 -l auto-generated,technical-debt

# Filter for agent review
fbd list --label needs-review --status in_progress --json

# Track automation metadata
fbd label add bd-42 ai-generated
fbd label add bd-42 needs-human-review
```

Example agent workflow:
```bash
# Agent discovers issues during refactor
fbd create "Extract validateToken function" -t chore -p 2 \
...
fbd label remove bd-42 needs-review
fbd label add bd-42 approved
fbd close bd-42
```
...
fbd create "Muted patrol: user requested during debugging" -t event \
  -l event-type:patrol-muted,actor:witness,reason:user-request

# State: Update the role bead's label to reflect current state
fbd label remove beads/witness patrol:active
fbd label add beads/witness patrol:muted
```
...
fbd list --type event | grep "patrol" | tail -1  # Slow, fragile

# With labels-as-state: direct query
fbd show beads/witness | grep "patrol:"  # Instant
```

**History preserved:**
```bash
# When was patrol muted? Why? Who did it?
fbd list --label event-type:patrol-muted --type event
```

**State recovery:**
```bash
# If labels get corrupted, rebuild from events
fbd list --type event --label event-type:patrol-muted | tail -1
...
transition_state() {
  ...
  fbd create "State change: $dimension $old_value → $new_value" -t event \
    -l "event-type:state-change,dimension:$dimension,from:$old_value,to:$new_value"

  # Update the cache
  fbd label remove "$role" "$dimension:$old_value"
  fbd label add "$role" "$dimension:$new_value"
}
...
fbd label list beads/witness | grep ":"

# All roles in a specific state
fbd list --label patrol:muted

# Roles NOT in expected state
fbd list --label-any mode:degraded,health:failing

# History of state changes
fbd list --type event --label event-type:state-change
```
...
```bash
# Query current state
fbd state beads/witness patrol     # → "muted"

# Transition with automatic event creation
fbd set-state beads/witness patrol=active --reason "Debugging complete"
```
...
fbd list --label backend,auth --priority 1

# Any frontend work that's small
fbd list --label-any frontend,ui --label small

# Critical issues across all components
fbd list --priority 0 --label-any backend,frontend,infrastructure
```
...
for id in bd-42 bd-43 bd-44 bd-45; do
  fbd label add "$id" sprint-12
done

# Track sprint progress
fbd list --label sprint-12 --status closed    # Velocity
fbd list --label sprint-12 --status open      # Remaining
fbd stats | grep "In Progress"                # Current WIP
```

### Technical Debt Tracking
```bash
# Mark debt
fbd create "Refactor legacy parser" -t chore -p 3 -l technical-debt,large

# Find debt to tackle
fbd list --label technical-debt --label small
fbd list --label technical-debt --priority 1  # High-priority debt
```
...
fbd label add bd-42 breaking-change
fbd label add bd-42 v2.0

# Find all breaking changes for next major release
fbd list --label breaking-change,v2.0

# Ensure they're documented
fbd list --label breaking-change --label needs-docs
```
...
fbd create "Muted patrol for witness-abc" -t event \
  --parent witness-abc \
  -d "Reason: investigating stuck polecat. Expected duration: 30m"

# 2. Update the cached state label
fbd label remove witness-abc patrol:active
fbd label add witness-abc patrol:muted
```
...
|----------|-------------|-----------------|
| Query current state | Scan all events, find latest | `fbd list --label patrol:muted` |
...
fbd create "witness-alpha" -t role -l patrol:active,mode:normal,health:healthy

# Agent enters degraded mode
fbd create "Degraded: high error rate" -t event --parent witness-alpha \
  -d "Error rate exceeded 5%. Reducing poll frequency."
fbd label remove witness-alpha mode:normal
fbd label add witness-alpha mode:degraded

# Query current state
fbd list --label mode:degraded --type role  # All degraded roles

# Agent recovers
fbd create "Recovered: error rate normal" -t event --parent witness-alpha
fbd label remove witness-alpha mode:degraded
fbd label add witness-alpha mode:normal
```
...
fbd list --label health:healthy,mode:normal

# Find any non-healthy agents
fbd list --label-any health:warning,health:failing

# Get state for a specific role
fbd label list witness-alpha
...
fbd state witness-alpha patrol
# Output: active

# List all state dimensions
fbd state list witness-alpha
# Output:
#   patrol: active
#   mode: normal
#   health: healthy
...
fbd set-state witness-alpha patrol=muted --reason "Investigating issue"
```
...
Labels require explicit fetching. The `fbd list` command shows issues but not labels in human output (only in JSON).

```bash
# See labels in JSON
fbd list --json | jq '.[] | {id, labels}'

# See labels for specific issue
fbd show bd-42 --json | jq '.labels'
fbd label list bd-42
```
...
fbd label add bd-42 Backend    # Capital B
fbd list --label backend       # Won't match
