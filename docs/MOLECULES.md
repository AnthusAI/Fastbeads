...
4. **Bonding = adding dependencies.** `fbd mol bond A B` creates a dependency between work graphs.
...
**Ready work:** `fbd ready` shows issues with no open blockers.
**Blocked work:** `fbd blocked` shows what's waiting.
...
fbd create "Task A" -t task
fbd create "Task B" -t task
fbd create "Task C" -t task

# Add dependency to make B wait for A
fbd dep add <B-id> <A-id>   # B depends on A (B needs A)
```

### Multi-Day Execution

An agent works through a molecule by:
1. Getting ready work (`fbd ready`)
2. Claiming it (`fbd update <id> --status in_progress`)
3. Doing the work
4. Closing it (`fbd close <id>`)
...
```bash
fbd create "Feature X" -t epic
fbd create "Design" -t task --parent <epic-id>
fbd create "Implement" -t task --parent <epic-id>
fbd create "Test" -t task --parent <epic-id>
fbd dep add <implement-id> <design-id>   # implement needs design
fbd dep add <test-id> <implement-id>      # test needs implement
```
...
```bash
fbd mol bond A B                    # B depends on A (sequential by default)
fbd mol bond A B --type parallel    # Organizational link, no blocking
fbd mol bond A B --type conditional # B runs only if A fails
```
...
```bash
fbd mol pour <proto>              # Proto → Mol (persistent instance)
fbd mol wisp <proto>              # Proto → Wisp (ephemeral instance)
fbd mol squash <id>               # Mol/Wisp → Digest (permanent record)
fbd mol burn <id>                 # Wisp → nothing (discard)
```
...
```bash
fbd create "Pipeline" -t epic
fbd create "Step 1" -t task --parent <pipeline>
fbd create "Step 2" -t task --parent <pipeline>
fbd create "Step 3" -t task --parent <pipeline>
fbd dep add <step2> <step1>
fbd dep add <step3> <step2>
```

### Parallel Fanout with Gate

```bash
fbd create "Process files" -t epic
fbd create "File A" -t task --parent <epic>
fbd create "File B" -t task --parent <epic>
fbd create "File C" -t task --parent <epic>
fbd create "Aggregate" -t task --parent <epic>
# Aggregate needs all three (waits-for gate)
fbd dep add <aggregate> <fileA> --type waits-for
```
...
for polecat in $(gt polecat list); do
  fbd mol bond mol-polecat-arm $PATROL_ID --ref arm-$polecat --var name=$polecat
done
```
...
**Wrong:** "Phase 1 comes before Phase 2" → `fbd dep add phase1 phase2`
**Right:** "Phase 2 needs Phase 1" → `fbd dep add phase2 phase1`

Use requirement language. Verify with `fbd blocked`.
...
```bash
fbd mol wisp list        # Check for orphans
fbd mol squash <id>      # Create digest
fbd mol burn <id>        # Or discard
fbd mol wisp gc          # Garbage collect old wisps
```
...
```bash
fbd ready                         # What's ready to work
fbd blocked                       # What's blocked
fbd update <id> --status in_progress
fbd close <id>
```
...
```bash
fbd dep add <issue> <depends-on>  # issue needs depends-on
fbd dep tree <id>                 # Show dependency tree
```

### Molecules

```bash
fbd mol pour <proto> --var k=v    # Template → persistent mol
fbd mol wisp <proto>              # Template → ephemeral wisp
fbd mol bond A B                  # Connect work graphs
fbd mol squash <id>               # Compress to digest
fbd mol burn <id>                 # Discard without record
```
